...
export default async function handler(req, res) {
  ...
  const { title, description, priceCents, unlimited, kind, shippingCents, signatureRequired } = req.body || {};
  ...
  try {
    const listing = await createListing(ctx.creator.id, { title, description, priceCents, unlimited, kind, shippingCents, signatureRequired });
