...
/**
 * Account-level moderation for fan (non-creator) accounts -- see
 ...
 *   -> 404 no such user; 400 bad input, or a creator account (their standing
 *      is set on the creator profile, POST /api/admin/profile).
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (err.code === USER_MODERATION_NOT_ALLOWED) {
      return res.status(400).json({ error: "That's a creator account -- suspend or ban it from its creator profile instead." });
    }
