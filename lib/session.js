 ...
 * kept working for the rest of its 30 days no matter how many times they
 * logged out. This additionally checks the token's `sv` against the
 * session version currently on the user record, which POST
 * /api/auth/logout bumps -- so logging out really does kill the tokens
 * issued before it.
 *
 * The catch is that this costs a storage read and is therefore async,
 * while getSessionUserId() is called synchronously from a dozen other API
 * routes and getServerSideProps functions. Those are still on the
 * stateless check; moving them over is mechanical (they're all already
 * async, so it's `await getSessionUser(req)` and read `.id`) but touches
