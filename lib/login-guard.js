 ...
 * to look the account up; the brake reads and writes happen in two batched
 * statements before the bcrypt compare and one after.
 *
...
export async function releaseLoginAttempts(keys, { windowMs = LOGIN_WINDOW_MS } = {}) {
  ...
  await query(
    `update login_attempts set attempts = greatest(attempts - 1, 0)
      where key = any($1::text[])
        and window_start > now() - make_interval(secs => $2::double precision / 1000)`,
    [list.map(keyDigest), windowMs],
  );
}
...
export async function clearLoginCounters(keys) {
  const list = keys.filter(Boolean);
  if (!list.length) return;
  await query(`delete from login_attempts where key = any($1::text[])`, [list.map(keyDigest)]);
}
