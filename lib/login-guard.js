...
/**
 * Every logical key the login route uses, built in one place (round-16
 * gates-token#0). The typed identifier is untrusted and may contain ':' and
 * '/', so it is never concatenated in raw: it is reduced to a fixed-length
 * hex digest first. A 64-hex digest contains no ':', so an identifier such as
 * 'alice:from:203.0.113.5' can no longer produce the byte-for-byte key of
 * alice's per-host failure marker (which let anyone raise the owner's own
 * "dirty" marker and lock them out). Each kind also has its own prefix, so no
 * kind's key can equal another kind's.
 */
export function loginGuardKeys({ identifier, ip, net = null }) {
  const idPart = crypto.createHash('sha256').update(String(identifier).toLowerCase(), 'utf8').digest('hex');
  const accountKey = `login:account:${idPart}`;
  return {
    ipKey: `login:ip:${ip}`,
    netKey: net ? `login:net:${net}` : null,
    accountKey,
    // Markers, not budgets (see pages/api/auth/login.js).
    accountFromIpKey: `login:account-from:${idPart}:${ip}`,
    accountFromNetKey: net ? `login:account-fromnet:${idPart}:${net}` : null,
  };
}
