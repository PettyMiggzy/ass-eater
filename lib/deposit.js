...
import { creditAccount, accountStanding, isFrozenStanding } from './credits-store';
...
// The account is banned or suspended (its credits are frozen). Checked inside
// the crediting transaction, so a ban landing while /api/credits/buy waited
// for the receipt is still refused -- and the hash is NOT claimed.
 ...
 */
export async function creditDepositFromChain({ userId, txHash, expectedFrom, config, allowFrozen = false }) {
  ...
  return recordDepositCredit({ userId, txHash, grossCents, feeCents, netCents, allowFrozen });
}
 ...
 *
 * Returns { creditedCents, feeCents, grossCents, balanceCents } and, when
 * this exact hash was ALREADY credited to this same account (a retry after a
 * lost response -- the page's "Verify this transaction" recovery),
 * `alreadyCredited: true` with the original figures instead of an error. A
 * hash claimed by any other account (or by anything but a deposit) is still
 * TX_ALREADY_USED.
 *
 * `allowFrozen` lets a frozen (banned/suspended) account be credited anyway;
 * only the admin manual-credit tool passes it, and only on the admin's
 * explicit `creditFrozen: true`.
 */
export async function recordDepositCredit({ userId, txHash, grossCents, feeCents, netCents, allowFrozen = false }) {
  return withTransaction(async (client) => {
    // The account must still exist and be in good standing, checked FIRST
    // and before the hash is claimed. /api/credits/buy checks the session
    // (and the standing) once, then waits up to a minute for the receipt:
    //  - an account deleted meanwhile used to have the deposit credited to a
    //    user id that no longer existed (credit_balances has no foreign key
    //    to users), the USDG kept and the hash used up for good;
    //  - an account BANNED or suspended meanwhile had the payment parked in a
    //    frozen balance that can never be spent or withdrawn, the hash used
    //    up, and a 200 "credited" answer.
    // accountStanding's lock takes the users row (then the creator row) FOR
    // SHARE, which also serialises against deleteFanAccount's and
    // deleteCreator's `users ... for update`, and against a ban updating the
    // creator row: a deletion or ban commits first and is seen here, or this
    // commits first and the deletion sees the credit.
    const standing = await accountStanding(String(userId), client, { lock: true });
    if (!standing.user) {
      throw Object.assign(
    ...
    // Already credited to THIS account: answer with that credit. Checked
    // before the frozen refusal, because saying "not credited" about money
    // that was credited is exactly what sent a fan paying twice.
    const prior = await priorDepositCredit(client, userId, txHash);
    if (prior) return prior;
    if (isFrozenStanding(standing) && !allowFrozen) {
      throw Object.assign(
        new Error('Your account is suspended or banned, so its credits are frozen and this payment was not credited to it. Contact team@onlyone1.fun with the transaction hash.'),
        { code: ACCOUNT_FROZEN },
      );
    }
    try {
      // In a savepoint, so a unique violation (the same hash credited by a
      // concurrent request that has just committed) leaves the transaction
      // usable for the re-check below.
      await client.query('savepoint claim_tx');
      await client.query('insert into used_payment_tx (tx_hash) values ($1)', [txHash]);
      await client.query('release savepoint claim_tx');
    } catch (err) {
      if (err.code === '23505') {
        await client.query('rollback to savepoint claim_tx');
        const raced = await priorDepositCredit(client, userId, txHash);
...
/**
 * Read-only: has `txHash` already been credited to THIS account? The same
 * answer recordDepositCredit gives before crediting, for a caller that must
 * refuse BEFORE reaching it -- /api/credits/buy's early frozen-account check,
 * which used to tell a suspended fan whose credit had already committed that
 * their payment "has not been used" (round-8 money#1). Returns the
 * `alreadyCredited: true` result or null.
 */
export async function findPriorDepositCredit(userId, txHash) {
  if (typeof txHash !== 'string' || !txHash) return null;
  return priorDepositCredit({ query }, userId, txHash.toLowerCase());
}

// This account's existing deposit ledger row for `txHash`, as a success
// result with `alreadyCredited: true`, or null.
async function priorDepositCredit(client, userId, txHash) {
  const { rows } = await client.query(
    `select amount_cents, meta from credit_ledger
      where user_id = $1 and type = 'deposit' and lower(meta->>'txHash') = lower($2)
      order by id limit 1`,
    [String(userId), String(txHash)],
  );
  if (!rows.length) return null;
  const meta = rows[0].meta || {};
  const { rows: bal } = await client.query('select balance_cents from credit_balances where user_id = $1', [String(userId)]);
  return {
    alreadyCredited: true,
    creditedCents: Number(rows[0].amount_cents),
    feeCents: Number(meta.feeCents) || 0,
    grossCents: Number(meta.grossCents) || Number(rows[0].amount_cents),
    balanceCents: bal.length ? Number(bal[0].balance_cents) : 0,
  };
}
