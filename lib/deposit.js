...
export async function recordDepositCredit({ userId, txHash, grossCents, feeCents, netCents, allowFrozen = false }) {
  return withTransaction(async (client) => {
    ...
    if (isFrozenStanding(standing) && !allowFrozen) {
      throw Object.assign(
        new Error('Your account is suspended or banned, so its credits are frozen and this payment was not credited. The transaction is still unused -- contact team@onlyone1.fun with the transaction hash.'),
        { code: ACCOUNT_FROZEN },
      );
    }
    try {
