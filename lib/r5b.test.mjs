...
section('media#0: finalize error deletes only an unfinalized, unreferenced, unguarded upload');
{
  ...
  // Round 7 (R7B media#0): the 'token' row is KEPT with its clock restarted
  // (the upload token may still be live and could re-create the file), not
  // dropped; the tombstone is written.
  {
    const { rows: tok } = await query(`select reason, created_at > now() - interval '1 minute' as fresh from media_uploads where pathname = $1`, [fresh]);
    check('...and its token row is kept, re-armed', tok.length === 1 && tok[0].reason === 'token' && tok[0].fresh, JSON.stringify(tok));
  }
