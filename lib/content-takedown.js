 ...
 * refused as a bad id.
 *
 * DETERMINISTIC (round-21 media#1 / social#2): the same conversation always
 * seals to the same reference. The panel identifies an open thread by its id
 * ("Show older messages" merges a page only into the thread with the same id,
 * the conversation list dedupes and highlights by it), and a random IV gave
 * every lookup response a different "ref." string, so paging older REPLACED
 * the thread instead of prepending to it -- in exactly the deleted-participant
 * threads a TAKE IT DOWN request is about. The IV is HMAC-SHA256(ivKey, "iv:" +
 * id) truncated to 12 bytes, under a key with its own context string
 * (SIV-style): an IV is only ever reused with the SAME plaintext, so GCM's
 * nonce-reuse failure (two different messages under one nonce) cannot occur,
 * and the reference stays opaque and authenticated. openConversationRef is
 * unchanged.
 */
...
function conversationRefIvKey() {
  return crypto.createHmac('sha256', String(ageVerificationSecret())).update('oa:admin-conversation-ref-iv:v1').digest();
}
export function sealConversationRef(conversationId) {
  const iv = crypto.createHmac('sha256', conversationRefIvKey()).update(`iv:${String(conversationId)}`).digest().subarray(0, 12);
 ...
 */
//
// The cursor is SEALED (round-21 social#0): it carries the page's last row id,
// and a conversation id is its two participants' ids joined, so a plain
// base64url cursor handed the admin panel the raw id of a thread whose
// participant has been deleted -- the id the sealed `id` field exists to hide.
// AES-256-GCM under a key with its own context string (as messages-store's
// encodeConversationCursor), random IV (a cursor is never compared by value).
// A cursor that does not open (forged, altered, or minted under an older
// secret) reads as no cursor: the first page.
function lookupCursorKey() {
  return crypto.createHmac('sha256', String(ageVerificationSecret())).update('oa:admin-lookup-cursor:v1').digest();
}

function encodeLookupCursor(r) {
  if (!r?.cursor_at || r.id === undefined || r.id === null) return null;
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv('aes-256-gcm', lookupCursorKey(), iv);
  const body = Buffer.concat([cipher.update(JSON.stringify([r.cursor_at, String(r.id)]), 'utf8'), cipher.final()]);
  return Buffer.concat([iv, cipher.getAuthTag(), body]).toString('base64url');
}

function decodeLookupCursor(cursor) {
  if (typeof cursor !== 'string' || !cursor || cursor.length > 800) return null;
  try {
    const raw = Buffer.from(cursor, 'base64url');
    if (raw.length < 29) return null;
    const decipher = crypto.createDecipheriv('aes-256-gcm', lookupCursorKey(), raw.subarray(0, 12));
    decipher.setAuthTag(raw.subarray(12, 28));
