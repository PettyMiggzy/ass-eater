  ...
  if (target.type === 'listing') {
    ...
    if (listing) {
      ...
      if (quarantine && files.length) {
        ...
        // deletion re-checks preservation and skips these files. The same
        // commit takes the listing off sale (preserveMedia), so a failure in
        // markListingRemoved below cannot leave it selling files nobody is
        // ever served.
