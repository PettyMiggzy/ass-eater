...
//    or a fan deleting their account); pending orders and other buyers'
//    orders are untouched.
// ROUND 18 (DECIDED, money#1/#2, public-pages#0, legal-journeys#0): a
// tracking number is the carrier reference the SELLER entered, not the
// buyer's personal data. An erasure removes ONLY the shipping name and
// address; the carrier, tracking number and trackingHistory stay, and buyer
// and seller see the same order. The expectations below were rewritten to
// that decision (they used to assert the numbers were erased).
//
// Run with:
...
// Round 16 (legal-journeys#0): an order whose buyer has no account row is
// erased the moment it ships, so each buyer here is a real (minimal) account.
const mk = async (data) => (data.buyerId ? query(
  "insert into users (id, data) values ($1, jsonb_build_object('email', $1 || '@buyer.test', 'role', 'fan')) on conflict do nothing",
  [String(data.buyerId)],
) : null).then(async () => String((await query(
...
const keepsNumbers = (d, current, history = []) => d.trackingNumber === current
  && (Array.isArray(d.trackingHistory) ? d.trackingHistory.map((h) => h.trackingNumber) : []).join(',') === history.join(',');

// ---------------------------------------------------------------------------
section('erasing one shipped order clears its address and keeps the seller\'s tracking (round 18)');
{
  ...
  check('...the tracking number and history stay', keepsNumbers(after, 'THIRD3', ['FIRST1', 'SECOND2']), JSON.stringify(after));
  ...
  check('...no tracking erasure is stamped, and no seller-only copy exists', !('trackingErasedAt' in after) && !('sellerTrackingNumber' in after),
    JSON.stringify(after));

  const fixed = await quiet(() => orders.markOrderShipped(o, '7', { carrier: 'UPS', trackingNumber: 'AGAIN4' }));
  check('a correction after the erasure is the one number buyer and seller both see', fixed.trackingNumber === 'AGAIN4'
    && (await row(o)).trackingNumber === 'AGAIN4', JSON.stringify(fixed));
  check('...and the replaced number is kept in history like any correction', (await row(o)).trackingHistory.length === 3);
  const view = (await orders.getOrdersForCreator('7')).find((x) => String(x.id) === o);
  check('the creator view counts corrections as for any order (3 used -> 0 left)', view && view.trackingCorrectionsLeft === 0, JSON.stringify(view));
  ...
  check('the buyer sees the same number the seller does, and no address', buyerView && buyerView.trackingNumber === 'AGAIN4'
    && buyerView.shippingAddress === null && typeof buyerView.addressErasedAt === 'string', JSON.stringify(buyerView));
...
section('an order whose address was erased before has nothing left to erase (its tracking stays)');
{
  ...
  const res = await orders.eraseOrderShippingAddress(o);
  const after = await row(o);
  check('erased: false, tracking untouched', res.erased === false && keepsNumbers(after, 'OLD1', ['OLD0']), JSON.stringify(after));
...
section('neighbours: a pending order still refuses; a digital order has nothing to erase');
{
  ...
  check('...and nothing is stamped on it', !('trackingErasedAt' in (await row(d))) && !('addressErasedAt' in (await row(d))));
}

section('a buyer\'s account deletion clears every shipped order\'s address, not its tracking');
{
  ...
  check('only the shipped order that still had an address is erased', count === 1, String(count));
  const r1 = await row(s1);
  const r2 = await row(s2);
  check('...it loses its address and keeps its numbers', r1.shippingAddress === null && keepsNumbers(r1, 'B3B', ['B3A']), JSON.stringify(r1));
  check('...the other (address already gone) is untouched', r2.trackingNumber === 'B3C' && !('trackingErasedAt' in r2), JSON.stringify(r2));
  const rp = await row(pend);
  check('the pending order keeps its address', rp.shippingAddress !== null && !('addressErasedAt' in rp));
