...
section('legal-journeys#0: erasing one shipped order clears its tracking numbers too');
{
  ...
  // Round 16 (legal-journeys#2): refused with a NEUTRAL message -- the seller
  // is no longer told the buyer asked for erasure.
  check('a creator cannot write a number back afterwards', err && err.code === orders.TRACKING_EDIT_LIMIT && !/buyer|erase/i.test(err.message), err && err.message);
