...
section('legal-journeys#0: erasing one shipped order clears its tracking numbers too');
{
  ...
  // Round 17 (money#2, dashboard#0 -- decided policy): the ONLY place the
  // number survives is the seller's own copy of what they entered
  // (sellerTrackingNumber, shown only in the seller's view so it does not
  // change when the buyer erases). Every buyer / admin copy is gone.
  const { sellerTrackingNumber, ...buyerSide } = after;
  check('no stored text outside the seller copy still holds a number', !/FIRST1|SECOND2|THIRD3/.test(JSON.stringify(buyerSide)));
  check('...and the seller copy holds only the latest number the seller entered', sellerTrackingNumber === 'THIRD3'
    && !/FIRST1|SECOND2/.test(JSON.stringify(after)), String(sellerTrackingNumber));

  const fixed = await quiet(() => orders.markOrderShipped(o, '7', { carrier: 'UPS', trackingNumber: 'AGAIN4' }));
  // Round 17: the correction answers like any other (the seller is not told
  // the buyer asked for erasure) but lands only in the seller copy.
  check('a creator cannot write a number back into the buyer copy', fixed.trackingNumber === 'AGAIN4' && (await row(o)).trackingNumber === null,
    JSON.stringify(fixed));
  check('...and the replaced number was not kept in history', noNumbers(await row(o)));
