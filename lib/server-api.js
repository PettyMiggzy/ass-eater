 ...
 * the creator next bridges. Call it after applyContentViolation() or an
 * admin status change, with the creator record's id; the status sent is the
 * creator's EFFECTIVE status, re-read here, never a value the caller made up.
 *
 * Best effort by design: it never throws, and returns { ok, ... } so a
 * caller can log a failure. A failed push is not lost for good -- the API
 * also applies the status on the creator's next bridge exchange. Skipped
 * (ok: false, skipped: true) when BRIDGE_SECRET is not configured.
 */
export async function pushCreatorStatus(creatorId) {
  try {
    ...
    const res = await fetch(`${API_BASE}/auth/bridge/status`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ token: mintBridgeStatusToken(user.id, status) }),
      signal: AbortSignal.timeout(TIMEOUT_MS),
    });
    // Any cached access token for this user is stale now either way.
    for (const k of tokenCache.keys()) if (k.startsWith(`${user.id}|`)) tokenCache.delete(k);
    const data = await res.json().catch(() => null);
