...
import { findUserByCreatorId, findUserById, combinedCreatorPushStanding } from './users-store';
...
async function standingFor(user) {
  ...
  // One rule, shared with setUserModeration (lib/users-store.js): an
  // account suspension on an unapproved profile carries no end, so server/
  // never lifts it into 'active' (approved).
  const { status: creatorStatus, suspendedUntil: lapse } = combinedCreatorPushStanding(creator, user);
...
export async function pushCreatorStatus(creatorId, { client = null } = {}) {
  try {
    ...
    const { status, suspendedUntil } = combinedCreatorPushStanding(creator, user);
    await enqueueStandingPushes([{ uid: user.id, status, role: 'CREATOR', suspendedUntil }], runner);
