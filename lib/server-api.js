...
async function creatorStatusFor(user) {
  ...
  return combinedCreatorStanding(creator, user);
}
...
async function exchangeBridgeToken(user, creatorStatus) {
  ...
  const token = mintBridgeToken(user, creatorStatus, {
    fanStanding: user.role === 'creator' ? null : effectiveUserStatus(user),
    standingAt: Date.now(),
  });
...
async function accessTokenFor(user) {
  ...
  const standing = creatorStatus || `fan:${effectiveUserStatus(user)}`;
  const key = [user.id, Number(user.sessionVersion || 0), user.role, standing].join('|');
  const now = Date.now();
...
export async function pushCreatorStatus(creatorId, { client = null } = {}) {
  try {
    ...
    await enqueueStandingPushes(
      [{ uid: user.id, status, role: 'CREATOR', suspendedUntil: status === 'suspended' ? creator?.suspendedUntil || user.moderationUntil || null : null }],
      runner,
    );
