import { mintBridgeToken, mintBridgeStatusToken } from './bridge-token';
import { getCreatorById } from './creators-store';
import { effectiveCreatorStatus } from './creator-status';
import { findUserByCreatorId } from './users-store';
 ...
 * the creator next bridges -- which a banned creator never does, since a
 * banned site user cannot get a bridge token. Called after every change that
 * moves a creator's status: lib/ncii-reports-store.js's ladder (via
 * /api/admin/ncii-reports-resolve) and /api/admin/profile. The status sent is
 * the creator's EFFECTIVE status, re-read here, never a value the caller made
 * up. Deleting a creator deletes their site login too, so the delete routes
 * use pushUserStanding() with the uid they resolved before the delete.
 *
 * Best effort by design: it never throws, and returns { ok, ... } so a
 * caller can log a failure (use reportPushFailure). Call it AFTER the change
 * commits, never inside a transaction. Skipped (ok: false, skipped: true)
 * when BRIDGE_SECRET is not configured or the creator has no login.
 */
export async function pushCreatorStatus(creatorId) {
  try {
    if (!process.env.BRIDGE_SECRET) return { ok: false, skipped: true };
    const user = await findUserByCreatorId(creatorId);
    if (!user) return { ok: false, skipped: true };
    const creator = await getCreatorById(creatorId);
    const raw = creator ? effectiveCreatorStatus(creator) : 'banned';
    const status = raw == null || raw === '' ? 'active' : String(raw);
    return await pushUserStanding(user.id, status);
  } catch (err) {
...
/**
 * The same push for a site user id directly -- for a creator whose site
 * account no longer exists (deleted), who is pushed as 'banned' so their
 * server/ account stops too. Never throws.
 */
export async function pushUserStanding(uid, creatorStatus) {
  try {
    if (!process.env.BRIDGE_SECRET) return { ok: false, skipped: true };
    if (uid === null || uid === undefined || String(uid) === '') return { ok: false, skipped: true };
    const res = await fetch(`${API_BASE}/auth/bridge/status`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ token: mintBridgeStatusToken(String(uid), creatorStatus) }),
      signal: AbortSignal.timeout(TIMEOUT_MS),
    });
    // Any cached access token for this user is stale now either way.
...
/** Logs a push that should have happened and did not (skips are not failures). */
export function reportPushFailure(result, context) {
  if (result && !result.ok && !result.skipped) {
    console.error(`[server-api] standing push to server/ failed (${context}):`, result.status ?? result.error);
  }
  return result;
}
