...
import { DM_PRICE_FLOOR_CENTS, formatCredits } from './brand';
import { DM_PRICE_MAX_CENTS } from './field-validation';
import { userWriteRestriction } from './user-moderation';
import { sliceText, isWellFormedText, hasControlChars } from './unicode-text';
import { lockAuthorRow, AUTHOR_ACCOUNT_GONE } from './author-lock';
...
export async function sendDirectMessage({ sender, recipientId, text, clientMessageId = null, expectedPriceCents = undefined }) {
  ...
  // C0/C1 control characters too (round-13 social#0): JSON writes each as a
  // 6-byte escape, so a thread of them inflated the admin takedown lookup
  // past the platform's response limit.
  if (!isWellFormedText(trimmed) || hasControlChars(trimmed)) {
  ...
  // Lock order (round-19 media#1 / money#1): BOTH accounts' users rows first
  // (FOR SHARE, in id order), THEN the conversation. Account and creator
  // deletion take users FOR UPDATE and only then this account's
  // conversations (lib/users-store.js purgeUserContent); a paid send used to
  // lock the conversation first and reach the users rows only inside
  // transferWithFee -- the opposite order on the same rows, so a send landing
  // during a deletion deadlocked and, when Postgres picked the send, the fan
  // got a 500. Taking users first makes the send wait for the deletion (and
  // then refuse: the account is gone) instead of forming a cycle. The whole
  // transaction also retries once on a deadlock (40P01) as a backstop: it
  // starts only from database state, and the idempotency key makes a rerun
  // safe.
  return withTransactionRetryOnDeadlock(async (client) => {
    const { rows: people } = await client.query(
      'select id from users where id::text = any($1::text[]) order by id for share',
      [[fromId, toId]],
    );
    const present = new Set(people.map((r) => String(r.id)));
    if (!present.has(toId)) throw dmError(DM_ERRORS.RECIPIENT_NOT_FOUND, 'That account does not exist.');
    if (!present.has(fromId)) throw dmError(DM_ERRORS.NOT_ALLOWED, 'Not logged in');
    ...
    // The fan agreed to a price; charge that price or nothing. The message
    // names it as credits AND dollars (formatCredits; 1 credit = $1) --
    // it used to print the cent amount as "credits", 100x the real price.
    // Checked after
    ...
    if (priceCents > 0 && Number(expectedPriceCents) !== priceCents) {
      throw Object.assign(
        dmError(DM_ERRORS.PRICE_CHANGED, `This creator's message price is now ${formatCredits(priceCents)}. Confirm the new price to send.`),
...
export async function setConversationBlocked(userId, otherUserId, blocked) {
  ...
  return withTransaction((client) => setConversationBlockedTx(client, uid, other, blocked));
}

// One transaction (round-21 media#2): blocking takes BOTH accounts' users rows
// FOR KEY SHARE first (lib/author-lock.js; users before conversations, the
// account-deletion purge's own order, in id order), so a block racing either
// account's deletion either lands before the purge (and is purged with it) or
// waits and is refused -- a createdByBlock row naming a deleted account is
// never inserted after the purge ran. The blocker's own row gone is
// AUTHOR_ACCOUNT_GONE; the other account's is RECIPIENT_NOT_FOUND, as before.
async function setConversationBlockedTx(client, uid, other, blocked) {
  if (blocked) {
    for (const id of [uid, other].sort()) {
      try {
        await lockAuthorRow(client, id);
      } catch (err) {
        if (err.code === AUTHOR_ACCOUNT_GONE && id === other) throw dmError(DM_ERRORS.RECIPIENT_NOT_FOUND, 'That account does not exist.');
        throw err;
      }
    }
...
export async function setWallBlocked(ownerUserId, authorId, blocked) {
  ...
  if (blocked) {
    // Both accounts' users rows FOR KEY SHARE first, in id order (round-21
    // media#2, as setConversationBlocked): a wall_blocks row naming an account
    // whose deletion is in flight is never inserted after the purge's DELETE.
    return withTransaction(async (client) => {
      for (const id of [owner, author].sort()) {
        try {
          await lockAuthorRow(client, id);
        } catch (err) {
          if (err.code === AUTHOR_ACCOUNT_GONE && id === author) throw dmError(DM_ERRORS.RECIPIENT_NOT_FOUND, 'That account does not exist.');
          throw err;
        }
      }
      const { rowCount } = await client.query(
        `insert into wall_blocks (owner_user_id, author_id) values ($1, $2)
 ...
 */
export async function removeConversationMessage(conversationId, messageId, { beforeRemove, client: outer = null } = {}) {
  // With `client`, it runs on the caller's transaction (lib/content-takedown.js
  // writes its audit copy in the same commit as the removal -- round-14
  // media#0); otherwise in a transaction of its own.
