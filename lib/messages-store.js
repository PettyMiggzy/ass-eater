 ...
 *
 * Only a participant can block, and only an existing conversation (someone
 * who has never written to you has nothing to block). Throws
 * CONVERSATION_NOT_FOUND otherwise. Returns the projection for `userId`.
 */
export async function setConversationBlocked(userId, otherUserId, blocked) {
  ...
  const { rows } = await query(
    ...
    [pairId(uid, otherUserId), uid, !!blocked],
  );
