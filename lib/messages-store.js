  ...
  const { rows } = await query(
    `select id, data, updated_at,
      ...
      where data->'participantIds' ? $1
        ...
        -- stands -- the inbox is the one place a block can be lifted, and a
        -- wall commenter who never DMed (their comment since deleted) was
        -- otherwise blocked for good. Unblocked, it drops out again.
...
/** 'me' when `userId` blocked the other side, 'them' when blocked by them, else null. */
async function blockStateBetween(userId, otherUserId) {
  const { rows } = await query(`select data->'blockedBy' as b from conversations where id = $1`, [pairId(userId, otherUserId)]);
  const ids = Array.isArray(rows[0]?.b) ? rows[0].b.map(String) : [];
  if (ids.includes(String(otherUserId))) return 'them';
  if (ids.includes(String(userId))) return 'me';
  return null;
}
 ...
 *
 * A block does NOT need an existing conversation. It used to (someone who
 * had never written to you "had nothing to block"), but a block is also the
 * remedy on the wall, and most wall harassers have never DMed the creator --
 * messaging a creator costs money -- so they could not be blocked at all.
 * Blocking an account with no conversation creates the pair row (the same
 * insert-if-missing sendDirectMessage uses), marked `createdByBlock` so an
 * empty one never shows up as a thread in the BLOCKED account's inbox; the
 * blocker still sees it while the block stands, so they can lift it
...
export async function setConversationBlocked(userId, otherUserId, blocked) {
  ...
  if (blocked) {
    ...
    await query(
      `insert into conversations (id, data) values ($1, $2)
       on conflict (id) do nothing`,
      [id, { id, participantIds: [uid, other], messages: [], senders: [], createdByBlock: true }],
    );
  }
  const { rows } = await query(
    `update conversations
...
/**
 * A BLOCK-ONLY row, from the blocker's side: created by a block
 * (createdByBlock), nothing ever sent in it, and `viewerId` is the one who
 * blocked. These are the rows getConversationsForUser lists to the blocker so
 * the block can be lifted -- and the only way a wall commenter ever reaches
 * the creator's inbox. The wall never reveals who wrote a comment
 * (pages/api/wall/block.js, lib/wall-store.js toPublicWallPost), so such a
 * row must not either (round-9 social#0 / dashboard#1).
 */
...
/**
 * Lifts `userId`'s block on the block-only row `handle` names, resolved here,
 * server side, against that viewer's own block-only rows -- the counterpart's
 * id never travels. Throws CONVERSATION_NOT_FOUND for a handle that names none
 * of them. Returns { ok: true, blockHandle }.
 */
export async function unblockByHandle(userId, handle) {
  ...
  for (const row of rows) {
    const got = Buffer.from(blockHandleFor(uid, row.id));
 ...
 * null otherwise. A block lives on the DM conversation row, but it is the
 * remedy for an abusive account everywhere that account can reach the
 * blocker -- the wall (pages/api/wall/post.js) reads it too.
 */
