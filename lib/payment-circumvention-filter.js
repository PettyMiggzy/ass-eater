...
function detectIn(normalized, crossTag, add, reasons) {
  ...
  for (const { label, re, handoverOnly, adjacentAtOnly, handleLikeOnly, ordinary, directInstruction, notInstruction, instructionAfter, instructionBefore } of CONTACT_RAIL_KEYWORDS) {
    for (const m of normalized.matchAll(re)) {
      if (!isWordNotNumber(m[0])) continue;
      const start = m.index;
      const end = m.index + m[0].length;
      ...
      const instruction = !crossTag && (!handoverOnly
        ...
        : !!directInstruction && !notInstruction.test(normalized.slice(end, end + 30))
          && hasPaymentInstructionAt(normalized, start, end, { directOnly: true, afterRe: instructionAfter, beforeRe: instructionBefore ? RAIL_INSTRUCTION_BEFORE[instructionBefore] : undefined }));
