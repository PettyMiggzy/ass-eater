...
// Keycap digits ("1️⃣6️⃣": a digit, the emoji variation selector, then
// U+20E3 COMBINING ENCLOSING KEYCAP) are a number style emoji keyboards offer
// directly. NFKD leaves them as they are, so a keycapped phone number or age
// kept the combining marks between its digits and no digit rule could read
// it (round-20 accounts#8). Folded to the plain character first, for both
// filters (lib/prohibited-terms.js reads text through normalizedReadings).
...
// Curly and modifier apostrophes (iOS/macOS smart punctuation types U+2019
// by default) fold to ASCII, so "I’ll" reads as the "i'll" every pattern is
// written against rather than as the two words "i" and "ll" (round-15
// accounts#2). lib/prohibited-terms.js folds the same set for its ageText.
...
// The after-cue for the handover-only rails with `directInstruction` (see
// "tele" below): an explicit channel phrase only -- "tele for cheaper", "tg
// payments", "sc: $20" -- and, for the rails with no ordinary reading of a
// price after them ("tg", "tele"), "tele $20" too.
const RAIL_INSTRUCTION_AFTER = {
  channel: /^[^a-z0-9$.!?]{1,3}(?:(?:for|4)[^a-z0-9$.!?]{1,3}(?:cheaper|less)(?![a-z0-9])|(?:payments?|pay)(?![a-z0-9])|:\s?\$\d)/,
  'channel-or-price': /^[^a-z0-9$.!?]{1,3}(?:(?:for|4)[^a-z0-9$.!?]{1,3}(?:cheaper|less)(?![a-z0-9])|(?:payments?|pay)(?![a-z0-9])|:\s?\$\d|\$\d)/,
};
// ...and the before-cue for a rail that is also a place or an ordinary word
// ("sc", "signal"): joined by a routing connector only (built below, once the
// cue patterns exist).
const RAIL_INSTRUCTION_BEFORE = {};
// A regex that never matches, for switching an after-cue off per match.
const NEVER_MATCHES_RE = /(?!)/;
...
const CONTACT_RAIL_KEYWORDS = [
  ...
  // Round 20 (accounts#7): a payment instruction DIRECTLY beside them counts
  // too, as for "tele" below ("pay me on tg", "$20 on sc", "cheaper on my
  // tg", "pay 20 via signal"). "tg" has no ordinary reading to guard; "sc"
  // (South Carolina) and "signal" do, so for those two the cue before must
  // be joined by a routing connector (on / via / with / my...) -- never "in",
  // "at" or "to" ("gas is cheaper in SC", "$20 at the SC state fair") -- and
  // a place or signal-strength word after the rail is not an instruction
  // ("paid my SC taxes", "pay attention to the signal lights"). After the
  // rail only an explicit channel phrase counts ("sc for cheaper", "tg
  // payments", "sc: $20"); a bare "$20" after "SC" is a price in a place.
  { word: 'signal', label: 'signal', handoverOnly: true, adjacentAtOnly: true, directInstruction: true,
    notInstruction: /^[^a-z0-9]{1,3}(?:boost|boosts|boosting|strength|bars?|lost|dropped|drops|light|lights|booster|tower|towers|jammer|flares?|fire|word|words|processing|noise|loss|quality|issues?|problems?)(?![a-z])/,
    instructionBefore: 'narrow', instructionAfter: 'channel' },
  // Round 21 (accounts#2): "tg" DOES have an ordinary reading here -- on an
  // adult platform it is the everyday category word for trans / transgender
  // ("Only $20 with my TG girlfriend", "$20 on TG content this week only",
  // "TG $20 bundle"). A category noun after it (directly, or after a price)
  // is never an instruction, and the cue before it must be joined by a
  // routing connector (the narrow form, as "sc" / "signal"). "pay me on tg",
  // "$20 on tg", "tg for cheaper", "tg payments" and "tg: 69kitty" are still
  // refused.
  // Round 22 (accounts#9): the category-noun exemption is only ever for the
  // AFTER-cue reading and a weak price-before ("$20 on TG content"). A STRONG
  // routing instruction before it -- "pay me on", "send it via", "pay $20 on",
  // "cheaper on my", "payments via" -- is an instruction whatever noun
  // follows: "pay me on tg babe", "pay $20 on tg bundle" and "cheaper on my
  // tg content" were delivered. And a bare price after "TG" that ends there,
  // or is followed by each / today / only / menu / "for <category>", is a
  // category price ("TG $20", "Hot TG $20", "TG $15 each", "TG: $20"), not a
  // channel instruction.
  { word: 'tg', label: 'telegram', handoverOnly: true, adjacentAtOnly: true, directInstruction: true,
    notInstruction: /^[^a-z0-9$]{1,3}(?:\$?\d[0-9,.]*[^a-z0-9]{1,3})?(?:girls?|gfs?|girlfriends?|content|sets?|bundles?|models?|porn|porno|clips?|vids?|videos?|bestie|besties|friends?|ts|babes?|cock|dick|collabs?|pics?|photos?|women|woman|chicks?|queens?|community|creators?|girly|lovers?|fans?|femboys?|boys?|guys?|couples?|stars?|pornstars?|customs?|nudes?|sexting|dolls?|goddess)(?![a-z])/,
    strongBefore: /(?:^|[^a-z0-9])(?:(?:pay|send|sent|cashapp|venmo|zelle)[^a-z0-9]{1,3}(?:me|us|it|money|cash)(?:[^a-z0-9]{1,3}(?:directly|instead|only))?|pay(?:ing)?[^a-z0-9]{1,3}[$\u00a3\u20ac]?\d[0-9,.]*|payments?|pay|cheaper|less|tips?|deposits?)[^a-z0-9]{1,3}(?:on|via|thru|through|over|using|with)(?:[^a-z0-9]{1,3}(?:my|our|the))?[^a-z0-9]{1,3}$/,
    categoryPrice: /^[^a-z0-9$\u00a3\u20ac]{0,3}[$\u00a3\u20ac]\s?\d[0-9,.]*(?![0-9])(?:\s+(?:each|ea|today|only|menu|this week|this weekend|for\s+(?:customs?|content|sets?|bundles?|clips?|vids?|videos?|pics?|photos?|sexting|nudes?|collabs?)))*\s*(?:$|[^a-z0-9\s$])/,
    instructionBefore: 'narrow', instructionAfter: 'channel-or-price' },
  ...
  { word: 'sc', label: 'snapchat', handoverOnly: true, adjacentAtOnly: true, directInstruction: true,
    notInstruction: /^[^a-z0-9]{1,3}(?:tax|taxes|state|dmv|license|licence|plates?|residents?|resident|beach|beaches|coast|university|college|parking|rent|bills?|tolls?|lottery|sales|county|counties|premium|highway|hwy|route|zip)(?![a-z])/,
    instructionBefore: 'narrow', instructionAfter: 'channel' },
  { word: 'line', label: 'line', handoverOnly: true, adjacentAtOnly: true, handleLikeOnly: true },
  // "tele" is common seller shorthand for Telegram ("my tele is jess_99",
  ...
  { word: 'tele', label: 'telegram', handoverOnly: true, adjacentAtOnly: true, handleLikeOnly: true, directInstruction: true,
    ...
    instructionAfter: 'channel-or-price' },
  ...
  ...k,
  re: buildKeywordRe(k.word),
  instructionAfter: k.instructionAfter ? RAIL_INSTRUCTION_AFTER[k.instructionAfter] : undefined,
}));
...
// Round 20 (accounts#7): "pay 20 on tele" / "send 20 via tg" -- the
// imperative with a bare amount is one cue here too, as it already is in
// BRIDGE_CUE; INSTRUCTION_CUE's "[0-9,.]*" only covered digits glued to it.
const DIRECT_AMOUNT_CUE = '(?<![a-z0-9])(?:pay|send)\\s+\\$?\\d[0-9,.]*(?![a-z0-9])';
const CUE_BEFORE_RAIL_RE = new RegExp(`(?:${DIRECT_AMOUNT_CUE}|${INSTRUCTION_CUE}[0-9,.]*)(?:${INSTRUCTION_SEP}${CONNECTOR_BEFORE}){0,3}${INSTRUCTION_SEP}$`);
// The narrow form for "sc" / "signal": no "in", "at", "to", "over" or "the"
// between the cue and the rail -- those make it a place ("cheaper in SC",
// "$20 at the SC fair").
const CONNECTOR_BEFORE_ROUTING = '(?:me|us|you|u|on|via|with|through|thru|using|by|only|my|our|your|directly|instead)';
RAIL_INSTRUCTION_BEFORE.narrow = new RegExp(`(?:${DIRECT_AMOUNT_CUE}|${INSTRUCTION_CUE}[0-9,.]*)(?:${INSTRUCTION_SEP}${CONNECTOR_BEFORE_ROUTING}){0,3}${INSTRUCTION_SEP}$`);
...
// Round 16 (accounts#5): "pay 20" / "send 20" (no "$") is ONE cue, like
// "$20" -- as two words, "pay 20 and I'll add you on snap" ran out of bridge
// words one short of the app while the "$20" form was refused.
const BRIDGE_CUE = '(?:(?<![a-z0-9])(?:pay|send)\\s+\\$?\\d[0-9,.]*(?![a-z0-9])|\\$\\d[0-9,.]*|(?<![a-z0-9])(?:pay(?![a-z0-9])(?!\\s+attention(?![a-z0-9]))|payment|payments|cheaper(?![a-z0-9])(?!\\s+than(?![a-z0-9])))(?![a-z0-9]))';
const CLAUSE_SEP = '[^a-z0-9$.!?,;|\\n]{1,3}';
const BRIDGE_W = (alt) => `${CLAUSE_SEP}(?:${alt})(?![a-z0-9'])`;
// Round 14 (accounts#2): the round-13 lists were so short that the commonest
// handover words stopped the bridge and phrasings round 12 refused passed
// again ("$15 and u get my snap", "pay 20 and ill send on snap", "Tip $10 and
// I add you on snap", "cheaper deals but only on my telegram", "prices are
// cheaper so check my telegram"). The tail is widened -- the stop list is not
// -- so the round-13 false positives ("...on here and my insta has previews",
// "pay my rent then post on insta") still pass.
// Leads: pronouns, joiners, the future/modal leads ("i'll", "ill", "will",
// "can"), "only"/"just" ("but only on my telegram"), and the comparatives a
// price pitch strings together ("cheaper and faster on telegram").
const BRIDGE_TAIL_LEAD = "i'?ll|i|we|u|you|and|then|also|just|only|will|can|faster|easier|quicker|safer|better";
// Handover verbs name a contact or a delivery, so they count on their own
// ("and u get my snap", "and hmu on telegram").
const BRIDGE_TAIL_VERB = 'send|sends|sending|dm|text|message|msg|hit|hmu|add|added|adding|contact|reach|pay|tip|find'
  + '|get|gets|getting|snap|kik|telegram|whatsapp';
// Promotional verbs are also how ordinary cross-promotion reads ("Sets from $9
// and follow my insta for updates", "Unlock for $12 and check my instagram
// stories" -- round-14 fix-up), so after "and"/"then"/"but" they count only
// with a direct-contact object: "and join me on telegram", "and see u on snap".
const BRIDGE_PROMO_VERB = 'check|follow|see|join';
// ...and only "join"/"see" at that. "follow me on insta" / "check me out on
// ig" is the commonest cross-promotion phrase there is -- it means the same
// as "follow my insta for updates", which is ordinary promotion -- and letting
// "follow"/"check" bridge with a bare me/you refused "Customs from $20 and
// follow me on IG" as fee-dodging (round-15 accounts#1).
const BRIDGE_DIRECT_PROMO_VERB = 'see|join';
const BRIDGE_TAIL_CONN = 'me|us|up|on|via|at|in|over|thru|through|to';
const BRIDGE_TAIL_MORE = `(?:${BRIDGE_W(`${BRIDGE_TAIL_VERB}|${BRIDGE_PROMO_VERB}|${BRIDGE_TAIL_CONN}|you|u|my|our|the`)})*`;
...
// "you" or another verb: "and hit my snap", "and I add you on snap", "and get
// added on snap") or bare connectors, which may take "my" only after at least
// one real connector ("but only on my telegram") -- never straight after the
// joiner ("and my insta" is a new subject).
const BRIDGE_CONTACT_TAIL = `(?:${BRIDGE_W(BRIDGE_TAIL_LEAD)})*`
  + `(?:${BRIDGE_W(BRIDGE_TAIL_VERB)}${BRIDGE_TAIL_MORE}`
  + `|${BRIDGE_W(BRIDGE_DIRECT_PROMO_VERB)}${BRIDGE_W('me|us|you|u')}${BRIDGE_TAIL_MORE}`
  + `|(?:(?:${BRIDGE_W(BRIDGE_TAIL_CONN)})+(?:${BRIDGE_W('my|our|the')})?)?)`
  + `${CLAUSE_SEP}$`;
// "so" after a price is a consequence, not a new subject: there a promotional
// verb may take "my" ("prices are cheaper so check my telegram").
const BRIDGE_SO_TAIL = `(?:${BRIDGE_W(BRIDGE_TAIL_LEAD)})*${BRIDGE_W(BRIDGE_PROMO_VERB)}${BRIDGE_TAIL_MORE}${CLAUSE_SEP}$`;
// "then my snap" straight after a price ("$20 then my snap") is a sequence,
// not a new subject -- only "then", and only with the price cue directly
// before it: "and my insta has previews" and "cheaper bundles then my
// instagram gets the teasers" are new statements.
const BRIDGE_THEN_MY_TAIL = `${BRIDGE_W('my|our')}${CLAUSE_SEP}$`;
// Round 16 (accounts#4): after an IMPERATIVE "pay/send <amount>", "follow" /
// "check" + me/us + on/at/via is the direct handover again ("pay $20 then
// follow me on snap", "Pay $20 and check me on snap") -- the round-15
// narrowing above was for a price DESCRIPTION ("Customs from $20 and follow me
// on IG", "Tip $5 and follow me on instagram"), which still passes, as does
// "follow my insta" after any cue. Up to two filler words may sit between the
// amount and and/then ("pay 20 now and follow me on snap", "pay $20 first
// then follow me on snap").
const BRIDGE_PAY_FOLLOW_TAIL = `${BRIDGE_W('follow|check')}${BRIDGE_W('me|us')}${BRIDGE_W('on|at|via|in|over')}${CLAUSE_SEP}$`;
const BRIDGE_STOP = `(?:here|and|then|but|so|while)(?![a-z0-9'])(?!${BRIDGE_CONTACT_TAIL})`
  + `(?<!(?<![a-z0-9])so(?=${BRIDGE_SO_TAIL}))`
  + `(?<!${BRIDGE_CUE}${CLAUSE_SEP}then(?=${BRIDGE_THEN_MY_TAIL}))`
  + `(?<!(?<![a-z0-9])(?:pay|send)(?:${CLAUSE_SEP}(?:me|us))?${CLAUSE_SEP}\\$?\\d[0-9,.]*(?:${CLAUSE_SEP}(?:now|first|me|us)(?![a-z0-9'])){0,2}${CLAUSE_SEP}(?:and|then)(?=${BRIDGE_PAY_FOLLOW_TAIL}))`;
// "i will" / "we will" count as ONE of the five words, the same as "i'll"
// (round-16 accounts#5: "pay 20 and I will add you on snap").
const CUE_BRIDGE_BEFORE_RAIL_RE = new RegExp(`${BRIDGE_CUE}(?:${CLAUSE_SEP}(?!${BRIDGE_STOP})(?:(?:i|we|you|u)\\s+will(?![a-z0-9'])|[a-z0-9']{1,15})){1,5}${CLAUSE_SEP}$`);
...
// explicit channel phrase ("tele", "tg", "sc", "signal" -- see
// CONTACT_RAIL_KEYWORDS), and beforeRe CUE_BEFORE_RAIL_RE for one that is
// also a place ("sc").
function hasPaymentInstructionAt(text, start, end, { directOnly = false, afterRe = CUE_AFTER_RAIL_RE, beforeRe = CUE_BEFORE_RAIL_RE } = {}) {
  ...
  return (beforeRe || CUE_BEFORE_RAIL_RE).test(before)
...
// Round 20 (accounts#5): the unit is an explicit list. Round 19 excluded ANY
// digits followed by up to six letters, which is also how ordinary handles are
// written ("69kitty", "23jess", "420babe", "1jess"), so "tg: 69kitty" and
// "sc: 23jess" published. Only a number with a real unit on it is a number.
// Round 21 (accounts#3): clothing sizes and counts too -- "Lingerie line:
// 2xl-5xl", "Plus size line: 3xl", "swim line: 2piece sets", "Spring line:
// 3pack", "bottom line: 4ever yours", "signal: 2bars lol". A size range
// ("2xl-5xl") is one number token. "line: 88mia" and "line: jane_doe99" (name
// letters after the digits) are still handles.
const NUMBER_UNIT = '(?:mm|cm|m|km|pcs|pc|px|k|x|gb|mb|tb|kb|fps|th|st|nd|rd|s|am|pm|min|mins|sec|secs|h|hr|hrs'
  + '|in|ft|lb|lbs|kg|g|ml|l|oz|mp|hz|p|d|w|mo|yr|yrs|pk'
  + '|xs|xxs|xl|xxl|xxxl|xxxxl|pack|packs|piece|pieces|bar|bars|way|ways|star|stars|ever)';
const NOT_A_NUMBER_TOKEN = `(?![0-9]+${NUMBER_UNIT}?(?:-[0-9]+${NUMBER_UNIT}?)?(?![a-z0-9_.]))(?=[a-z0-9_.]*[a-z])`;
...
// Round 20 (accounts#6): or a pointer between them ("tg 👉 @jessrose", "sc ->
// @jessxo", "tele ➡️ @jess"): " 👉 " and " -> " are four UTF-16 units, past
// the three the plain separator allows, while "tg 👉 jess_rose" (a
// handle-looking token) was already refused. An explicit "@" is always a
// handle.
const ADJACENT_AT_RE = new RegExp(
  `^[^a-z0-9@]{0,3}(?:${POINTER}{1,3}[^a-z0-9@]{0,3})?(?:(?:me|is|at|im|i'?m|on)[^a-z0-9@]{1,3}){0,2}@[a-z0-9_.]{3,30}`
  // after a pointer, "@ jessxo" too
  + `|^[^a-z0-9@]{0,3}${POINTER}{1,3}\\s{0,3}@\\s{1,2}[a-z0-9_.]{3,30}`,
  'u',
);
...
function detectIn(normalized, crossTag, add, reasons) {
  ...
  for (const { label, re, handoverOnly, adjacentAtOnly, handleLikeOnly, ordinary, directInstruction, notInstruction, strongBefore, categoryPrice, instructionAfter, instructionBefore } of CONTACT_RAIL_KEYWORDS) {
    for (const m of normalized.matchAll(re)) {
      if (!isWordNotNumber(m[0])) continue;
      const start = m.index;
      const end = m.index + m[0].length;
      ...
      const afterText = normalized.slice(end, end + 30);
      // A strong routing instruction before the rail outranks the ordinary
      // noun after it (tg only; see its entry).
      const strongHere = !!strongBefore && strongBefore.test(normalized.slice(Math.max(0, start - 40), start));
      const instruction = !crossTag && (!handoverOnly
        ? hasPaymentInstructionAt(normalized, start, end)
        : !!directInstruction && (strongHere || (!notInstruction.test(afterText)
          && hasPaymentInstructionAt(normalized, start, end, {
            directOnly: true,
            // A category price after the rail is not the channel-or-price cue.
            afterRe: categoryPrice && categoryPrice.test(afterText) ? NEVER_MATCHES_RE : instructionAfter,
            beforeRe: instructionBefore ? RAIL_INSTRUCTION_BEFORE[instructionBefore] : undefined,
          }))));
