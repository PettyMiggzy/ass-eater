...
/**
 * Small capitals and dotless letters folded to plain Latin, and nothing else.
 * Exported for lib/creator-status.js sanitizeTags, so a tag typed in small
 * capitals is STORED as the plain word the screen judged it as.
 */
export function foldLookalikeLetters(text) {
  return String(text ?? '').replace(LETTER_FOLD_RE, (ch) => LETTER_FOLDS[ch]);
}
...
function detectIn(normalized, crossTag, add, reasons) {
  ...
  for (const { label, re, handoverOnly } of CONTACT_RAIL_KEYWORDS) {
    for (const m of normalized.matchAll(re)) {
      ...
      const start = m.index;
      const end = m.index + m[0].length;
      if (isHandover(normalized, start, end) || (!handoverOnly && !crossTag && hasStrongPaymentCueNear(normalized, start, end))) {
