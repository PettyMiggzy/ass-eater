...
const DIRECT_HANDOVER_RE = new RegExp(
  `^(?:${DIRECT_NOUN}\\s{0,2}(?:[:=@]\\s{0,3}(?:${HANDLE_LIKE}|${PLAIN_WORD_HANDLE})|-\\s{0,3}${HANDLE_LIKE})|\\s{1,2}${SPACED_HANDLE})`,
);
...
function isHandover(text, start, end, { adjacentAtOnly = false, handleLikeOnly = false } = {}) {
  ...
  return POSSESSIVE_BEFORE_RE.test(before) && (handleLikeOnly ? HANDOVER_AFTER_STRICT_RE : HANDOVER_AFTER_RE).test(after);
}
...
function detectIn(normalized, crossTag, add, reasons) {
  ...
  for (const { label, re, handoverOnly, adjacentAtOnly, handleLikeOnly, ordinary } of CONTACT_RAIL_KEYWORDS) {
    for (const m of normalized.matchAll(re)) {
      ...
      if (isHandover(normalized, start, end, { adjacentAtOnly: !!adjacentAtOnly || ordinaryHere, handleLikeOnly: !!handleLikeOnly }) || (!handoverOnly && !crossTag && hasStrongPaymentCueNear(normalized, start, end))) {
