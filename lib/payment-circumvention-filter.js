...
//      every listing priced "1M $ONLYASS", "new set drops at midnight. Link in
...
//
// "onlyass" stays in this list alongside "onlyone": the token was renamed on
// 2026-09-18, and old listings, old DMs and anyone still using the old name
// must not start getting flagged as payment circumvention because of a
// rename. Removing it would turn history into violations.
const CASHTAG_RE = /(?<![a-z0-9$])\$([a-z][a-z0-9_]{1,19})(?![a-z0-9_])/g;
const EXEMPT_TICKERS = new Set(['onlyass', 'onlyone']);
...
// (order ids, epoch timestamps, $ONLYASS amounts) and a 3-3-4 grouping split
...
const PLATFORM_EMAIL_DOMAINS = ['onlyass.fun', 'onlyass.xyz', 'onlyass.online', 'onlyone1.fun'];
