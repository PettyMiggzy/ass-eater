...
const OFF_PLATFORM_LINK_RE = /(?<![a-z0-9])(?:beacons\.ai|bio\.link|lnk\.bio|throne\.com|linktr\.ee|allmylinks\.com|fanvue\.com|manyvids\.com)(?![a-z0-9])/;
...
const CONTACT_RAIL_KEYWORDS = [
  ...
  { word: 'signal', label: 'signal', handoverOnly: true },
  { word: 'tg', label: 'telegram', handoverOnly: true },
].map((k) => ({ ...k, re: buildKeywordRe(k.word) }));
...
const HANDOVER_AFTER_RE = new RegExp(
  '^[^a-z0-9]{0,3}(?:'
  // "my snap: jdoe", "my kik = jdoe", "my snap tag: jdoe"
  + `(?:[a-z]{1,10}[^a-z0-9]{1,3}){0,2}[:=][^a-z0-9]{0,3}[a-z0-9_.]{3,30}`
...
const DIRECT_HANDOVER_RE = new RegExp(
  `^(?:\\s{0,2}(?:[:=@]\\s{0,3}(?:${HANDLE_LIKE}|${PLAIN_WORD_HANDLE})|-\\s{0,3}${HANDLE_LIKE})|\\s{1,2}${SPACED_HANDLE})`,
);
...
const CONTACT_LINK_RE = /(?<![a-z0-9])(?:t\.me|telegram\.me|wa\.me|api\.whatsapp\.com\/send|snapchat\.com\/add|kik\.me)\/[a-z0-9_+?=]/;
...
const PHONE_CUE_RE = /(?<![a-z0-9])(?:call|text|txt|whatsapp|whats ?app|sms|imessage|hmu|phone|cell|cellphone|mobile|signal|facetime|skype|telegram|tel|ph|callme|textme|txtme|my (?:number|digits|line))(?![a-z])/;
...
export function detectPaymentCircumvention(text) {
  ...
  for (const { label, re, handoverOnly } of CONTACT_RAIL_KEYWORDS) {
    for (const m of normalized.matchAll(re)) {
      if (!isWordNotNumber(m[0])) continue;
      const start = m.index;
      const end = m.index + m[0].length;
      if (isHandover(normalized, start, end) || (!handoverOnly && hasStrongPaymentCueNear(normalized, start, end))) {
  ...
  if (PHONE_FORMATTED_RE.test(normalized) || PHONE_INTERNATIONAL_RE.test(normalized)) {
    ...
    for (const re of [PHONE_AMBIGUOUS_RE, PHONE_SPACED_DIGITS_RE]) {
      for (const m of normalized.matchAll(re)) {
