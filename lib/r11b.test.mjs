...
section('gates-token#3: a host over its budget writes no account rows');
{
  ...
  const a = await loginGuard.readLoginCounters([loginGuard.loginGuardKeys({ identifier: 'counted@r11b.test', ip: 'x' }).accountKey]);
  check('a host under its budget still counts against the account', a[loginGuard.loginGuardKeys({ identifier: 'counted@r11b.test', ip: 'x' }).accountKey] === 1, JSON.stringify(a));
  ...
  const e = await loginGuard.readLoginCounters([loginGuard.loginGuardKeys({ identifier: 'exists@r11b.test', ip: 'x' }).accountKey]);
  check('...and a refused host never touches an existing account\'s counter', e[loginGuard.loginGuardKeys({ identifier: 'exists@r11b.test', ip: 'x' }).accountKey] === 0, JSON.stringify(e));
}
