...
// --- round-2 accounts#1: the plain-space handover and the missing apps -------
mustBlock('snapchat jane99', 'service, space, handle');
mustBlock('snap jane99', 'short service name, space, handle');
mustBlock('Telegram jane_doe for customs', 'service, space, underscore handle');
mustBlock('kik jane99', 'kik, space, handle');
mustBlock('hmu on signal jane99', 'invitation plus signal');
mustBlock('signal: jane.99', 'signal, colon, handle');
mustBlock('skype: jane.doe', 'skype, colon, handle');
mustBlock('signal me 6175551234', 'signal as a phone cue');
mustBlock('facetime me 6175551234', 'facetime as a phone cue');
mustBlock('reach me 6175551234', '"reach me" as a phone cue');
mustBlock('dm me 6175551234', '"dm me" as a phone cue');
mustBlock('customs + GFE on Telegram jane_doe99 (cheaper)', 'the bio from the audit');
mustPass('signal strength is bad here', 'signal as an ordinary word');
mustPass('the signal dropped, paid for it anyway', 'signal next to a payment word is not a handover');
mustPass('snap a pic for me', 'snap as a verb');
mustPass('snap 100pics in one shoot', 'a number with a unit is not a handle');
mustPass('telegram 2fa is annoying', 'a number with a unit is not a handle');
mustPass('it was paid yesterday', '"was" is not whatsapp');
mustPass('instagram vibes all week', 'a plain word after the service name');
mustBlock('hit me up at 6175551234', '"hit me up at" directly before a number');
mustPass('dm me about order 1234567890', 'an invitation cue next to an order id is not a phone');
mustPass('reach me here, order #1234567890 shipped', 'an invitation cue then an order number');
mustPass('hit me with your best offer, bundle id 55512345678', 'an invitation cue far from a bare digit run');

// Round-4 accounts#1: one glued separator (or a glued cue word) hid a number.
for (const t of ['text me -617-555-1234', 'text-617-555-1234', 'tel.617.555.1234', 'ph.617-555-1234',
  'Call/text:-617-555-1234', '..617-555-1234', 'text6175551234', 'textme.6175551234', 'call6175551234']) {
  mustBlock(t, 'a separator or cue glued to the number must not hide it');
}
mustPass('order 1234-5678 shipped 2026-09-24', 'a date and an order id are not a phone');
mustPass('priced at $19.99, shipping 5.00', 'prices are not a phone');
mustPass('set #12 of 40, 1080p', 'numbers in a caption are not a phone');

// Round-4 accounts#2: competing stores, link pages and TLD-less emails.
for (const t of ['manyvids', 'fancentro', 'loyalfans', 'justforfans', 'fanvue', 'chaturbate', 'stripchat',
  'clips4sale', 'iwantclips', 'myfreecams', 'allmylinks.com/jane', 'beacons.ai/jane', 'throne.com/jane',
  'bio.link/jane', 'lnk.bio/jane', 'https://www.manyvids.com/Profile/123/jane/Store/Videos',
  'full vids on fanvue / clips4sale', 'email janedoe@gmail', 'hmu janedoe@protonmail', 'janedoe(at)gmail',
  'janedoe at gmail', 'jd99 at gmail', 'allmylinks', 'allmylinks.com']) {
  mustBlock(t, 'off-platform store, link page or email');
}
for (const t of ['thank you to my loyal fans', 'many vids coming this week', 'just for fans who stayed',
  'strip chat later?', 'I want clips of the beach', 'beacons of light', 'bio link below', 'throne room set',
  'email me at gmail', 'my gmail is full', 'check all my links', 'all my links are in the menu',
  'she is at gmail right now', 'we at yahoo', 'my friend works at google, I was at yahoo']) {
  mustPass(t, 'ordinary phrase, not an off-platform name');
}
