 ...
 */
export function detectProhibitedTerms(text, { nameLike = false } = {}) {
  const reasons = [];
  ...
  if (!nameLike) {
    for (const reading of normalizedReadings(text)) detectIn(foldStretched(reading), add);
  } else {
...
// With the same digit swaps as the term ("h0tteen").
const NAME_DESCRIPTOR = `(?:${['hot', 'sexy', 'cute', 'girl', 'boy', 'babe', 'model', 'petite', 'tiny', 'young', 'little', 'baby']
  .map((w) => compoundBody(w) + (['girl', 'boy', 'babe', 'model'].includes(w) ? '(?:s)?' : '')).join('|')})`;
const NAME_DESCRIPTOR_RES = [
  ...
  const startOfWord = t.term === 'incest' ? `|(?<![a-z])${body}` : '';
  return { ...t, re: new RegExp(`${NAME_DESCRIPTOR}${body}|${body}${NAME_DESCRIPTOR}${startOfWord}`) };
});

function detectIn(normalized, add, { compound = true } = {}) {
  ...
  if (!compound) {
    for (const { term, category, re } of NAME_COMPOUND_RES) if (re.test(normalized)) add(term, category);
    for (const { term, category, re } of NAME_DESCRIPTOR_RES) if (re.test(masked)) add(term, category);
  }
...
export function screenPublicText(text, { context = null } = {}) {
  ...
  // A handle or a fan's username is judged word by word (splitNameWords).
  const prohibited = detectProhibitedTerms(text, { nameLike: NAME_LIKE_CONTEXTS.has(context) });
