...
//   - Only ages 10-17. "my 2 year old cat", "3 years old page anniversary" and
//     "I'm 1 year old on here" are not about a minor performer.
//   - The short forms ("15yo", "16 y/o", "17 y.o.") are unambiguous and match
//     on their own. The long forms ("15 years old", "17-year-old") match only
//     as a self-description ("i'm 16 years old", "aged 15 years old") or in
//     front of a person noun ("17-year-old girl") -- "my dog is 12 years old"
//     is fine.
const MINOR_AGE_NUM = '(?<![0-9])1[0-7]';
const MINOR_SHORT_AGE = '[\\s-]{0,2}(?:y\\s?\\/\\s?o|y\\.\\s?o\\b\\.?|yo)(?![a-z])';
const MINOR_ANY_AGE = '[\\s-]{0,2}(?:y\\s?\\/\\s?o|y\\.\\s?o\\b\\.?|yo|yrs?[\\s-]{0,2}old|years?[\\s-]{0,2}old)(?![a-z])';
const MINOR_AGE_RES = [
  new RegExp(MINOR_AGE_NUM + MINOR_SHORT_AGE),
  // Self-description: "i'm/im/i am/she's/he's/aged/age/(just) turned", with
  // up to two words in between ("i'm only 16 years old").
  new RegExp("(?<![a-z])(?:i'?m|i am|she'?s|she is|he'?s|he is|aged?|just turned|turned)(?:\\s+[a-z]+){0,2}[\\s:,-]{1,3}" + MINOR_AGE_NUM.replace('(?<![0-9])', '') + MINOR_ANY_AGE),
