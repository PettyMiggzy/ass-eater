...
const PROHIBITED_TERMS = [
  ...
  // `familyExempt` (round-22 accounts#7): in FREE text only, skipped directly
  // after a possessive or "mom/mommy of a" when the WHOLE text has no sexual
  // or commerce word ("My little girl just turned 5!", "Sorry late reply, my
  // little girl was sick") -- the family noun round 21 already treats it as.
  // "daddy's little girl", "your little girl", any sexual or commerce word
  // anywhere, a tag and a name are all still refused. See isFamilyLittleOne.
  { term: 'little girl', familyExempt: true, category: 'minor-suggestive' },
  { term: 'little boy', familyExempt: true, category: 'minor-suggestive' },
  // Round 22 (accounts#6): the "lil" spelling, handled exactly the same way.
  { term: 'lil girl', familyExempt: true, category: 'minor-suggestive' },
  { term: 'lil boy', familyExempt: true, category: 'minor-suggestive' },
  // ...and the clinical word, which has no reading on this platform but one.
  { term: 'prepubescent', compound: true, category: 'minor-suggestive' },
  { term: 'pre pubescent', category: 'minor-suggestive' },
  // The acronym, and its long form (neither is a "child" + content-word pair,
  // so the pair groups never read them). `disclaimable: 'free'` like "child
  // porn": "We report CSAM to NCMEC" passes; "csam for sale", "selling csam",
  // "csam links" have no disclaimer; a commerce word right after one is never
  // disclaimed (CHILD_COMMERCE_AFTER_RE). Compound: "csam4sale".
  // `disclaimable: 'advocacy'`: 'free' plus an advocacy lead ("Fighting
  // CSAM", "Stop child sexual abuse material") -- these words name the crime,
  // and the people who write them in a bio are mostly fighting it.
  { term: 'csam', compound: true, disclaimable: 'advocacy', category: 'minor-suggestive' },
  { term: 'child sexual abuse material', disclaimable: 'advocacy', category: 'minor-suggestive' },
...
// sex ed teacher"). This always-on list is the ORIGINAL one: it applies in
// every context, tags and names included.
// Round 22 (accounts#2): round 21 widened it with the advocacy nouns
// (slavery, slaves, tourism, trade, rings, work, workers, exploit...), which
// made "child sex slaves for sale", "kid sex slave videos" and the tag "child
// sex slaves" pass everywhere. Those nouns are SEX_ADVOCACY_NOUNS now, and a
// pair ending on one is skipped only in FREE text, only behind an advocacy
// lead ("fighting child sex slavery", "Stop child sex tourism") and never
// with a commerce word after it -- see isAdvocacyPair.
const SEX_EDUCATION_AFTER = '(?![^a-z0-9]{0,3}(?:ed|education|educator|abuse|abused|abuser|abusers|trafficking|traffickers?|trafficked|offenders?|crimes?|predators?|assault|exploitation|survivors?|abuse material)(?![a-z]))';
const SEX_ADVOCACY_NOUN_RE = /^[^a-z0-9]{0,3}(?:slavery|slaves?|tourism|tourists?|rings?|trade|trades|trading|work|workers?|exploit\w*)(?![a-z])/;
// An advocacy or disclaimer lead directly in front of the child word.
const ADVOCACY_LEAD_BEFORE_RE = /(?:^|[^a-z])(?:fight(?:s|ing)?(?:\s+against)?|stop(?:ping)?|end(?:ing)?|against|anti|survivors?\s+of|victims?\s+of|report(?:ing)?|combat(?:ing)?|prevent(?:ing)?|eradicat(?:e|ing)|abolish(?:ing)?|awareness\s+of|campaign(?:ing)?\s+against|no|zero\s+tolerance\s+for)(?:\s{1,3}(?:all|any|the|of))?\s{1,3}$/;
// A commerce word soon after a child pair: never a disclaimer, never advocacy
// ("child sex slaves for sale", "csam links", "We report csam for sale").
const CHILD_COMMERCE_AFTER_RE = /^[^.!?;\n]{0,24}?(?<![a-z])(?:for\s+sale|for\s+hire|4\s?sale|available|links|vids?|videos?|pics?|photos?|clips?|dm(?:\s+me)?|inbox|selling|sell|buy|trade\s+(?:with|for)|cheap)(?![a-z])|^[^.!?;\n]{0,24}?[$\u00a3\u20ac]\s?[0-9]/;
// ...the narrower form for overriding a disclaimer: a sale, a link or a
// price right after the term ("Zero tolerance for child porn, dm for customs"
// is still a disclaimer; "No csam for sale here" is not).
const CHILD_COMMERCE_STRONG_AFTER_RE = /^[^a-z0-9.!?;\n]{0,3}(?:(?:[a-z]+[^a-z0-9.!?;\n]{1,3}){0,2})(?:for\s+sale|for\s+hire|4\s?sale|links?|vids?|videos?|available|selling|[$\u00a3\u20ac]\s?[0-9])(?![a-z])/;
function isAdvocacyPair(text, start, end) {
  const after = text.slice(end, end + 60);
  const noun = after.match(SEX_ADVOCACY_NOUN_RE);
  if (!noun) return false;
  if (!ADVOCACY_LEAD_BEFORE_RE.test(text.slice(Math.max(0, start - 40), start))) return false;
  return !CHILD_COMMERCE_AFTER_RE.test(after.slice(noun[0].length));
}
...
// Round 22 (accounts#6): and the younger schools -- "elementary school porn",
// "kindergarten porn", "grade schooler nudes", "primary school girl".
const SCHOOL_PAIR_LEADS = ['high school', 'middle school', 'junior high', 'jr high', 'high schooler', 'high schoolers',
  'middle schooler', 'middle schoolers', 'elementary school', 'grade school', 'grade schooler', 'grade schoolers',
...
// Round 22 (accounts#6): and "kiddo(s)" ("kiddo porn", "kiddos nudes").
const CHILD_LEADS = ['child', 'children', 'childs', 'kid', 'kids', 'kiddie', 'kiddy', 'kiddies', 'kiddo', 'kiddos', 'toddler', 'toddlers',
  'infant', 'infants', 'schoolkid', 'schoolkids'];
...
const CHILD_XX_LEADS = new Set(['child', 'children', 'childs', 'kiddie', 'kiddy', 'kiddies', 'kiddo', 'kiddos', 'toddler', 'toddlers', 'infant', 'infants']);
...
PROHIBITED_TERMS.push(
  ...
  // Round 22 (accounts#6): the REVERSE order -- a content word, then the child
  // word: "nude kids", "naked children", "nude children for sale", "naked kids
  // pics", "porn kids", the handle "nakedkids". Same join, same free-text
  // disclaimer ("No nude kids ever"), glued forms name-safe.
  ...['nude', 'nudes', 'naked', 'topless', 'porn'].map((lead) => ({
    term: lead, words: ['kid', 'kids', 'child', 'children', 'toddler', 'toddlers', 'minor', 'minors', 'infant', 'infants', 'kiddie', 'kiddies', 'kiddo', 'kiddos'],
    gap: CHILD_PAIR_GAP, suffixes: '', compound: true, nameSafeCompound: true, disclaimable: 'free', category: 'minor-suggestive',
  })),
);
...
// across " - " ("Graduated high school - girls trip to Cancun!"), but a
// separator must not be a way round the rule either: a school lead, ONE label
// separator, a person word, then a sexual word ("high school - girl nudes",
// "middle school -- boys naked") is refused in free text.
// Round 22 (accounts#3): any label separator on ONE line (also between the
// person word and the sexual word, comma excepted: "school - girls - nude") -- "-" "–" "—" ":"
// ";" "|" "•" "·" "/" "," -- not just a spaced dash, and a bare "school" as
// well as the named schools: "Highschool: girl nudes $20", "High School •
// Girl Nudes", "school, girl nudes", "school | girl porn" were published. The
// reminiscing lines have no sexual word after the person word ("In high
// school, boys never noticed me") and keep passing.
const SCHOOL_DASH_SEXUAL_RE = new RegExp(`(?<![a-z])(?:${SCHOOL_PAIR_LEADS.map((l) => l.replace(/ /g, '[\\s_-]{0,2}')).join('|')}|school)`
  + '[ \\t]{0,2}[-\u2013\u2014:;|\u2022\u00b7/,]{1,2}[ \\t]{0,2}(?:girls?|boys?|gfs?|bfs?|babes?|sis|daughters?|cheerleaders?|teens?|twinks?)'
  + `(?:[ \\t]{0,2}[-\u2013\u2014:;|\u2022\u00b7/]{1,2}[ \\t]{0,2}|${CHILD_PAIR_GAP})(?:(?:hot|sexy|tight|wet|little|naughty|horny)[ \\t]{1,2})?(?:nudes?|naked|porn\\w*|sex|sexy|slut\\w*|whores?|pussy|pussies|xx+|horny|cum\\w*|fuck\\w*|tits|boobs|dick|cock|onlyfans)(?![a-z])`);
// Round 22 (accounts#6): "sex / fuck / porn / nudes WITH (a) kid / child /
// minor / toddler" and "fuck(ing) a / the kid / child / minor". FREE text
// honours the anti-CSAM disclaimer ("No sex with minors ever") and a few
// ordinary continuations ("Sex with kids in the house is impossible lol"); a
// bare expletive has no article ("my fucking kids woke me up") and does not
// match.
const CHILD_WORD_WITH = '(?:kids?|kiddos?|child|children|minors?|toddlers?|infants?)';
const CHILD_WITH_RE = new RegExp(`(?<![a-z])(?:sex|fuck(?:ing|ed)?|porn|nudes?|naked)[ \\t]{1,2}with[ \\t]{1,2}(?:(?:a|an|the|some|little|young)[ \\t]{1,2})?${CHILD_WORD_WITH}(?![a-z])`
  + '(?![^a-z0-9]{1,3}(?:in the (?:house|next room|room|home|car)|around|at home|home|asleep|awake|nearby|running around|in bed|next door))'
  + `|(?<![a-z])fuck(?:ing|ed|s)?[ \\t]{1,2}(?:a|the|some)[ \\t]{1,2}(?:little[ \\t]{1,2})?(?:kid|child|toddler|minor(?![^a-z0-9]{1,2}(?:details?|things?|issues?|stuff|changes?|edits?|league|key|chords?|problems?|inconvenience|mistakes?)(?![a-z])))(?![a-z])`, 'g');
// Round 22: "cp" is also a checkpoint, a caption, a cheese pizza and cerebral
// palsy, so only beside a sale or trade word: "cp for sale", "selling cp",
// "cp links", "buy cp", "trading cp" -- also dotted or spaced ("selling
// c.p", "c p links"). Every context.
const CP_COMMERCE_RE = /(?<![a-z0-9])c[._\s]?p[\s_.-]{0,2}(?:for[\s_.-]{0,2}sale|4[\s_.-]{0,2}sale|links?|vids?|videos?|pics?|collection|trade|trading|available|dealer|seller)(?![a-z])|(?<![a-z])(?:sell\w*|buy\w*|trad(?:e|es|ing)|want|wanting|looking[\s_.-]{1,2}for|need)[\s_.-]{1,3}c[._\s]?p(?![a-z0-9])/;
...
// Round 22 (accounts#2/#6): a child term followed by a commerce word ("csam
// for sale", "child sex slaves for sale") is never disclaimed.
function isChildDisclaimedNoSale(text, start, end) {
  return !CHILD_COMMERCE_STRONG_AFTER_RE.test(text.slice(end, end + 40)) && isChildDisclaimed(text, start, end);
}
function isAdvocacyDisclaimed(text, start, end) {
  if (CHILD_COMMERCE_STRONG_AFTER_RE.test(text.slice(end, end + 40))) return false;
  return isChildDisclaimed(text, start, end) || ADVOCACY_LEAD_BEFORE_RE.test(text.slice(Math.max(0, start - 40), start));
}
function disclaimTest(t, strictAge) {
  if (t.disclaimable === 'free') return strictAge ? null : isChildDisclaimedNoSale;
  if (t.disclaimable === 'advocacy') return strictAge ? null : isAdvocacyDisclaimed;
...
// Round 22 (accounts#5): ages UNDER TEN glued the same way -- "9andhorny",
// "jess9andhorny", "8andnaked", "9nhorny", "sevenandhorny", "9andwet". In a
// label a letter may come before the digit (a name: "jess9andhorny"); in free
// text it may not ("size2andsexy" is a size -- and is neutralized anyway). A
// spelled one..nine never after a letter ("canineandhorny", "someoneandnaked").
const GLUED_AND_SINGLE_TAIL = `(?:and|n)(?:so|very|super)?(?:${SEXUAL_ADJ}|${SEXUAL_NOUN}|girls?|boys?|virgin|a(?:virgin|slut|whore|hoe|bitch))(?![a-z])`;
// A spelled age may lose its "n" joiner to the stretched-letter fold
// ("sevennnaughty" reads "sevennaughty"), so before a word that starts with
// "n" the joiner is optional ("onesexymama" stays a name).
const GLUED_SPELLED_SINGLE = `(?<![a-z])(?:one|two|three|four|five|six|seven|eight|nine)(?:${GLUED_AND_SINGLE_TAIL}|(?:naughty|naked|nude|nudes|nympho)(?![a-z]))`;
const GLUED_AND_SINGLE_STRICT_RE = new RegExp(`(?<![0-9])[1-9](?![0-9])${GLUED_AND_SINGLE_TAIL}|${GLUED_SPELLED_SINGLE}`);
const GLUED_AND_SINGLE_FREE_RE = new RegExp(`(?<![0-9a-z])[1-9](?![0-9])${GLUED_AND_SINGLE_TAIL}|(?<![a-z])(?:one|two|three|four|five|six|seven|eight|nine)${GLUED_AND_SINGLE_TAIL}`);
...
const SQUASHED_MINOR_AGE_RE = new RegExp([
  ...
  // Round 22 (accounts#5): a SINGLE digit (or one..nine) only through an
  // explicit and / n / und joiner -- "9andhorny", "jess9nwet" -- since a bare
  // single digit beside a word is far too often a name or a version
  // ("cloud9girl", "b2boy").
  `(?:(?<![0-9])[1-9](?![0-9])|${'(?:one|two|three|four|five|six|seven|eight|nine)'})(?:and|n|und)a?(?:${AGE_NOUN_ADJ})?(?:${SQ_OPEN_WORD}|${SQ_CLOSED_WORD}(?![a-z]))`,
].join('|'));
...
//     bio there is) -- a number DIRECTLY after "<parent word> of/to" (no
//     "a"/"my" between, no age word after it) counts children (round 22:
//     see FAMILY_COUNT_RE);
...
// it.
// FAMILY_AGE_WORD (below), for the count rule's "no age word after it".
const FAMILY_AGE_WORD_EARLY = '[\\s-]{0,2}(?:yo|y\\.?o\\.?|y\\s?\\/\\s?o|years?[\\s-]{0,2}olds?|yrs?[\\s-]{0,2}olds?)(?![a-z])';
...
// Round 22 (accounts#0): a COUNT only. Round 21 also took "(a|an|my)" after
// "of"/"to" and any age word after the number, so "Daddy to my 13 year old
// slut", "mommy of a 12 year old slut" and "Papa to my 16 yo slut" had their
// age rewritten to 0 before any age rule ran. A number after "of a" / "to my"
// is never a count of children, and a number followed by an age word never
// is. And the DDLG words ("daddy", "mommy", "papa", "parents") count only a
// single digit: "Daddy of 14 and horny" is not a parent's bio, while "Mom of
// 13 and horny" (a bare count, no age word) stays a MILF bio.
const FAMILY_COUNT_PARENT_WORD = '(?:mom|moms|mother|mothers|mum|mums|mama|mamas|momma|mummy|mamma|dad|dads|father|fathers|grandma|grandmother|granny|nana|grandpa|grandfather|stepmom|stepdad)';
const FAMILY_COUNT_DDLG_WORD = '(?:daddy|mommy|papa|parent|parents)';
const FAMILY_COUNT_NOT_AGE = `(?!${FAMILY_AGE_WORD_EARLY})`;
const FAMILY_COUNT_RE = new RegExp(`(?<![a-z])(${FAMILY_COUNT_PARENT_WORD}[\\s-]{1,2}(?:of|to)[\\s-]{1,2})(?<![0-9])${FAMILY_NUMBER}${FAMILY_COUNT_NOT_AGE}`
  + `|(?<![a-z])(${FAMILY_COUNT_DDLG_WORD}[\\s-]{1,2}(?:of|to)[\\s-]{1,2})(?<![0-9])${FAMILY_SINGLE}${FAMILY_COUNT_NOT_AGE}`, 'g');
...
const FAMILY_CLAUSE_COMMERCE_RE = new RegExp('(?<![a-z])(?:pics?|pix|pictures?|photos?|vids?|videos?|clips?|content|sell\\w*|for sale|buy\\w*|price\\w*|rates?'
  ...
  + '|[$\u00a3\u20ac]\\s?[0-9]');
// Round 22 (accounts#1): the WHOLE text, not the rest of the sentence. Round
// 21 read only up to the next . ! ? ; so "My daughter just turned 13! Pics of
// her for sale" and "our 13 year old daughter. pics $20" had the age
// neutralized with the sale one sentence away. `offset` is kept for the
// callers' shape; the answer no longer depends on it.
function familyClauseIsClean(text) {
  return !FAMILY_CLAUSE_SEXUAL_RE.test(text) && !FAMILY_CLAUSE_COMMERCE_RE.test(text);
}
function neutralizeFamilyAges(text) {
  let t = text.replace(FAMILY_COUNT_RE, (m, lead, lead2) => `${lead ?? lead2}0`);
  ...
  for (const re of [FAMILY_SUBJECT_AGE_RE, FAMILY_BABY_SINGLE_RE, FAMILY_PRONOUN_AGE_RE, FAMILY_POSSESSIVE_AGE_RE, FAMILY_POSSESSIVE_SINGLE_RE, FAMILY_BOY_MOM_RE]) {
    const before = t;
    t = before.replace(re, (m, lead) => (familyClauseIsClean(before) ? `${lead}0` : m));
  }
  return t;
}
// ROUND 22 -- THE CHILD-SAFETY PRINCIPLE (decided; overrides "free text never
// refuses legitimate input" for child safety ONLY). In free text and in tags,
// a text is refused when it contains BOTH
//   (a) a MINOR SIGNAL anywhere:
//       - an age 1-17 with an explicit age word ("13 year old", "7yo", "9 y/o")
//         that is not the age of a thing ("my 3 year old laptop");
//       - a family / child subject with an age ("my daughter just turned 13",
//         "my niece is 9", "our son, aged 12"), a third-person pronoun with an
//         age ("she's 7", "he just turned 8", a bare "just turned 5"), or a
//         first-person one ("i'm 9.");
//       - a child lead (child, kid, kiddie, kiddo, toddler, infant, minors,
//         underage, u18, preteen, schoolgirl/schoolboy, prepubescent) that is
//         not negated or disclaimed ("no kids", "zero tolerance for child
//         porn") and not a count ("mom of 3 kids");
//   (b) and a SEXUAL or COMMERCE word ANYWHERE in the text -- any sentence
//       (PRINCIPLE_SEXUAL_RE / PRINCIPLE_COMMERCE_RE).
// So "My daughter just turned 13! Pics of her for sale", "She's 7. Nudes $20",
// "Daddy to my 13 year old slut" and "my niece is 9 and horny" are refused,
// while "Mom of 2 and horny" (a bare count: no age word, no child subject with
// an age), "My daughter just turned 5!" and "my 3 year old boy is sick" pass.
// Accepted cost, deliberately: a parent who puts a child's age and "pics" or
// "dm" in the same message ("my son just turned 7! party pics soon") is
// refused and has to reword. That is the trade the owner decided.
// The sexual list leaves out the words a parent says about a small child every
// day (naughty, dirty, wet, tight, needy, thirsty, freaky) -- the narrower
// age rules still read those beside an age ("12 and naughty", "9 and wet").
const PRINCIPLE_DIGIT = '(?<![0-9$£€.,#])(?:1[0-7]|[1-9])(?![0-9])';
const PRINCIPLE_SPELLED = '(?<![a-z])(?:one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen)(?![a-z])';
const PRINCIPLE_AGE_WORD = '[\\s-]{0,2}(?:yo|y\\.?o\\.?|y\\s?\\/\\s?o|years?[\\s-]{0,2}olds?|yrs?[\\s-]{0,2}olds?)(?![a-z])(?!\\s*mama)';
// Not a count, a unit, a height, a range or a score after a bare digit age.
// On ONE line: a count word on the next line starts a new line, it is not a
// unit ("my daughter just turned 13\npics of her for sale").
const PRINCIPLE_NOT_COUNT = `(?![0-9])(?![.,/:x-]?[ \\t]?[0-9])(?![%$+])(?!in(?![a-z]))(?![ \\t]+(?:for|of|out of)[ \\t]+[0-9])(?![ \\t-]{0,2}${COUNT_WORD}(?![a-z]))`
  + '(?![ \\t]?[\'\u2019\u2032"])(?![ \\t]*(?:to|-|\u2013|or)[ \\t]*[0-9])(?![ \\t-]{0,2}(?:each|ea|per|apiece|ft|feet|foot)(?![a-z]))';
const PRINCIPLE_THING_SUBJECT = `(?<!${THING_NOUN}[\\s-]{1,2}(?:is|was|turned|turns|just\\s+turned|will\\s+be)[\\s-]{1,2})`;
// The age of a THING, optionally through one ordinary adjective ("our 3 year
// old sexy dog", "my 2 year old red car") -- never through a person or sexual
// noun, and never with one following the thing ("a 13 year old house slut").
// The principle's own extra things: the long form is ordinary English for the
// age of a game, a song, a film, a recipe ("this 15 year old video game").
const PRINCIPLE_THING_NOUN = `(?:${THING_NOUN}|video[\\s-]?games?|games?|songs?|movies?|films?|shows?|memes?|recipes?|versions?|marriage|relationship|friendship|albums?|records?|jokes?|photos?\\s+of\\s+me|pics?\\s+of\\s+me)`;
const PRINCIPLE_NOT_A_THING = `(?![\\s-]{1,3}(?:(?!${PERSON_OR_SEXUAL_NOUN}(?![a-z]))[a-z]+[\\s-]{1,2})?(?:${THING_MODIFIER}[\\s-]{1,2})?${PRINCIPLE_THING_NOUN}(?![a-z])`
  + `(?!(?:[^a-z0-9]{1,3}[a-z]+)?[^a-z0-9]{1,3}${PERSON_OR_SEXUAL_NOUN}(?![a-z])))`;
const PRINCIPLE_AGE_WORD_RE = new RegExp(`${PRINCIPLE_THING_SUBJECT}(?<!(?:i|we)\\s+(?:was|were)\\s+)(?:${PRINCIPLE_DIGIT}|${PRINCIPLE_SPELLED})${PRINCIPLE_AGE_WORD}${PRINCIPLE_NOT_A_THING}${NOT_A_DRINK}`);
const PRINCIPLE_CHILD_SUBJECT = '(?:daughters?|sons?|stepdaughters?|stepsons?|nieces?|nephews?|kids?|kiddos?|child|children|toddlers?|boys?|girls?|bab(?:y|ies)|little\\s+ones?|youngest|oldest|eldest|middle\\s+child|twins|grand(?:sons?|daughters?|kids?|child(?:ren)?))';
const PRINCIPLE_VERB = "(?:just\\s+turned|turned|turns|is\\s+now|is|'s|was|will\\s+be|just\\s+hit|aged?|age)";
const PRINCIPLE_ADVERB = '(?:(?:only|just|now|already|barely|literally)\\s+)?';
// A digit, or a spelled number that ends the clause ("she's seven.") -- a
// spelled number running on is too often not an age ("she's one hot mama").
const PRINCIPLE_NUMBER = `(?:${PRINCIPLE_DIGIT}${PRINCIPLE_NOT_COUNT}|${PRINCIPLE_SPELLED}(?=\\s*(?:$|[.!?,;:)])))`;
const PRINCIPLE_SUBJECT_AGE_RE = new RegExp(`(?<![a-z])${PRINCIPLE_CHILD_SUBJECT}(?:\\s+who)?[\\s,-]{1,3}${PRINCIPLE_VERB}[\\s:,-]{1,3}${PRINCIPLE_ADVERB}${PRINCIPLE_NUMBER}`);
// A child subject with an age is an age whatever noun follows it: after
// is / aged / turned the digit is never a count of the pics that follow
// ("my daughter is 12 pics", "son aged 13 vids"). Units, heights, ranges and
// time counts stay excluded ("the baby is 2 weeks old", "he is 5 foot").
const PRINCIPLE_COUNT_WORD_NO_CONTENT = COUNT_WORD.replace('|pics?|photos?|pictures?|videos?|vids?|clips?|sets?|packs?|items?|posts?', '');
const PRINCIPLE_NOT_UNIT = PRINCIPLE_NOT_COUNT.replace(COUNT_WORD, PRINCIPLE_COUNT_WORD_NO_CONTENT)
  .replace('(?![ \\t]+(?:for|of|out of)[ \\t]+[0-9])', '');
const PRINCIPLE_SUBJECT_AGE_LOOSE_RE = new RegExp(`(?<![a-z])(?:${PRINCIPLE_CHILD_SUBJECT}|sis|sisters?|bros?|brothers?|step[\\s-]?(?:sis|sister|bro|brother))(?:\\s+who)?[\\s,-]{1,3}${PRINCIPLE_VERB}[\\s:,-]{1,3}${PRINCIPLE_ADVERB}${PRINCIPLE_DIGIT}${PRINCIPLE_NOT_UNIT}`);
// A bracketed age after a person word ("girl (13) nudes", "my niece (13)",
// "sis [12]"), and a bare age straight after a family noun ("daughter 12
// pics", "my son, 13, dm me") -- a family noun followed by a number is that
// child's age, not a count.
const PRINCIPLE_PERSON_SINGULAR = '(?:daughter|son|stepdaughter|stepson|step[\\s-]?(?:daughter|son|sis|sister|bro|brother)|niece|nephew|kid|kiddo|child|toddler|boy|girl|baby|sis|sister|bro|brother|grandson|granddaughter|grandkid|grandchild)';
const PRINCIPLE_BRACKET_AGE_RE = new RegExp(`(?<![a-z])${PRINCIPLE_PERSON_SINGULAR}[\\s,:-]{0,3}[(\\[{]\\s*(?:age[\\s:]{0,2})?${PRINCIPLE_DIGIT}\\s*(?:${PRINCIPLE_AGE_WORD.replace('[\\s-]{0,2}', '')})?\\s*[)\\]}]`);
const PRINCIPLE_FAMILY_BARE_AGE_RE = new RegExp(`(?<![a-z])(?:daughter|son|stepdaughter|stepson|niece|nephew|grandson|granddaughter)[\\s,:-]{1,3}${PRINCIPLE_DIGIT}${PRINCIPLE_NOT_UNIT}`);
const PRINCIPLE_PRONOUN_AGE_RE = new RegExp(`(?<![a-z])(?:(?:she|he)(?:'s|\\s+is|\\s+just\\s+turned|\\s+turned|\\s+turns|\\s+is\\s+now|\\s+will\\s+be|\\s+was)`
  + `|(?<!(?:${THING_NOUN}|i|i'm|im|i am|we)[\\s,]{1,2})just\\s+turned)[\\s:,-]{1,3}${PRINCIPLE_ADVERB}${PRINCIPLE_NUMBER}`);
const PRINCIPLE_FIRST_PERSON_RE = new RegExp(`(?<![a-z])(?:i'?m|i am|aged)[\\s:,-]{1,3}${PRINCIPLE_ADVERB}${PRINCIPLE_DIGIT}${PRINCIPLE_NOT_COUNT}`
  + '(?=\\s*(?:$|[.!?,;:)]|(?:and|&|n|yo|y\\.?o|years?|yrs?)(?![a-z])))');
const PRINCIPLE_CHILD_LEAD_RE = new RegExp('(?<![a-z0-9])(?<!(?:adult|grown|grown-up|grownup)[\\s-]{1,2})(?:child(?:ren|s)?|kids?|kiddies|kiddie|kiddy|kiddos?|toddlers?|infants?|minors'
  + '|underage|under[\\s-]?aged?|u[\\s_/-]?18|pre[\\s-]?teens?|schoolgirls?|schoolboys?|pre[\\s-]?pubescent|prepubescent)(?![a-z0-9])(?![\\s-]{0,2}free)', 'g');
// A count in front of the child word ("mom of 3 kids", "two toddlers").
const PRINCIPLE_COUNT_BEFORE_RE = /(?:(?<![0-9])[0-9]{1,2}|(?<![a-z])(?:one|two|three|four|five|six|seven|eight|nine|ten))[\s-]{1,2}(?:(?:little|small|young|grown|adult|amazing|beautiful|wonderful|wild|crazy)[\s-]{1,2})?$/;
// A negation or a disclaimer earlier in the same clause ("no kids, no drama",
// "No sex with minors ever", "u18 not allowed" is the general isDisclaimed).
const PRINCIPLE_NEGATION_BEFORE_RE = /(?:^|[^a-z])(?:no|not|never|nothing|nobody|zero|none|dni|anti|against|without|don'?t|do not|won'?t|will not|report\w*|ban\w*|block\w*|fight\w*|survivors?|victims?|protect\w*|prevent\w*|combat\w*|stop|end|free of)(?![a-z])[^.!?;\n]{0,30}$/;
const PRINCIPLE_SEXUAL_RE = new RegExp('(?<![a-z])(?:horny|slutty|kinky|nudes?|naked|sexy|sex|sexual|sexting|sexts?|slut\\w*|whores?|hoes?|bitch\\w*|cunts?|pussy|pussies'
  + '|porn\\w*|cum|cums|cumming|cummed|cumslut\\w*|fuck\\w*|dick|cock|tits|boobs|virgins?|milf|onlyfans|twink|bimbo|nympho|touch(?:ing)?\\s+(?:her|him|them))(?![a-z])'
  + '|(?<![a-z])xx+(?![a-z])(?=[^a-z0-9]*[a-z0-9])');
const PRINCIPLE_COMMERCE_RE = new RegExp('(?<![a-z])(?:pics?|pix|pictures?|photos?|vids?|videos?|clips?|content|customs?|for\\s+sale|4\\s?sale|sell(?:s|ing)?|price\\w*'
  + '|dms?|inbox|message\\s+me|msg\\s+me|wanna\\s+see|want\\s+to\\s+see|see\\s+(?:her|him|them)|(?:her|his|their)\\s+body|bodies|in\\s+person|meet(?:up|s)?'
  + '|link\\s+in\\s+bio|subscribe)(?![a-z])|[$£€]\\s?[0-9]');
// Words that are not sexual here: sex education and the abuse / trafficking
// vocabulary always; the advocacy nouns only behind an advocacy lead.
const PRINCIPLE_SEX_NONSEXUAL_RE = new RegExp(`(?<![a-z])sex(?:ual)?${SEX_EDUCATION_AFTER.replace('(?!', '(?=')}[^a-z0-9]{0,3}[a-z]+(?:\\s+material)?`, 'g');
function principleSexualOrCommerce(text) {
  let t = text.replace(PRINCIPLE_SEX_NONSEXUAL_RE, ' ');
  if (ADVOCACY_ANYWHERE_RE.test(t)) t = t.replace(/(?<![a-z])sex(?:ual)?[^a-z0-9]{0,3}(?:slavery|slaves?|tourism|tourists?|rings?|trade|trades|trading|work|workers?|exploit\w*)(?![a-z])/g, ' ');
  return PRINCIPLE_SEXUAL_RE.test(t) || PRINCIPLE_COMMERCE_RE.test(t);
}
const ADVOCACY_ANYWHERE_RE = /(?<![a-z])(?:fight(?:s|ing)?|stop(?:ping)?|end(?:ing)?|against|anti|survivors?\s+of|victims?\s+of|report(?:ing)?|combat(?:ing)?|prevent(?:ing)?|eradicat(?:e|ing)|abolish(?:ing)?|awareness|advocate|activist)(?![a-z])/;
// True when `text` carries a minor signal (see above). In a tag (strictAge)
// only the general disclaimer excuses a child lead.
function hasMinorSignal(text, strictAge = false) {
  if (PRINCIPLE_AGE_WORD_RE.test(text) || PRINCIPLE_SUBJECT_AGE_RE.test(text) || PRINCIPLE_PRONOUN_AGE_RE.test(text)
    || PRINCIPLE_FIRST_PERSON_RE.test(text) || PRINCIPLE_SUBJECT_AGE_LOOSE_RE.test(text)
    || PRINCIPLE_BRACKET_AGE_RE.test(text) || PRINCIPLE_FAMILY_BARE_AGE_RE.test(text)) return true;
  for (const m of text.matchAll(PRINCIPLE_CHILD_LEAD_RE)) {
    const before = text.slice(Math.max(0, m.index - 40), m.index);
    if (PRINCIPLE_COUNT_BEFORE_RE.test(before)) continue;
    // A tag honours only the general disclaimer the "underage" term itself
    // honours there ("Do not message me if underage"); free text also the
    // anti-CSAM wording and a negation earlier in the clause.
    if (isDisclaimed(text, m.index, m.index + m[0].length)) continue;
    if (!strictAge && (PRINCIPLE_NEGATION_BEFORE_RE.test(before) || isChildDisclaimed(text, m.index, m.index + m[0].length))) continue;
    return true;
  }
  return false;
}
// Cheap gate: no number and no child word means no minor signal at all.
const PRINCIPLE_PREFILTER_RE = /[0-9]|(?<![a-z])(?:one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen)(?![a-z])|child|kid|toddler|infant|minors|under|u[\s_/-]?18|teen|school|pubescent/;
function childSafetyPrincipleHit(text, strictAge = false) {
  if (!PRINCIPLE_PREFILTER_RE.test(text)) return false;
  const t = text.replace(EMOJI_RUN_RE, ' ');
  return principleSexualOrCommerce(t) && hasMinorSignal(t, strictAge);
}

// Round 22 (accounts#7): "my little girl" / "our lil boy" / "mommy of a little
// boy" in FREE text with no sexual or commerce word anywhere is a parent.
const LITTLE_ONE_OWNER_BEFORE_RE = /(?:(?<![a-z])(?:my|our|his|her|their)|(?<![a-z])(?:mom|mommy|mum|mummy|mama|momma|mother|dad|daddy|father|papa)[\s-]{1,2}(?:of|to)[\s-]{1,2}an?)[\s-]{1,2}$/;
// The exemption needs the absence of EVERY adult cue, not just the family
// clause lists: the principle's own sexual and commerce words, the platform's
// selling vocabulary (customs, tips, tip menu, menu, spoil), DDLG and bedroom
// cues (daddy / papa after the phrase, bed, bath, shower, spread, "waiting for
// you", "wants you") and sexual emoji. "my little girl, customs open", "our
// little girl wants daddy", "her little girl in the bath", "my little girl 🍑"
// stay refused; "my little girl just turned 5!" passes.
const LITTLE_ONE_ADULT_CUE_RE = new RegExp('(?<![a-z])(?:customs?|tips?|tip[\\s-]?menu|menu|spoil\\w*|beds?|bedroom|bath|bathtub|shower\\w*|spread\\w*|wet|tight|naughty|needy|kinky|freaky|nasty|stroke\\w*|ride|riding'
  + '|waiting\\s+for\\s+(?:you|u|daddy|me)|wants?\\s+(?:you|u|it|more|daddy|papa)|ready\\s+for\\s+(?:you|u|daddy)|play\\s*time|come\\s+play)(?![a-z])'
  + '|[\\u{1F351}\\u{1F346}\\u{1F4A6}\\u{1F445}\\u{1F608}\\u{1F975}\\u{1F60F}\\u{1FAE6}\\u{1F48B}\\u{1F924}]', 'u');
// DDLG words count only AFTER the phrase -- "daddy of a little girl" is the
// owner lead itself.
const LITTLE_ONE_DDLG_AFTER_RE = /(?<![a-z])(?:daddy|daddies|papa|mommy|dom|domme|sub|master)(?![a-z])/;
function isFamilyLittleOne(text, start) {
  const window = text.slice(Math.max(0, start - 20), start);
  const owner = window.match(LITTLE_ONE_OWNER_BEFORE_RE);
  if (!owner) return false;
  const rest = text.slice(0, Math.max(0, start - 20) + owner.index) + ' ' + text.slice(start);
  return !FAMILY_CLAUSE_SEXUAL_RE.test(text) && !FAMILY_CLAUSE_COMMERCE_RE.test(text)
    && !principleSexualOrCommerce(text) && !LITTLE_ONE_ADULT_CUE_RE.test(rest)
    && !LITTLE_ONE_DDLG_AFTER_RE.test(text.slice(start));
}

// Round 22 (accounts#8), FREE text only: a COUNT of items in listing copy --
// "Includes 12 nudes and 3 videos", "Set of 10 nudes", "Pack of 15 nudes",
// "Unlock 15 nudes", "x12 nudes", "15 Nudes + 2 Videos". The number is
// neutralized after a collection lead, or when the content word is followed by
// a list joiner and another "<number> <counted thing>". Never with an age word
// between the number and the content word (the adjective slot is a fixed
// list), and never when the text carries any minor signal (the principle
// decides those).
const COLLECTION_CONTENT = '(?:(?:hot|sexy|new|exclusive|explicit|nude|naked|wet|spicy|full|private|custom|unseen|bonus|extra|hd|4k)[\\s-]{1,2})?(?:nudes?|porn(?:o|\\s+(?:videos?|vids?|clips?|pics?))?|pics?|photos?|videos?|vids?|clips?|sets?|selfies?|gifs?|images?)(?![a-z])';
const COLLECTION_LEAD_COUNT_RE = new RegExp(`(?<![a-z])((?:(?:set|pack|bundle|total|collection|batch|bunch|series)[\\s-]{1,2}of|includes?|including|contains?|containing|unlocks?|you\\s+get|get|comes\\s+with|with)[\\s:-]{1,3}|(?<![a-z0-9])x\\s?)(?:1[0-7])(?![0-9])(?=[\\s-]{1,2}${COLLECTION_CONTENT})`, 'g');
const COLLECTION_LIST_COUNT_RE = new RegExp(`(?<![0-9$£€.])(1[0-7])(?![0-9])(?=[\\s-]{1,2}${COLLECTION_CONTENT}[\\s]{0,2}(?:and|&|\\+|,)[\\s]{0,2}[0-9]{1,3}[\\s-]{1,2}(?:[a-z]+[\\s-]{1,2})?(?:videos?|vids?|clips?|pics?|photos?|gifs?|selfies?|sets?|nudes?|images?)(?![a-z]))`, 'g');
function neutralizeCollectionCounts(text) {
  const out = text.replace(COLLECTION_LEAD_COUNT_RE, (m, lead) => `${lead}0`).replace(COLLECTION_LIST_COUNT_RE, '0');
  // Only when there was a count to neutralize is the (costlier) signal read.
  if (out === text || hasMinorSignal(text.replace(EMOJI_RUN_RE, ' '))) return text;
  return out;
}
...
export function neutralizeNonAges(text, { strictAge = false } = {}) {
  ...
  t = neutralizeCollectionCounts(neutralizeFamilyAges(t))
...
// Round 22 (accounts#5): the single-digit "and" tail is built from the same
// lists as the two-digit SEXUAL_AND / SPELLED_AND: the sexual adjectives (wet,
// sexy, naughty, freaky included -- "i'm 9 and wet", "im 8 and sexy"), the
// person and sexual nouns, and "wants / loves cock" and the like. Round 20's
// list held only horny / slutty / kinky / nude / naked / a virgin.
// The THIRD-PERSON form (SINGLE_AND_THIRD) leaves out the words a parent says
// about a small child (naughty, dirty, wet, needy, thirsty): "she's 3 and so
// naughty today" is a parent. The sexual verbs and wanting verbs count only
// with a sexual object there ("she's 8 and wants cock").
const SINGLE_WANT_SEXUAL = '(?:wants?|loves?|likes?|needs?|craves?|craving)\\s+(?:cock|dick|cum|sex|porn|to\\s+(?:fuck|suck|cum)|older\\s+men|daddy)';
const SINGLE_AND = '(?=\\s*(?:and|&|n)\\s+(?:(?:so|very|super|always|really|still)\\s+)?(?:' + SEXUAL_ADJ + '|' + PERSON_OR_SEXUAL_NOUN.replace('kids?|', '').replace('daughters?|stepdaughters?|sons?|stepsons?|', '').replace('sisters?|stepsis(?:ters?)?|brothers?|cousins?|nieces?|nephews?|', '')
  + '|' + SINGLE_WANT_SEXUAL + '|a\\s+(?:virgin|slut|whore|hoe|bitch))(?![a-z]))';
// ...after a FIRST-person lead, the wider SPELLED_AND list (sexual verbs, "and
// looking", "and ready") -- "i'm 9 and love cock".
const SINGLE_AND_FIRST = '(?=\\s*(?:and|&|n)\\s+(?:(?:so|very|super|always|really|still)\\s+)?(?:'
  + SEXUAL_ADJ + '|' + SEXUAL_NOUN + '|' + SEXUAL_VERB + '|' + SINGLE_WANT_SEXUAL
  + '|loves?\\s+(?:cock|dick|cum|sex|porn)|ready|dtf|a\\s+(?:virgin|slut|whore|hoe|bitch))(?![a-z]))';
const SINGLE_AND_THIRD = '(?=\\s*(?:and|&|n|,)\\s+(?:(?:so|very|super|always|really|still)\\s+)?(?:horny|slutty|kinky|nude|naked|sexy'
  + '|' + SEXUAL_NOUN + '|' + SINGLE_WANT_SEXUAL + '|a\\s+(?:virgin|slut|whore|hoe|bitch))(?![a-z]))';
const SINGLE_DIGIT_AGE_RE = new RegExp([
  ...
  // Round 22 (accounts#5): the age word, then "and" + a sexual word ("9 years
  // old and horny", "8 yo and sexy"), and a spelled age opening the label
  // ("seven and horny").
  `${SINGLE_AGE}${SINGLE_AGE_WORD}${SINGLE_AND}`,
  `(?:^|[.!?;:,\\n]\\s*)(?:one|two|three|four|five|six|seven|eight|nine)${SINGLE_AND}`,
  ...SINGLE_ADJ_ALTS,
].join('|'));
...
//   - a third-person lead counts only with a sexual continuation ("she's 7
//     and horny"), and a parent's "She just turned 4." is neutralized before
//     any rule reads it (neutralizeFamilyAges). Round 22: "and naked" counts
//     too -- under the decided child-safety principle a pronoun with an age
//     and a sexual word is refused, so "she's 2 and naked in the bath" is the
//     accepted cost;
//   - (round 21 had the bare "9 and horny" count only where the digit OPENS
//     the text; round 22 reads it after any word, since a count, a rank or a
//     size in front -- "mom of 2", "round 2", "size 8" -- is neutralized to 0
//     first, which is what the round-21 restriction was protecting);
...
// Round 22 (accounts#4): round 21 dropped refusals round 20 made -- "my
// niece is 9 and horny", "hot 9 and horny", "Hi, 9 and horny", "she is 9 and
// a virgin", "she's 8, horny". Restored: any digit directly followed by the
// "and" tail (a count or rank in front -- "mom of 2", "round 2", "size 8" --
// has already been neutralized to 0 by neutralizeNonAges, which is what
// round 21 was really protecting), and a third-person lead with the "and" or
// comma tail including "a virgin". The child-safety principle catches the
// rest (a family subject with an age anywhere near a sexual word).
const SINGLE_AND_STRONG = SINGLE_AND_THIRD;
const SINGLE_MENU_ITEM = '(?<![a-z])(?:pussy|nudes?|videos?|vids?|pics?|photos?|sexting|customs?|clips?|lingerie|panties|socks|toys?|sets?|tapes?|porn|boobs|tits|feet|gifs?|selfies?|ratings?|rates?|bundles?|gfe)';
const SINGLE_DIGIT_AGE_FREE_RE = new RegExp([
  `${SINGLE_AGE}${SINGLE_AGE_WORD}[\\s_.,:-]{0,3}(?:${AGE_NOUN_ADJ}[\\s_.-]{1,3})?${SINGLE_AGE_PERSON}(?![a-z])`,
  `(?<![a-z])${SINGLE_AGE_PERSON}[\\s_.,:-]{1,3}${SINGLE_AGE}${SINGLE_AGE_WORD}`,
  `${SINGLE_FIRST_LEAD}${SINGLE_AGE}(?:${SINGLE_AGE_WORD})?(?:(?=\\s*(?:$|[.!?;,)]))|${SINGLE_AND_FIRST})`,
  `${SELF_LEAD}${SINGLE_AGE}(?:${SINGLE_AGE_WORD})?${SINGLE_AND_STRONG}`,
  // ...not a single-digit price after a menu item ("Nudes 5 & wet pussy 9").
  `(?<![0-9$\u00a3\u20ac./a-z])(?<!${SINGLE_MENU_ITEM}[\\s:]{1,2}[$\u00a3\u20ac]?)[1-9](?![0-9])${SINGLE_AND}`,
  // a spelled single age only opening the text or a sentence ("nine and
  // horny"; never "cloud nine and horny")
  `(?:^|[.!?;:\\n]\\s*)(?:one|two|three|four|five|six|seven|eight|nine)${SINGLE_AND}`,
  `${SINGLE_AGE}${SINGLE_AGE_WORD}${SINGLE_AND}`,
  ...SINGLE_ADJ_ALTS,
].join('|'));
...
function minorAgeHit(ageText, { strictAge, squashWhole }) {
  ...
  return ageRe.test(ageMasked) || BARELY_MINOR_AGE_RE.test(ageMasked) || BARELY_MINOR_AGE_RE.test(gluedAge)
    ...
    || [gluedAge, gluedDash, glued].some((g) => (strictAge ? GLUED_AND_SINGLE_STRICT_RE : GLUED_AND_SINGLE_FREE_RE).test(g))
...
function detectIn(normalized, add, { compound = true, strictAge = !compound, squashWhole = !compound } = {}) {
  ...
  for (const t of COMPILED) {
    ...
    if (t.words) {
      for (const m of normalized.matchAll(re)) {
        ...
        // Round 22 (accounts#2): "fighting child sex slavery" -- free text,
        // an advocacy lead, no commerce after.
        if (!strictAge && word === 'sex' && isAdvocacyPair(normalized, m.index, m.index + m[0].length)) continue;
    ...
    if (t.familyExempt && !strictAge) {
      for (const m of normalized.matchAll(new RegExp(re.source, 'g'))) {
        if (/[a-z]/.test(m[0]) && !isFamilyLittleOne(normalized, m.index)) {
          add(term, category);
          break;
        }
      }
  ...
  // Round 22 (accounts#6): "sex with minors", "fuck a kid" -- disclaimable in
  // free text only; "cp" beside a sale word, every context.
  for (const m of normalized.matchAll(CHILD_WITH_RE)) {
    const start = m.index;
    const end = m.index + m[0].length;
    if (!strictAge && (isChildDisclaimedNoSale(normalized, start, end)
      || (PRINCIPLE_NEGATION_BEFORE_RE.test(normalized.slice(Math.max(0, start - 40), start))
        && !CHILD_COMMERCE_STRONG_AFTER_RE.test(normalized.slice(end, end + 40))))) continue;
    add('child sexual content', 'minor-suggestive');
    break;
  }
  if (CP_COMMERCE_RE.test(normalized)) add('cp', 'minor-suggestive');
  // Round 22: the child-safety principle (see childSafetyPrincipleHit) -- a
  // minor signal and a sexual or commerce word anywhere in the text. Free
  // text and tags; a name (compound off) is one token and has its own rules.
  if (compound && childSafetyPrincipleHit(normalized.replace(/[\u2018\u2019\u02bc]/g, "'"), strictAge)) {
    add('minor with sexual or commerce content', 'minor-suggestive');
  }
