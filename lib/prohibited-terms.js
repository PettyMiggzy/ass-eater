...
const NAME_SPELLED_AGE_RE = new RegExp(
  `(?<![a-z])${NAME_SPELLED_MINOR}[^a-z0-9]{0,2}${NAME_SEXUAL_WORD}(?![a-z])`
  + `|(?<![a-z])${NAME_SEXUAL_WORD}[^a-z0-9]{1,2}${NAME_SPELLED_MINOR}(?![a-z])`
  + `|(?<![a-z])ten[^a-z0-9]{1,2}${NAME_TEN_SEXUAL_WORD}(?![a-z])`
...
function detectIn(normalized, add, { compound = true } = {}) {
  ...
  if (!compound) {
    ...
    if (NAME_SPELLED_AGE_RE.test(nameMasked)) add('under-18 age', 'minor-suggestive');
  }
