...
// refused. It now also has the everyday descriptors of PERSON_ADJ and the
// usual handle nouns. Each is only ever tested GLUED to the term, on the
// allowlist-masked text, so a surname (Steen, Kirsteen) is never involved.
...
const NAME_DESCRIPTOR_RES = [
  ...
  // "teenass", which no Teena name spells.
  const tail = t.term === 'teen' ? '(?:(?![a4@])[a-z0-9]|[a4@][s5$]{2})' : '[a-z0-9]';
...
function detectIn(normalized, add, { compound = true } = {}) {
  ...
  if (!compound) {
    // On the masked text too (round-13 accounts#3): "kirsteendickson" and
    // "justeencummings" are a first name and a surname, not "teen" + "dick".
    for (const { term, category, re } of NAME_COMPOUND_RES) if (re.test(masked)) add(term, category);
...
export function screenPublicText(text, { context = null } = {}) {
  ...
  // (splitNameWords).
  const prohibited = detectProhibitedTerms(text, { nameLike: isNameLikeContext(context) });
