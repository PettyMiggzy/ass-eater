 ...
 * NAME_COMPOUND_RES slice ("teenslut") and an edge-"x" reading ("xteenx").
 * The accepted cost: any other term glued inside an all-lowercase handle
 * ("rapeplay") is not caught there. A camelCase break needs two lowercase letters on its left and
...
function detectIn(normalized, add, { compound = true } = {}) {
  ...
  for (const { term, category, re, disclaimable } of COMPILED_COMPOUND) {
