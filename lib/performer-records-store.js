...
export async function updatePerformerRecord(id, input) {
  requireKey();
 ...
 */
export async function archivePerformerRecord(id, reason) {
  requireKey();
  const { rows } = await query(
    `update performer_records
...
export async function attachPerformerDocument(id, buffer, contentType, fileName) {
  requireKey();
  ...
  const { rows } = await query(
    `update performer_records
        set id_document = $2,
            data = data || jsonb_build_object('document', $3::jsonb, 'documentLocation', 'in-app'),
            updated_at = now()
      where id = $1
      returning id, data`,
    [id, encryptRecordDocument(buffer), JSON.stringify(meta)],
  );
  if (!rows.length) throw new Error('Record not found');
  return toAdminRecord(rows[0]);
}
