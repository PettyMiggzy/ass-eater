...
function retainUntil(producedAt) {
  const from = new Date(producedAt);
 ...
 */
export function matchesRecord(record, term, { extraIds = null } = {}) {
  const q = String(term || '').trim().toLowerCase();
  if (!q) return true;
  if ((record.aliases || []).some((a) => a.includes(q))) return true;
  ...
  if (target && record.creatorId != null && String(record.creatorId) === target.creatorId) return true;
...
/**
 * What a pasted URL points at on this site, or null: { creatorId, src?,
 * listingId? }. Accepts a full URL on any of the site's hosts or a bare
 * path; query strings and fragments are ignored.
 */
export function contentUrlTarget(term) {
  let path = String(term || '').trim();
  if (!path) return null;
  try {
