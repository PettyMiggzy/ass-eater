...
import { isWellFormedText, findMalformedText } from './unicode-text.js';
...
/**
 * The ONE shared refusal for text that cannot be stored (a NUL, or half an
 * emoji -- an unpaired surrogate), run at the top of every API route that
 * accepts a body. Such a string used to reach Postgres and fail the write
 * (jsonb refuses "\u0000" and lone "\udXXX" escapes; a text parameter refuses
 * NUL), which the routes reported as a generic 500 -- on a paid DM, after
 * screening; on a possible-minor report, never filed (round-11 social#1,
 * accounts#4). Now it is a 400 naming the field, before anything else runs.
 *
 * Checks req.body and req.query (values and keys, nested). `skip` lists
 * top-level body keys to leave alone -- only an EXISTING password being
 * verified, which must be compared exactly as typed. Returns true when it has
 * answered (the caller returns), false otherwise.
 */
export const MALFORMED_TEXT_ERROR = 'contains an invalid character (a control character or half of an emoji). Remove it and try again.';
export function refuseMalformedText(req, res, { skip = [] } = {}) {
  let field = findMalformedText(req?.body, { skip });
  if (field === null) field = findMalformedText(req?.query);
  if (field === null) return false;
  res.status(400).json({ error: `${field || 'Your text'} ${MALFORMED_TEXT_ERROR}`, field: field || null, code: 'MALFORMED_TEXT' });
  return true;
}
...
export function validateTextFields(fields, keys) {
  for (const key of keys) {
    ...
    // Half an emoji (an unpaired surrogate) or a NUL cannot be stored as
    // jsonb: refused here instead of failing the write with a 500.
