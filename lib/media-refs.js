 ...
 *     deleteUnfinalizedUpload and deleteMediaQuietly) or quarantined as
 *     evidence (media_preservations) with MEDIA_UPLOAD_EXPIRED, rather than
 *     recording a reference to nothing (or to evidence).
 *
...
export async function lockMediaForFinalize(client, src) {
  ...
  // Refused too: a pathname that is preserved evidence. Once evidence moves to
  // evidence/, its original path is empty, and a still-valid upload token can
  // re-create a file there (round-8 media#2) -- which must never be attached
  // to a gallery, avatar or listing and so served under a name that points at
  // quarantined material.
  const { rows } = await client.query(
    `select 1 from media_reaped where pathname = $1
      union all
...
/**
 * Whether this app has deleted `pathname` (media_reaped). A reaped path must
 * never be served again, even to its owner: an upload token can outlive the
 * file it was issued for and re-create it at the same path, and nothing
 * references or records such a file (it is swept after the token expires).
 */
export async function isMediaReaped(pathname) {
  if (!pathname) return false;
  const { rows } = await query('select 1 from media_reaped where pathname = $1', [pathname]);
  return rows.length > 0;
}

/**
 * Whether an owner may still be served `pathname` although no record points
 * at it: only an upload in progress -- a media_uploads row with reason
 * 'token' (issued by /api/media/upload-token, not yet finalized or swept) --
 * that this app has not deleted (media_reaped), quarantined
 * (media_preservations) or put on a report hold (media_holds).
 *
 * A file whose reference a takedown removed is none of these: its row is
 * 'delete_pending' / 'delete_failed', or it sits on a hold waiting for the
 * sweep, and the owner used to keep getting fresh presigned URLs for exactly
 * the file recorded as removed on a TAKE IT DOWN request until it was really
 * deleted (round-17 media#1).
 */
export async function isOwnerUploadInProgress(pathname) {
  if (!pathname) return false;
  const { rows } = await query(
    `select 1
       where exists (select 1 from media_uploads where pathname = $1 and reason = 'token')
         and not exists (select 1 from media_reaped where pathname = $1)
         and not exists (select 1 from media_preservations where pathname = $1)
         and not exists (select 1 from media_holds where pathname = $1)`,
    [pathname],
  );
  return rows.length > 0;
}
