...
section('public-pages#0 / dashboard#0 / dashboard#1: shipping text, history, notifications');
{
  ...
  // Round 19 (money#0, DECIDED): no digit minimum -- a bare word is the
  // accepted residual (non-blocking warning); an app name is still refused.
  const handle = orders.trackingFieldsError({ carrier: 'UPS', trackingNumber: 'jessxo' });
  check('a tracking number with no digit is accepted (round 19)', handle === null, JSON.stringify(handle));
  const app = orders.trackingFieldsError({ carrier: 'UPS', trackingNumber: 'snapchatjess' });
  check('...but an app name with no digit is still refused', app?.field === 'trackingNumber' && app.suspicious === true, JSON.stringify(app));
...
section('social#1: the admin conversation list is keyset-paged');
{
  ...
  for (let i = 1; i <= 5; i++) {
    // The other side is a real account (round 20: a thread with a DELETED
    // participant is listed under a sealed reference, not its id -- see
    // lib/r20b.test.mjs; this section is about paging, so both sides exist).
    await query(`insert into users (id, data) values ($1, $2::jsonb)`, [`o${i}`, JSON.stringify({ email: `o${i}@r14b.test`, role: 'fan' })]);
