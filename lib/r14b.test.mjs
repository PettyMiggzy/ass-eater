...
section('public-pages#0 / dashboard#0 / dashboard#1: shipping text, history, notifications');
{
  ...
  const cash = await ship('Cash App $jessxo 20% off direct', '9400100000000000000000');
  ...
  check('...both are logged to the violations queue', viol.includes('order_carrier') && viol.includes('order_tracking'), JSON.stringify(viol));
  ...
  const first = await ship('USPS', '9400 1000 0000 0000 0000 00');
  ...
  check('...without the tracking number in the text', !/9400/.test(notes[0].message), notes[0].message);

  const same = await ship('USPS', '9400 1000 0000 0000 0000 00');
  ...
  check('...and keeps the replaced carrier/number in trackingHistory', Array.isArray(hist) && hist.length === 1
    && hist[0].carrier === 'USPS' && hist[0].trackingNumber === '9400 1000 0000 0000 0000 00' && !!hist[0].replacedAt, JSON.stringify(hist));
  ...
  await ship('FedEx', '7489 1234 5678');
  ...
  const ghost = await ship('UPS', '1Z999AA10123456785', o2);
  check('an order with no buyer id still ships', ghost.statusCode === 200, JSON.stringify(ghost.body));
  check('the store helper accepts ordinary carriers', ['UPS', 'Royal Mail', 'DHL eCommerce', 'J&T Express', 'Canada Post / Postes Canada']
    .every((c) => !orders.trackingFieldsError({ carrier: c, trackingNumber: '1234' })));
  ...
  check('real carriers that merely contain an app-like word still pass',
    ['Instant Courier', 'XPO Logistics', 'Line Haul Express', 'Royal Mail', 'Chronopost', 'Canpar', 'Purolator', 'Evri', 'Yodel', 'La Poste']
      .every((c) => !orders.trackingFieldsError({ carrier: c, trackingNumber: 'RR123456785GB' })));
