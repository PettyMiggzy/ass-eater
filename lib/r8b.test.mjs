...
section('gates-token#0: login brakes are per IPv6 /64');
{
  ...
  // A different NETWORK. (Round 9 also brakes other /64s of the same /48 once
  // the account is saturated -- lib/r9b.test.mjs gates-token#0 -- so this
  // one sits in a different /48.)
  const other = await call(loginRoute, { body: { email: 'nobody@r8b.test', password: 'x' }, ip: '2001:db8:9:a::1' });
...
section('money#0: closing an unfulfillable physical order');
{
  ...
  // The close is for a seller who was banned (or deleted): round 9 enforces
  // that precondition (lib/r9b.test.mjs money#1), so the seller is banned here.
