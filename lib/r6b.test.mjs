...
section('media#0: quarantine takes listings off sale; checkout and delivery honour it');
{
  ...
  // Round 11 (public-pages#0): files held as evidence are withheld, not
  // "removed" -- nothing was deleted and the review may release them.
  check('delivery withholds preserved files', del.statusCode === 200 && del.body.items.length === 0 && del.body.withheld === 1 && del.body.removed === false && del.body.removedReason === null, JSON.stringify(del.body));
