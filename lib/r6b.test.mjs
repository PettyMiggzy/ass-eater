...
//    same commit; checkout refuses a listing with a preserved or held file;
...
section('media#0: quarantine takes listings off sale; checkout and delivery honour it');
{
  ...
  // A listing on a report HOLD stays 'active' but checkout refuses it.
  ...
  check('listingMediaBlocked sees a hold', await preservation.listingMediaBlocked(held));
  ...
  check('checkout refuses a listing with a held file', err?.code === 'LISTING_UNAVAILABLE', err && err.message);
