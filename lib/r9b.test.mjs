...
section('social#0 / dashboard#1: a block-only inbox row names nobody');
{
  ...
  // and the pair id is '<uidA>__<uidB>'.
  ...
  check('the limit=1 cursor exists and does not carry the commenter\'s id', !!cur && !decoded.includes(String(commenter.id))
    && !cur.includes(String(commenter.id)), decoded);
  const next = await call(conversationsRoute, { method: 'GET', user: a.user, query: { limit: '1', before: cur } });
  check('...and still pages (nothing after the only row)', next.statusCode === 200 && next.body.conversations.length === 0, JSON.stringify(next.body));
  ...
  check('an old-style plain cursor reads as the first page', f.statusCode === 200 && f.body.conversations.length === 1, JSON.stringify(f.body));
  ...
  check('...and the block is gone', (await messages.blockBetween(a.user.id, commenter.id)) === null);
