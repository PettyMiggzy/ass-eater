...
// unreferenced an hour later. It runs opportunistically on every upload-token
// request (a small batch) and in full from POST /api/admin/media-sweep.
  ...
  try {
    await query(
      `insert into media_uploads (pathname, reason) values ($1, $2)
         on conflict (pathname) do update set reason = excluded.reason, created_at = now()`,
      [pathname, reason],
    );
    return true;
  } catch (err) {
 ...
 */
export async function referencedMediaPaths(pathnames) {
...
/**
 * Reaps up to `limit` recorded pathnames older than `olderThanMs`: deletes
 * each file that nothing references, forgets each one that something does
 * (it was finalized). Rows are claimed with a DELETE ... SKIP LOCKED, so two
 * sweeps never work the same row; a file whose deletion fails is recorded
 * again and retried by a later sweep.
 *
 * Returns { checked, deleted, kept, failed }.
 */
export async function sweepOrphanedMedia({ olderThanMs = ORPHAN_AGE_MS, limit = 50, deleteFile = del } = {}) {
  ...
  const { rows } = await query(
    `delete from media_uploads
      ...
      returning pathname`,
    [Math.max(0, Math.floor(olderThanMs)), max],
  );
  const claimed = rows.map((r) => r.pathname);
  const summary = { checked: claimed.length, deleted: 0, kept: 0, failed: 0 };
  ...
  for (const pathname of claimed) {
    ...
    try {
      ...
      if (err instanceof BlobNotFoundError) {
        summary.deleted++; // never uploaded, or already gone: nothing to do
        continue;
      }
