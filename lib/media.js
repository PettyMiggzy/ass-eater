...
// Whether an upload token for a path recorded at `recordedAt` could still be
// valid. media_uploads rows are stamped when a token is issued and restamped
// when their file is deleted, so a row older than the TTL (plus a margin for
// clock skew between this server and Blob's) has no live token behind it.
const TOKEN_SKEW_MARGIN_MS = 5 * 60 * 1000;
export function tokenMayBeLive(recordedAt) {
  const t = recordedAt instanceof Date ? recordedAt.getTime() : Date.parse(recordedAt || '');
  if (Number.isNaN(t)) return true;
  return Date.now() - t < UPLOAD_TOKEN_TTL_MS + TOKEN_SKEW_MARGIN_MS;
}
...
export async function deleteUnfinalizedUpload(pathname, { deleteFile = del } = {}) {
  ...
  try {
    return await withTransaction(async (client) => {
      ...
      // The 'token' row is KEPT, with its clock restarted, not deleted: the
      // client token for this pathname is valid for UPLOAD_TOKEN_TTL_MS and
      // is not single-use once the file is gone (allowOverwrite:false only
      // refuses an EXISTING blob). Dropping the row let the uploader re-PUT
      // the refused file within the window and leave it in the store with
      // nothing recording it -- tombstoned, so never finalizable, and never
      // swept. The sweep comes back after ORPHAN_AGE_MS, by which time the
      // token has expired, and deletes whatever was re-uploaded.
      await client.query(
        `update media_uploads set created_at = now(), claimed_at = null, claim_token = null
          where pathname = $1 and reason = 'token'`,
        [pathname],
      );
...
export async function pendingDeletionCounts() {
  const { rows } = await query(
    `select count(*) filter (where reason = 'delete_failed')::int as failed,
            ...
            min(created_at) filter (where reason not in ('token', 'moved_token')) as oldest
       from media_uploads`,
  );
  return {
 ...
 *
 * ONE exception, 'moved_token' rows (round-8 media#2): the ORIGINAL path of a
 * preserved file whose evidence has been moved to evidence/ (or was found
 * missing), recorded by movePreservedToEvidence. The upload token issued for
 * that path may have been live at the move and could have re-created a file
 * there. Such a row waits out olderThanMs like a 'token' row, skips the
 * reference check (the taken-down record still points at the original src,
 * which is now only a pointer to evidence), and deletes whatever is at the
 * original path. The evidence copy lives at a different pathname and is never
 * touched here.
 *
 ...
 * fails is re-recorded as 'delete_failed' and retried by a later sweep (a
 * vacated evidence path as 'moved_token' again, so it stays claimable).
 *
...
export async function sweepOrphanedMedia({
    ...
    `update media_uploads
      ...
      where pathname in (
              select pathname from media_uploads
               where (reason not in ('token', 'moved_token') or created_at < now() - ($1::bigint * interval '1 millisecond'))
                 ...
                 and not exists (select 1 from media_preservations mp
                                  where mp.pathname = media_uploads.pathname
                                    and not (media_uploads.reason = 'moved_token'
                                             and (mp.evidence_pathname is not null or mp.missing_at is not null)))
                 and not exists (select 1 from media_holds mh where mh.pathname = media_uploads.pathname)
               order by created_at
               limit $2
               for update skip locked)
      returning pathname, reason`,
    [Math.max(0, Math.floor(olderThanMs)), max, token, CLAIM_STALE_MS],
  );
  const claimed = rows.map((r) => r.pathname);
  const vacated = new Set(rows.filter((r) => r.reason === 'moved_token').map((r) => r.pathname));
  ...
  try {
    ...
    for (const pathname of claimed) {
      ...
      summary.checked++;
      if (referenced.has(pathname) && !vacated.has(pathname)) {
      ...
      const outcome = await withTransaction(async (client) => {
        ...
        const { rows: mine } = await client.query(
          'select reason, created_at from media_uploads where pathname = $1 and claim_token = $2',
          [pathname, token],
        );
        if (!mine.length) return 'skipped';
        if (mine[0].reason === 'moved_token') {
          // The original path of moved (or missing) evidence: re-checked
          // under the lock, and never a reference check -- see above.
          const { rows: ev } = await client.query(
            'select 1 from media_preservations where pathname = $1 and (evidence_pathname is not null or missing_at is not null)',
            [pathname],
          );
          if (!ev.length) return 'preserved';
        } else {
        ...
        // A row younger than the token TTL (a removal of a file uploaded
        // minutes ago) may still have a live upload token behind it, and a
        // token can re-create the file once it is gone. Such a row is
        // re-recorded as 'token' with its clock restarted rather than
        // dropped, so a later sweep -- after any token has expired -- deletes
        // whatever was re-uploaded. An older row cannot: every row is
        // (re)stamped when its token is issued or its file is deleted.
        if (tokenMayBeLive(mine[0].created_at)) {
          await client.query(
            // A vacated evidence path stays 'moved_token' (plain 'token' rows
            // at a preserved path are never claimed).
            `update media_uploads
                set reason = case when reason = 'moved_token' then 'moved_token' else 'token' end,
                    created_at = now(), claimed_at = null, claim_token = null
              where pathname = $1 and claim_token = $2`,
            [pathname, token],
          );
        } else {
      ...
      if (outcome === 'failed') {
        summary.failed++;
        // A vacated evidence path is re-queued as 'moved_token', never as
        // 'delete_failed': the claim above only takes a preserved path while
        // its reason is 'moved_token', so a 'delete_failed' row there would
        // never be retried (and a re-created original would never be deleted).
        await recordPendingMediaPath(pathname, vacated.has(pathname) ? 'moved_token' : 'delete_failed');
        continue;
      }
      if (outcome === 'held') {
