...
export async function accountStanding(userId, client, { lock = false } = {}) {
  ...
  const { rows: u } = await c.query('select id, data from users where id = $1', [String(userId)]);
  if (!u.length) return { user: null, creator: null, effectiveStatus: null };
  const user = { ...u[0].data, id: u[0].id };
  if (user.role !== 'creator' || !user.creatorId) return { user, creator: null, effectiveStatus: null };
...
export function canReceiveStanding(standing) {
  return !!standing?.creator && standing.effectiveStatus === 'active' && standing.creator.seed !== true;
}
...
export function normalizePayoutWallet(wallet) {
  ...
  if (!raw || !isAddress(raw, { strict: false })) {
    throw Object.assign(
      new Error('Your saved payout wallet isn’t a valid wallet address (0x followed by 40 hex characters). Fix it in your profile first.'),
  ...
  for (const row of rows) {
    ...
    out.push({
      ...
      account: {
        ...
        seed: standing.creator?.seed === true,
      },
