...
export async function accountStanding(userId, client, { lock = false } = {}) {
  ...
  // Account-level moderation can also sit on a creator ACCOUNT whose profile
  // is not approved (lib/users-store.js setUserModeration); the stricter of
  // the two standings wins, so a suspended applicant's credits freeze too.
  let effectiveStatus = creator ? effectiveCreatorStatus(creator) : null;
  const userStatus = effectiveUserStatus(user);
  if (userStatus === 'banned') effectiveStatus = 'banned';
  else if (userStatus === 'suspended' && effectiveStatus !== 'banned') effectiveStatus = 'suspended';
  return { user, creator, effectiveStatus };
}
...
/**
 * Whether a creator's SAVED payout wallet can actually be paid (round-20
 * dashboard#0): { saved, payable, error }. The dashboard no longer blocks
 * Save Profile on an unchanged legacy wallet that fails today's checksum rule
 * (round 19), so it needs this to warn and to hide Cash Out -- the payout
 * request below would refuse that wallet anyway (normalizePayoutWallet).
 * Pure; the same rule as normalizePayoutWallet.
 */
export function payoutWalletStatus(wallet) {
  const raw = typeof wallet === 'string' ? wallet.trim() : '';
  if (!raw) return { saved: false, payable: false, error: null };
  const error = payoutWalletError(raw);
  return { saved: true, payable: !error, error: error || null };
}
  ...
  for (const row of rows) {
    ...
    out.push({
      ...
      account: {
        ...
        // The login no longer exists (a force-deleted account). Rejecting its
        // pending payout forfeits the credits instead of refunding them
        // (rejectPayout, round-18 money#3): the panel should offer "Reject
        // (forfeit)", not "Reject & refund".
        deleted: !standing.user,
      },
 ...
 *
 * A DELETED account (force-deleted with the request still pending) gets no
 * refund (round-18 money#3): crediting it created a balance row for a login
 * that no longer exists -- a liability owed to nobody, outside the
 * account-deletion forfeiture record. The request is still marked rejected,
 * and the reversal is recorded as forfeited: a 'payout_reversed' +amount and
 * an 'account_deleted' -amount ledger pair (net zero, the same type
 * deleteFanAccount writes for a forfeited balance), with no balance row
 * touched and no notification. The returned request carries
 * `forfeited: true`. The users row is locked FOR SHARE before the request, so
 * a deletion in flight is waited for (the users -> payout order deletion
 * itself takes).
 */
export async function rejectPayout(id, reason) {
  const why = typeof reason === 'string' ? sliceText(reason.trim(), 500) : '';
  if (!why) throw new Error('A reason is required to reject a payout');
  const request = await withTransaction(async (client) => {
    const { rows: owner } = await client.query('select user_id from payout_requests where id = $1', [id]);
    let accountExists = false;
    if (owner.length) {
      const { rowCount } = await client.query('select 1 from users where id = $1 for share', [String(owner[0].user_id)]);
      accountExists = rowCount > 0;
    }
    const { rows } = await client.query(
    ...
    if (!accountExists) {
      const meta = JSON.stringify({ payoutRequestId: r.id, reason: why, accountDeleted: true });
      await client.query(
        `insert into credit_ledger (user_id, type, amount_cents, meta)
         values ($1, 'payout_reversed', $2, $3), ($1, 'account_deleted', $4, $5)`,
        [String(r.user_id), cents, meta, -cents,
          JSON.stringify({ payoutRequestId: r.id, reason: 'account deleted; rejected payout forfeited' })],
      );
  ...
  if (request.forfeited) return request;
  await createNotification({
    userId: request.user_id,
