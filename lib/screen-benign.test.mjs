...
for (const t of ['Zero tolerance for child pornography', 'All content 18+. We have zero tolerance for child pornography.',
  ...
  'fighting child sex slavery', 'Stop child sex tourism', 'we report child sexual abuse material',
...
// Round 22: "child sex rings are real" was a row here. Under the decided
// child-safety principle (a child lead and a sexual word anywhere, refused)
// it moved to refuse in lib/screen-generated.test.mjs: it has no advocacy
// lead ("fighting", "stop", "end"...), and "child sex rings dm me for links"
// is the same shape with a sale.
...
// --- 13. Round 22: a parent's little one, item counts, TG prices, advocacy ---
const LITTLE_ONES = ['little girl', 'little boy', 'lil girl', 'lil boy', 'Little girl', 'little girls', 'little boys'];
const LITTLE_ENDS = [' just turned 5!', ' is sick today', ' starts school tomorrow', ' was sick', ' says hi', ' lost a tooth', ' is napping',
  ' drew this for me', '', '!', ' 🎂', ' is at grandma’s', ' has the flu', ' woke me up'];
for (const owner of ['my', 'My', 'our', 'Our', 'his', 'her', 'their', 'Mommy of a', 'mom of a', 'Mom to a']) {
  for (const one of LITTLE_ONES) {
    for (const end of LITTLE_ENDS) {
      add('parenting: my little girl (r22)', `${owner} ${one}${end}`);
      add('parenting: my little girl (r22)', `${pick(['Sorry late reply, ', 'Happy birthday to ', 'Omg ', 'Brb, ', ''])}${owner.toLowerCase()} ${one}${end}`);
    }
  }
}
const COUNT_LEADS = ['Set of', 'Pack of', 'Bundle of', 'Total of', 'Includes', 'Unlock', 'You get', 'Collection of', 'Contains', 'x'];
const COUNT_WORDS = ['nudes', 'porn videos', 'sexy nudes', 'hot pics', 'exclusive nudes', 'explicit pics'];
for (const lead of COUNT_LEADS) {
  for (let n = 10; n <= 17; n++) {
    for (const w of COUNT_WORDS) add('listing: item counts (r22)', `${lead}${lead === 'x' ? '' : ' '}${n} ${w}${pick(['', ' 💦', '!', ' for $20', ', instant unlock'])}`);
  }
}
for (let n = 10; n <= 17; n++) {
  for (const tail of [' and 3 videos', ' + 2 clips', ', 3 vids', ' & 3 vids', ' and 2 gifs', ' + 4 pics']) {
    for (const w of ['nudes', 'Nudes', 'sexy nudes', 'hot pics']) add('listing: item counts (r22)', `${pick(['', 'Includes ', 'You get ', 'Bundle: '])}${n} ${w}${tail}`);
  }
}
for (const p of ['5', '10', '15', '20', '25', '40']) {
  for (const lead of ['TG', 'Hot TG', 'New TG', 'TG:', 'TG,', 'tg']) {
    for (const tail of ['', ' each', ' today only', ' only', ' menu', ' this week', ' for customs', ' for content', '!']) {
      add('trans: TG category price (r22)', `${lead} $${p}${tail}`);
    }
  }
}
for (const lead of ['fighting', 'Fighting', 'Stop', 'stop', 'End', 'end', 'Against', 'Anti', 'Survivor of', 'Report']) {
  for (const noun of ['sex slavery', 'sex tourism', 'sex trade', 'sex work exploitation', 'sex trafficking', 'sex abuse', 'sexual abuse material']) {
    for (const tail of ['', ' now', '.', ' — donate at the link in my bio', ' 🙏', ' everywhere']) add('safety: advocacy (r22)', `${lead} child ${noun}${tail}`);
  }
}
