...
section('R4B fix-up: evidence is never lost to ordering or a stale request');
{
  ...
  // Round 7 (R7B media#0): the file the sweep DID delete keeps one row,
  // re-recorded as 'token' with its clock restarted, so a later sweep (after
  // any live upload token for it has expired) deletes a token-replayed
  // re-upload. The preserved files are off the queue.
  {
    const left = (await query('select pathname, reason from media_uploads')).rows;
    check('...and are off the queue', left.length === 1 && left[0].pathname === a && left[0].reason === 'token', JSON.stringify(left));
  }
