...
const { draftFrom, fieldsFromDraft } = await import('../components/admin/creatorDraft.js');
...
async function addRecord(creatorId) {
  await query('insert into performer_records (data) values ($1)', [{ creatorId: String(creatorId), status: 'active', aliases: [] }]);
}
