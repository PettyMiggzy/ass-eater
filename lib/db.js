...
-- Claims a real on-chain payment before any order rows are written from it.
-- One crypto payment can (and usually does) cover several cart items, so
-- the orders table itself can't carry a unique-per-hash constraint -- several
-- rows are SUPPOSED to share one hash. What must never happen is the SAME
-- hash being spent twice across two separate checkout submissions (replaying
-- a real payment to mint a second batch of free orders). The insert into
-- this table happens first, inside the same transaction as creating the
-- orders (see lib/orders-store.js createOrdersFromPayment) -- a duplicate
-- hash fails the insert on the primary key and the whole transaction rolls
-- back, so there is no window between "check if used" and "mark used" for
-- two concurrent submissions of the same hash to both slip through.
