...
-- 28 C.F.R. 75.4: a record is kept seven years from when it was CREATED or
-- last AMENDED, not from the content's production date (round-20
-- legal-journeys#0; lib/performer-records-store.js retainUntil). Rows written
-- before that fix carried producedAt + 7 years, which for back-catalogue
-- content could already be in the past: raised to the later of what they hold
-- and their last change + 7 years. Never lowered.
update performer_records
   set data = data || jsonb_build_object('retainUntil', to_char(
         greatest(
           coalesce(case when data->>'retainUntil' ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' then (data->>'retainUntil')::timestamptz end, '-infinity'::timestamptz),
           greatest(created_at, updated_at) + interval '7 years'
         ) at time zone 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'))
 where coalesce(case when data->>'retainUntil' ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' then (data->>'retainUntil')::timestamptz end, '-infinity'::timestamptz)
       < greatest(created_at, updated_at) + interval '7 years';
...
-- Round 18 (money#1/#2, public-pages#0, legal-journeys#0): a tracking number
-- is the carrier reference the SELLER entered, not the buyer's personal data,
-- so an erasure now removes only the shipping name and address. The round-17
-- seller-only copy (sellerTrackingNumber, written when a buyer erased) is
-- folded back into trackingNumber -- dropping the legacy trackingErasedAt
-- marker with it, since the number is back -- and the key is removed, so
-- buyer and seller see the same order. Idempotent: only rows still holding
-- the key match.
update orders
   set data = case
                when coalesce(data->>'sellerTrackingNumber', '') <> ''
                 and (data->'trackingNumber' is null or data->'trackingNumber' = 'null'::jsonb)
                then (data - 'sellerTrackingNumber' - 'trackingErasedAt')
                     || jsonb_build_object('trackingNumber', data->'sellerTrackingNumber')
                else data - 'sellerTrackingNumber'
              end
 where data ? 'sellerTrackingNumber';
...
/**
 * withTransaction, run once more if Postgres aborts it as a deadlock victim
 * (SQLSTATE 40P01). Only for transactions whose closure starts from nothing
 * but the database (it is simply run again from the top). A backstop for
 * lock orders that cannot all be made to agree -- account deletion against
 * an admin resolving a report of that account (round-18 media#0 /
 * social#0) -- not a substitute for taking locks in one order.
 */
export async function withTransactionRetryOnDeadlock(fn, { retries = 1 } = {}) {
  for (let attempt = 0; ; attempt += 1) {
    try {
      return await withTransaction(fn);
    } catch (err) {
      if (err?.code !== '40P01' || attempt >= retries) throw err;
    }
  }
}
