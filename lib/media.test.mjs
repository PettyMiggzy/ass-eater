...
section('media route entitlement');
{
  ...
  // Round 17 (media#1): only an upload IN PROGRESS (its 'token' row from
  // /api/media/upload-token) -- a file off the record with no such row (a
  // taken-down file waiting on a hold or the sweep) is not.
  check('an unreferenced file with no upload row is not served to its owner', !served(await fetchMedia(orphan, { cookie: cookieFor(ownerUser) })));
  await media.recordPendingMediaPath(orphan, 'token');
  check('unreferenced upload (in progress) is served to its owner', served(await fetchMedia(orphan, { cookie: cookieFor(ownerUser) })));
