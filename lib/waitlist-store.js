...
/**
 * `source` arrives from a public, unauthenticated form, so it is stored only
 * when it looks like the page slugs WaitlistForm actually sends ('landing',
 * 'blocked-region', 'founding-creator', ...): lowercase letters, digits and
 * hyphens, starting with a letter or digit. Anything else becomes 'unknown'.
 * Free text here ended up in the admin CSV export, where a value starting
 * with '=' is a spreadsheet formula.
 */
const SOURCE_RE = /^[a-z0-9][a-z0-9-]{0,59}$/;

export function normalizeWaitlistSource(source) {
  const value = typeof source === 'string' ? source.trim().toLowerCase() : '';
  return SOURCE_RE.test(value) ? value : 'unknown';
}

/**
 * Spreadsheet formula-injection guard for anything exported to CSV: Excel
 * and Google Sheets evaluate a cell that starts with = + - @ (or a tab/CR
 * that some importers strip first), quotes or not. A leading single quote
 * makes the cell plain text. Full-width variants are included because
 * Sheets normalises them.
 */
