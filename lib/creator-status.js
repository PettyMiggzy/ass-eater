...
import { sanitizeCategories } from './categories';
import { foldLookalikeLetters } from './payment-circumvention-filter';
import { sliceText } from './unicode-text';

// Re-exported so server code that already imports its sanitisers from here
// (or from lib/creators-store.js, which re-exports this module) finds it too.
export { sanitizeCategories };
...
//
// `categories` (lib/categories.js) is public -- it is what the browse
// sidebars filter on -- but is projected below as the SANITISED list rather
// than copied, so a legacy or hand-edited value never reaches a page as
// anything but known keys.
 ...
 * - The platform's own seed/demo creators (isDemoCreator: seed OR demo) are
 *   AI-generated imagery, so every
...
export function toPublicCreator(creator, { viewerMayUnlock = false } = {}) {
  ...
  pub.categories = sanitizeCategories(creator.categories);
  const gallery = Array.isArray(creator.gallery) ? creator.gallery : [];
  // Seed OR admin-marked demo: both are the platform's own AI-generated
  // sample imagery (isDemoCreator is the one definition, hoisted from below).
  const forceAi = isDemoCreator(creator);
  ...
  pub.gallery = gallery
    ...
    .map((item) => {
      const aiGenerated = forceAi || !!item.aiGenerated;
 ...
 */
export function toPublicListing(listing, creator = null) {
  if (!listing) return listing;
  const pub = { ...listing };
  for (const key of PRIVATE_LISTING_FIELDS) delete pub[key];
  // A demo listing (its own flag, or its creator's -- pass the creator) is the
  // platform's own AI-generated sample, labelled AI whatever the stored row
  // says, the same rule toPublicCreator applies to a demo gallery.
  const forceAi = isDemoListing(listing, creator);
  if (forceAi) pub.aiGenerated = true;
  const media = Array.isArray(listing.media) ? listing.media : [];
  pub.media = media
    ...
    .map((m) => ({
      ...
      aiGenerated: forceAi || !!(m.aiGenerated || listing.aiGenerated),
    }));
...
// What the four platforms allow in a username: letters, digits, "_", "." and
// (Reddit) "-". Anything else -- a space, an emoji, "$", "/", a "@" in the
// middle -- is not a handle on any of them, so it can only be decoration or a
// second message riding in the link field. Round 13: handles are screened in
// name mode (lib/prohibited-terms.js), so the charset is what keeps that
// field a single name token.
const SOCIAL_HANDLE_RE = /^[A-Za-z0-9._-]{1,50}$/;

// A handle as it will be stored: a pasted profile URL ("https://instagram.com/
// jess?igsh=..", "tiktok.com/@jess", "reddit.com/u/jess") is reduced to the
// bare name. Only a URL with a scheme, or a bare platform host, is stripped,
// so a handle with a dot in it ("jess.xo") is never read as a domain.
function bareSocialHandle(raw) {
  return raw
    .trim()
    .replace(/^(?:https?:\/\/[^/]+|(?:www\.|m\.|old\.|mobile\.)?(?:twitter|x|instagram|tiktok|reddit)\.com)\/+/i, '')
    .replace(/[?#].*$/, '')
    .replace(/^\/?u(?:ser)?\//i, '')
    .replace(/^@/, '')
    .replace(/\/+$/, '');
...
// The only website form stored: an https URL with no whitespace.
const SOCIAL_WEBSITE_RE = /^https:\/\/[^\s]+$/i;
export const SOCIAL_WEBSITE_MAX = 200;

/**
 * The social handles in `input` that are not a valid username, as
 * `[{ key, message }]` (empty when every present handle is fine). The profile
 * routes refuse these with a 400 naming the field; sanitizeSocials drops them.
 * A non-empty website that is not an https URL is listed too (key
 * 'website'): sanitizeSocials used to drop "jess.com" or "http://jess.com"
 * without a word and the save answered 200, taking any stored link with it
 * (round-14 accounts#3). So is one longer than SOCIAL_WEBSITE_MAX, which
 * sanitizeSocials used to truncate into a broken link (round-15 accounts#3).
 */
export function invalidSocialHandles(input) {
  const out = [];
  if (!input || typeof input !== 'object') return out;
  for (const key of SOCIAL_HANDLE_KEYS) {
    const raw = input[key];
    if (typeof raw !== 'string') continue;
    const handle = bareSocialHandle(raw);
    if (handle && !SOCIAL_HANDLE_RE.test(handle)) {
      out.push({ key, message: `${key} link: use just your username -- letters, numbers, ".", "_" or "-" (up to 50).` });
    }
  }
  if (typeof input.website === 'string') {
    const site = input.website.trim();
    if (site && !SOCIAL_WEBSITE_RE.test(site)) {
      out.push({ key: 'website', message: 'Website must be a full link starting with https:// (for example https://example.com).' });
    } else if (site.length > SOCIAL_WEBSITE_MAX) {
      // Refused rather than cut: sanitizeSocials used to slice a longer link
      // to 200 characters and the save answered 200, publishing a truncated
      // URL that 404s (round-15 accounts#3).
      out.push({ key: 'website', message: `Website link is too long (${SOCIAL_WEBSITE_MAX} characters at most). Use a shorter link.` });
    }
  }
  return out;
}
...
// or a "javascript:" string can never end up as a raw href. A handle outside
// the platforms' own charset is dropped (invalidSocialHandles is how the
// routes refuse it with a message instead).
export function sanitizeSocials(input) {
  ...
  for (const key of SOCIAL_HANDLE_KEYS) {
    ...
    if (typeof raw !== 'string') continue;
    const handle = bareSocialHandle(raw);
    if (handle && SOCIAL_HANDLE_RE.test(handle)) out[key] = handle;
  }
  if (typeof input.website === 'string') {
    const site = input.website.trim();
    // The slice is only a defensive bound: the routes refuse a longer link
    // first (invalidSocialHandles), so nothing valid is ever cut here.
    if (SOCIAL_WEBSITE_RE.test(site)) out.website = sliceText(site, SOCIAL_WEBSITE_MAX);
  }
  return out;
}

/**
 * sanitizeSocials for a save that may echo stored values back: the routes
 * post every field on every save, so a stored handle from before the charset
 * rule would otherwise vanish, without a word, the next time the creator
 * changes something unrelated (a payout wallet, say). A key whose raw input
 * equals its stored value verbatim keeps that stored value; any changed key
 * must pass the charset (the routes refuse it first via invalidSocialHandles).
 */
export function sanitizeSocialsKeepingLegacy(input, stored) {
  const out = sanitizeSocials(input);
  if (!input || typeof input !== 'object' || !stored || typeof stored !== 'object') return out;
  for (const key of SOCIAL_HANDLE_KEYS) {
    const raw = input[key];
    if (key in out || typeof raw !== 'string' || typeof stored[key] !== 'string' || !stored[key]) continue;
    if (raw === stored[key]) out[key] = stored[key];
  }
  return out;
}
...
// Also the refusal limit in lib/field-validation.js FIELD_LIMITS.location, so
// an over-long location is refused rather than silently cut.
export const MAX_LOCATION_LENGTH = 60;

export function sanitizeLocation(input) {
  return sliceText(String(input ?? '').replace(/\s+/g, ' ').trim(), MAX_LOCATION_LENGTH).trim();
}
...
export function sanitizeTags(input) {
  ...
  for (const item of raw) {
    ...
    // Small capitals and dotless letters are stored as the plain word the
    // screens judged them as ("ᴄᴏsᴘʟᴀʏ" -> "cosplay"), so what is published is
    // exactly what was screened, and a tag search finds it.
    // Cut with sliceText, never .slice: a "fancy font" letter is two UTF-16
    // units, and half of one fails the jsonb write (round-10 accounts#0).
    const tag = sliceText(
      foldLookalikeLetters(item.toLowerCase())
        .replace(/[^\p{L}\p{N} -]+/gu, '')
        .replace(/\s+/g, ' ')
        .trim(),
      MAX_TAG_LENGTH,
