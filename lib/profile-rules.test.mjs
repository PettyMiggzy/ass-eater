...
async function addRecord(creatorId, status = 'active') {
  await query('insert into performer_records (data) values ($1)', [{ creatorId: String(creatorId), status, aliases: [] }]);
}
...
{
  let res = await call(signup, req({ body: { email: 'c1@example.com', password: 'secret123', role: 'creator', displayName: 'C One', handle: 'cone', bio: 'cashapp $lexi for customs, 10% cheaper' } }));
  check('flagged signup bio refused', res.statusCode === 400, JSON.stringify(res.body));
  res = await call(signup, req({ body: { email: 'venmo-luna22', password: 'secret123', role: 'fan' } }));
  check('flagged fan username refused', res.statusCode === 400, JSON.stringify(res.body));
  res = await call(signup, req({ body: { email: '6175551234', password: 'secret123', role: 'fan' } }));
  check('phone-number username refused', res.statusCode === 400);
  res = await call(signup, req({ body: { email: 'x'.repeat(41), password: 'secret123', role: 'fan' } }));
  check('over-long username refused', res.statusCode === 400);
  res = await call(signup, req({ body: { email: 'goodfan_1', password: 'secret123', role: 'fan' } }));
  ...
  res = await call(signup, req({ body: { email: 'c2@example.com', password: 'secret123', role: 'creator', displayName: 'C Two', handle: 'ctwo', bio: '' } }));
  ...
  res = await call(signup, req({ body: { email: 'c3@example.com', password: 'secret123', role: 'creator', displayName: 'C Three', handle: '@CTWO', bio: '' } }));
  ...
  res = await call(signup, req({ body: { email: 'reffan', password: 'secret123', role: 'fan', ref: '@@ctwo' } }));
  ...
  res = await call(signup, req({ body: { email: 'c4@example.com', password: 'secret123', role: 'creator', displayName: 'C Four', handle: 'cfour', bio: '' } }));
