...
export async function purgeUserContent(client, userId) {
  ...
  // A DM takedown recorded on a TAKE IT DOWN request keeps a copy of the
  // message with the thread's participant list (lib/content-takedown.js), and
  // that list names this account. It becomes null, as it does in a report's
  // copy below (round-20 media#0 / social#0). FIRST, before the conversations
  // are locked: every path that holds both a takedown request and a
  // conversation takes the request row first (lib/content-takedown.js
  // takeDownContent: ncii_reports -> conversations), and creator deletion
  // reaches here holding the creator row, which also comes before a request
  // row everywhere (lib/ncii-reports-store.js). The conversation id on the
  // entry is kept -- it is the stored audit record -- and no admin response
  // returns it (adminNciiReportView). This UPDATE only reaches rows that
  // already name the account when it starts; a takedown committed while this
  // deletion is in flight is covered at READ time instead (round-21 media#0:
  // adminNciiReportsView nulls every participant that is not a live user).
  await client.query(
    `update ncii_reports
        set data = jsonb_set(data, '{takedowns}', (
              select coalesce(jsonb_agg(
                       case when jsonb_typeof(t->'snapshot'->'participantIds') = 'array'
                            then jsonb_set(t, '{snapshot,participantIds}', (
                                   select coalesce(jsonb_agg(case when p #>> '{}' = $1 then 'null'::jsonb else p end order by ord), '[]'::jsonb)
                                     from jsonb_array_elements(t->'snapshot'->'participantIds') with ordinality as x(p, ord)))
                            else t end
                       order by tord), '[]'::jsonb)
                from jsonb_array_elements(data->'takedowns') with ordinality as y(t, tord)))
      where jsonb_typeof(data->'takedowns') = 'array'
        and exists (
              select 1 from jsonb_array_elements(data->'takedowns') as t2
               where jsonb_typeof(t2->'snapshot'->'participantIds') = 'array'
                 and exists (select 1 from jsonb_array_elements(t2->'snapshot'->'participantIds') as p2 where p2 #>> '{}' = $1))`,
    [uid],
  );
  ...
  // takedown addresses. A takedown of a message in such a thread stores the
  // conversation id on its audit row and its request entry, but no admin
  // response returns it (lib/ncii-reports-store.js adminTakedownEntryView,
  // round-20 media#0) and the copy's participant list reads null for them.
  await client.query(
    `update reports
