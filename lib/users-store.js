...
export async function purgeUserContent(client, userId) {
  ...
  // deadlocked. Now both take conversations before reports. Creator deletion
  // (lib/creators-store.js) already holds users -> creators before calling
  // this, and takes no conversation lock of its own earlier, so nothing is
  // inverted there.
  await client.query(
  ...
  // break moderation of the very message reported. It is an internal id:
  // /api/admin/reports drops it from every report it returns, and shows a
  // gone participant as null (lib/reports-store.js attachReportTargets,
  // round-18 social#1).
  await client.query(
    `update reports
