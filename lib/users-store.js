...
export async function setUserModeration(userId, { status, until = null, reason = null, by = 'admin' } = {}) {
  ...
  return withTransaction(async (client) => {
    ...
    if (user.creatorId) {
      ...
      await enqueueStandingPushes([{ uid: user.id, status: combined, role: 'CREATOR', suspendedUntil: combined === 'suspended' ? latestLapse(accountStatus === 'suspended' ? user.moderationUntil : null, creatorStatus === 'suspended' ? cr[0]?.data?.suspendedUntil : null) : null }], client);
    } else {
 ...
 * and moderation records are not touched. Shared by fan self-deletion,
