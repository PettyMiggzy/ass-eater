...
import { USER_MODERATION_STATUSES } from './user-moderation';
 ...
 * account already holds is dead the moment this commits. Refused
 * (USER_MODERATION_NOT_ALLOWED) for a creator account: a creator's standing
 * is its creator record. Returns the updated user, or null if no such user.
 */
export async function setUserModeration(userId, { status, until = null, reason = null, by = 'admin' } = {}) {
  ...
  if (current.role === 'creator' || current.creatorId) {
    throw Object.assign(new Error("A creator account's standing is set on its creator profile."), { code: USER_MODERATION_NOT_ALLOWED });
  }
  const patch = {
