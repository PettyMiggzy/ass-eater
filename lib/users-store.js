...
export async function deleteFanAccount(userId, { force = false, strict = false, selfService = false, expectedForfeit = null } = {}) {
  ...
  return withTransaction(async (client) => {
    ...
    // every one that has already shipped loses the address now. An unshipped
    // one keeps it: the creator still has to ship it (self-service refuses
    // above while there are any; an admin-forced deletion leaves them so the
    // item can still be sent).
