 ...
 * and moderation records are not touched -- except that the shipping name,
 * address and tracking numbers on every order this login BOUGHT that has
 * already shipped are erased (Privacy section 7: deleting the account is the
 ...
 * the moment it ships (lib/orders-store.js markOrderShipped). Shared by fan self-deletion,
...
export async function deleteFanAccount(userId, { force = false, strict = false, selfService = false, expectedForfeit = null } = {}) {
  ...
  return withTransaction(async (client) => {
    ...
    // markOrderShipped erases it at ship time once the buyer is gone).
