...
async function manifestIsGenuinelyAbsent() {
  try {
    ...
    return err?.name === 'BlobNotFoundError' || /not ?found/i.test(String(err?.message || ''));
  }
}
...
export async function bumpSessionVersion(userId, expectedVersion) {
  return updateJsonList(MANIFEST_PATH, (users) => {
    ...
    if (idx === -1) {
      ...
      // whatever the transform returns, and its read silently degrades to
      // the empty fallback on ANY storage failure -- not just on a manifest
      // that doesn't exist yet. Returning `users` would then write `[]` over
      // data/users.json and delete every account on the platform, with the
      // ETag precondition happily matching because nobody else wrote. The
      // caller reports this instead of claiming a revocation that didn't
      // happen.
