...
section('who may message whom');
{
  ...
  check('a pending creator account messaging a creator pays like a fan (no free creator<->creator)',
    await expectCode(() => msgs.sendDirectMessage({ sender: { id: 'upend', role: 'creator', creatorId: 'cpend' }, recipientId: 'ub', text: 'hi' }), credits.INSUFFICIENT_BALANCE) === true);
  ...
  check('a seed/demo creator account cannot message for free either',
    await expectCode(() => msgs.sendDirectMessage({ sender: { id: 'useed', role: 'creator', creatorId: 'cseed' }, recipientId: 'ub', text: 'hi' }), credits.INSUFFICIENT_BALANCE) === true);
  ...
  const r1 = await msgs.sendDirectMessage({ sender: fan, recipientId: 'ua', text: 'hello', clientMessageId: 'k1' });
  ...
  const again = await msgs.sendDirectMessage({ sender: fan, recipientId: 'ua', text: 'hello', clientMessageId: 'k1' });
  ...
  const r2 = await msgs.sendDirectMessage({ sender: fan, recipientId: 'ub', text: 'hi b' });
  ...
  check('not enough credits is refused',
    await expectCode(() => msgs.sendDirectMessage({ sender: fan, recipientId: 'ua', text: 'pricey' }), credits.INSUFFICIENT_BALANCE) === true);
...
section('concurrent paid sends');
{
  ...
  const results = await Promise.allSettled(Array.from({ length: 10 }, (_, i) =>
    msgs.sendDirectMessage({ sender: fz, recipientId: 'uz', text: `m${i}` })));
