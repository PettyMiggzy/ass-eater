...
section('R10B fix-up: parallel bursts cannot under-count the login brakes');
{
  ...
  const netMarker = loginGuard.loginGuardKeys({ identifier: 'mark48@r10b.test', ip: '2001:db8:dd:1::/64', net: '2001:db8:dd::/48' }).accountFromNetKey;
  const fromnet = await loginGuard.readLoginCounters([netMarker]);
  check('a parallel burst from one /64 adds 1 (not 3) to the /48 dirty count',
    fromnet[netMarker] === 1, JSON.stringify(fromnet));
}
