...
// ---------------------------------------------------------------------------
section('gates-token#0: the /48 budget brakes only a dirty caller');
{
  await reset();
  const owner = await mkFan('owner48@r10b.test');
  // A carrier /48 whose failure budget someone else has spent.
  ...
  check('a clean /64 of a saturated /48 still logs in with the right password', good.statusCode === 200, `${good.statusCode} ${JSON.stringify(good.body)}`);
  const wrong = await call(loginRoute, { body: { email: 'someone@r10b.test', password: 'nope' }, ip: '2001:db8:aa:2::1' });
  check('...and a clean /64 with a wrong password gets its answer (401, not 429)', wrong.statusCode === 401, String(wrong.statusCode));
  const again = await call(loginRoute, { body: { email: 'else@r10b.test', password: 'nope' }, ip: '2001:db8:aa:2::9' });
  check('...but that /64, now dirty, is braked by the spent /48', again.statusCode === 429, String(again.statusCode));
