...
import { wallBlockedAuthors } from './messages-store';
import { sliceText, hasControlChars } from './unicode-text';
 ...
 * FROM THE WALL, as a Map of post id -> boolean. Read from wall_blocks only
 * (lib/messages-store.js wallBlockedAuthors), the state pages/api/wall/block.js
 * sets -- never from DM blocks. A DM block is made by user id, so flagging
 * comments by it let an owner block a known account from the inbox and watch
 * which anonymous comments flipped (round-10 social#0). Without this the wall
 * showed "Block" again after every reload, so a block could not be seen or
 * lifted from the wall. Pass the result to toPublicWallPost as
...
export async function wallBlockFlagsFor(ownerUserId, posts) {
  ...
  const blocked = await wallBlockedAuthors(String(ownerUserId), posts.map((p) => p.authorId).filter((a) => a != null));
 ...
 */
export const WALL_CONTROL_CHARS_MESSAGE = 'Your comment contains an invalid character (a control character). Remove it and try again.';

export async function addWallPost({ creatorId, authorId, authorName, text, isWallOwner = false, requireAuthor = false }) {
  const trimmed = String(text || '').trim();
  if (!trimmed) throw new Error('Comment cannot be empty');
  // Control characters are refused, as for DMs and the NCII form (round-13
  // social#0): each is a 6-byte JSON escape in every page that loads it.
  if (hasControlChars(trimmed)) {
    throw Object.assign(new Error(WALL_CONTROL_CHARS_MESSAGE), { code: 'WALL_CONTROL_CHARS' });
  }
  const entry = {
    creatorId,
    authorId,
    authorName: sliceText(authorName || 'Someone', 60),
    text: sliceText(trimmed, MAX_TEXT_LENGTH),
  ...
  // `requireAuthor` (every route that posts for a signed-in account, round-20
  // social#1): both paths first take the author's users row FOR KEY SHARE
  // (lib/author-lock.js), so a comment racing its author's account deletion
  // either lands before the purge (and is purged) or is refused with
  // AUTHOR_ACCOUNT_GONE -- never inserted after the purge ran, where it stayed
  // public for good.
  if (isWallOwner) {
    return withTransaction(async (client) => {
      if (requireAuthor) await lockAuthorRow(client, authorId);
 ...
 */
export async function deleteWallPost(rawId, requesterId, { isWallOwner, client: outer = null } = {}) {
  // Normalised here too (defense in depth): a non-string id must never reach
  // the bigint parameter raw -- pg serialises an array as '{"5"}' and the
  // cast throws (round-12 social#1).
  if (!/^[1-9]\d{0,17}$/.test(String(rawId ?? '')) || (typeof rawId !== 'string' && typeof rawId !== 'number')) {
    throw new Error('Comment not found');
  }
  const id = String(rawId);
  // With `client`, everything runs on the caller's transaction (an admin
  // takedown commits its audit copy together with the delete -- round-14
  // media#0), reading the row under a lock. The reports on this comment are
  // locked FIRST, the same reports -> wall_posts order the plain path takes
  // (its snapshot UPDATE on reports runs before the DELETE), so a takedown
  // racing an author delete of the same comment cannot deadlock.
  if (outer) {
    await outer.query(
  ...
  const { rows } = await (outer || { query }).query(`select id, data from wall_posts where id = $1${outer ? ' for update' : ''}`, [id]);
