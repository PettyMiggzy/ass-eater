...
for (const age of SINGLE_AGES) {
  ...
  for (const lead of ["i'm", 'im', 'I am', "she's", 'aged']) {
    // Round 21 (DECIDED): in free text a third-person lead ("she's 7", "she's
    // 7 years old.") is a parent talking about a child -- refused only in a
    // tag or a name, and everywhere with a strongly sexual continuation.
    const ctx = lead === "she's" ? NAME_LIKE : EVERY;
    addCase('refuse: single-digit age', 'refuse', ctx, `${lead} ${age}`);
    addCase('refuse: single-digit age', 'refuse', ctx, `${lead} ${age} years old.`);
...
for (const t of ['babysitter', 'kid friendly', 'Minor edits', 'baby oil', 'baby blue lingerie', 'kids menu', 'child care', 'no kids on set',
  ...
  'no kids, sex positive', 'baby porn star', 'Sweet baby, nudes dropping tonight', 'night night kids xxx',
  'love you kid xx']) {
...
for (const lead of ['kid', 'kids', 'child', 'children', 'toddler', 'toddlers']) {
  for (const w of ['Nudes', 'Porn', 'Sex', 'Naked', 'Pornography'].filter((x) => !(lead === 'child' && x === 'Porn'))) {
    for (const sep of ['. ', '! ', '? ', ' - ', ' — ', '\n', ' -\n', '.\n']) {
      // Round 22 (decided child-safety principle): a child lead and a sexual
      // word ANYWHERE in the text is refused, sentence break or not, so this
      // row moved from pass to refuse. The negated form below still passes.
      addCase('refuse: child word + sexual word, any sentence (r22)', 'refuse', FREE, `Busy with the ${lead}${sep}${w} drop friday`);
...
// --- ROUND 21 --------------------------------------------------------------
// Every round-21 rule as a GENERATOR family. The benign side of each lives in
// lib/screen-benign.test.mjs (>= 30,000 legitimate strings that must pass).

// accounts#0/#1: the single-digit age in free text counts only with an age
// word or a first-person lead-in; a third-person lead only with a strongly
// sexual continuation; the bare "N and horny" only opening the text or a
// sentence. Still refused, every context:
for (const n of ['1', '2', '3', '5', '7', '9']) {
  for (const w of ['horny', 'slutty', 'kinky', 'a slut', 'a whore']) {
    for (const lead of ["she's", "he's", 'she is', 'she just turned', 'just turned']) addCase('refuse: third-person + sexual (r21)', 'refuse', EVERY, `${lead} ${n} and ${w}`);
  }
  for (const w of ['horny', 'naked', 'a virgin', 'a slut']) {
    addCase('refuse: bare digit opening a sentence (r21)', 'refuse', EVERY, `${n} and ${w}`);
    addCase('refuse: bare digit opening a sentence (r21)', 'refuse', EVERY, `Hi. ${n} and ${w}`);
    addCase('refuse: bare digit opening a sentence (r21)', 'refuse', EVERY, `${n} & ${w} 😈`);
  }
  for (const lead of ["i'm", 'im', 'I am', 'aged', 'i just turned', 'I turned']) {
    addCase('refuse: first-person single digit (r21)', 'refuse', EVERY, `${lead} ${n}`);
    addCase('refuse: first-person single digit (r21)', 'refuse', EVERY, `${lead} ${n}!`);
  }
  for (const w of ['girl', 'boy', 'slut', 'pussy']) {
    addCase('refuse: no possessive (r21)', 'refuse', EVERY, `${n}yo ${w}`);
    addCase('refuse: no possessive (r21)', 'refuse', EVERY, `${n} year old ${w}`);
  }
  // a possessive does not launder a sexual word in the same clause
  for (const w of ['girl is horny', 'boy nudes', 'girl naked pics', 'girl pussy']) addCase('refuse: possessive + sexual (r21)', 'refuse', EVERY, `my ${n} year old ${w}`);
}
for (const n of ['13', '14', '15', '16']) {
  for (const t of [`my ${n} yo girl`, `my ${n} year old girl`, `my daughter is ${n} and horny`,
    `my ${n} year old son is horny`]) addCase('refuse: possessive, two digits (r21)', 'refuse', EVERY, t);
}

// accounts#2: "tg" handovers stay refused.
for (const t of ['pay me on tg', '$20 on tg', 'tg for cheaper', 'tg payments', 'tg: 69kitty', 'TG: jess_99', 'pay $20 via my tg', 'cheaper on my TG']) {
  addCase('refuse: tg handover (r21)', 'refuse', FREE, t);
}
// accounts#3: a handle after "line:" is still a handle.
for (const t of ['line: 88mia', 'line: jane_doe99', 'LINE: 23jess', 'line: 2jess', 'line - 69kitty']) addCase('refuse: line handle (r21)', 'refuse', FREE, t);

// accounts#4: promotional child pairs, and every child pair in a label.
for (const child of ['child', 'kiddie', 'kid', 'kids', 'children']) {
  for (const w of ['porn', 'pornography']) {
    for (const t of [`new ${child} ${w} here`, `${child} ${w}`, `hot ${child} ${w} dm me`, `${child} ${w} for sale`,
      `${child} ${w} never removed here`, `${child} ${w} not reported lol`]) addCase('refuse: child pair, no disclaimer (r21)', 'refuse', EVERY, t);
    for (const lead of ['Zero tolerance for', 'We report', 'Anti', 'No']) addCase('refuse: child pair in a label (r21)', 'refuse', NAME_LIKE, `${lead} ${child} ${w}`);
  }
}

// accounts#5: school pairs in a label, and spaced / glued in free text.
for (const school of ['high school', 'highschool', 'high-school', 'middle school']) {
  for (const w of ['girl', 'boy', 'porn', 'slut', 'nudes']) {
    addCase('refuse: school pair (r21)', 'refuse', EVERY, `${school} ${w}`);
    addCase('refuse: school pair across punctuation, labels (r21)', 'refuse', NAME_LIKE, `${school}, ${w}`);
  }
}
for (const t of ['school_girl', 'school.girl', 'school-girl', 'school girl', 'schoolgirl']) addCase('refuse: school pair (r21)', 'refuse', EVERY, t);

// accounts#6: under 18 / u18 / almost 18 / not 18 yet + a sexual word; almost
// / nearly legal.
for (const lead of ['under 18', 'under18', 'u18', 'U18', 'almost 18', 'under-18', 'just under 18', 'not 18 yet']) {
  for (const sep of ['', ' ', '_', '-']) {
    for (const w of ['porn', 'nudes', 'slut', 'horny', 'naked', 'pussy']) {
      if (sep === '' && /\s$/.test(lead)) continue;
      for (const tail of ['', ' 💦', ' dm me', ' here']) addCase('refuse: under 18 pair (r21)', 'refuse', EVERY, `${lead}${sep}${w}${tail}`);
    }
  }
  addCase('refuse: under 18 pair (r21)', 'refuse', EVERY, `${lead} and horny`);
}
for (const lead of ['almost legal', 'nearly legal', 'almostlegal', 'nearlylegal', 'almost-legal']) {
  for (const tail of ['', ' porn', ' nudes', ' slut', ' 💦']) addCase('refuse: almost/nearly legal (r21)', 'refuse', EVERY, `${lead}${tail}`);
}
for (const t of ["if you're under 18, leave", 'no one under 18', 'minors (under 18) not allowed', 'Under 18? Leave now.', 'under 18s not welcome',
  'Zero tolerance for under 18 porn']) addCase('pass: under 18 disclaimer (r21)', 'pass', FREE, t);

// accounts#7: sugar dating, every context.
for (const sep of [' ', '_', '-']) {
  for (const b of ['baby', 'babies']) {
    for (const sep2 of [' ', '_', '-']) for (const w of ['porn', 'porno']) addCase('pass: sugar baby (r21)', 'pass', EVERY, `sugar${sep}${b}${sep2}${w}`);
  }
}
for (const t of ['baby porn', 'babies porn', 'baby_porn', 'new baby porn']) addCase('refuse: baby porn (r21)', 'refuse', EVERY, t);

// accounts#8: UK/AU school years, -con forms, school classes.
for (const y of ['7', '8', '9', '10', '11']) {
  for (const w of ['girl', 'boy', 'slut', 'nudes', 'porn', 'pussy']) {
    addCase('refuse: school year (r21)', 'refuse', EVERY, `year ${y} ${w}`);
    addCase('refuse: school year (r21)', 'refuse', EVERY, `Year ${y} hot ${w}`);
    addCase('refuse: school year (r21)', 'refuse', NAME_LIKE, `year${y}${w}`);
  }
  for (const w of ['teacher', 'maths', 'exams', 'science', 'teaching assistant']) addCase('pass: school year (r21)', 'pass', FREE, `year ${y} ${w}`);
}
for (const t of ['toddlercon', 'toddlercon art', 'hot toddlercon']) addCase('refuse: -con forms (r21)', 'refuse', EVERY, t);
for (const t of ['freshman girl nudes', 'high school freshman nudes', 'sophomore slut', 'freshman porn', 'schoolkid nudes', 'schoolkids porn']) {
  addCase('refuse: school class (r21)', 'refuse', EVERY, t);
}
for (const t of ['college freshman here', 'I was a sophomore in 2012', 'freshman year was wild', 'my sophomore album']) addCase('pass: school class (r21)', 'pass', FREE, t);

// Round 21 fix-up (reviewer): a disclaimer word must not be a way round the
// child pairs. Every one of these passed after the first round-21 fix and was
// refused before it: a bare "stop" / "end" / "report" lead across sentence
// punctuation, the general disclaimer's "no." across a sentence, and an AFTER
// word reached through other words.
for (const t of ['Stop, child porn for sale dm me', 'End. child porn vids $20', 'stop! child porn for sale', 'Stop child porn for sale',
  'child porn, reported to be the best', 'child porn gets reported lol but here it is', 'child porn here, will be reported? nah',
  'kid nudes, dm before removed', 'minor nudes - get them before banned', 'minor nudes dm me, reported accounts get banned',
  'kids porn available, is banned elsewhere', 'kids porn is banned elsewhere, dm me', 'the fight against child porn? lol no. child porn here',
  'under 18 porn, reported lol', 'Report. child porn $20', 'ban? child porn here', 'no. child porn dm me']) {
  addCase('refuse: child pair, disclaimer bypass (r21 fix-up)', 'refuse', EVERY, t);
}
// ...a family exemption must not wave through a sale of a child (10-17 or not).
for (const t of ['my 13 year old daughter selling pics', 'my 12 year old daughter pics available',
  'my daughter just turned 13 wanna see her body', 'our 14 year old son vids $20',
  'my 5 year old girl pics for sale']) addCase('refuse: family age + commerce (r21 fix-up)', 'refuse', EVERY, t);
// ...a spaced dash is not a way round the school pairs, and a third-person
// single digit followed by "naked" is a self-description.
for (const t of ['high school - girl nudes', 'high school — girls naked', 'middle school - boy porn']) {
  addCase('refuse: school pair across a dash (r21 fix-up)', 'refuse', EVERY, t);
}
for (const t of ["she's 8 and naked", "he's 7 and nude"]) addCase('refuse: third-person single digit (r21 fix-up)', 'refuse', EVERY, t);

// --- ROUND 22 ---------------------------------------------------------------
// The decided CHILD-SAFETY PRINCIPLE: a minor signal (an age 1-17 with an age
// word; a family / child subject or a pronoun with an age; a child lead) AND a
// sexual or commerce word anywhere in the text is refused in every free
// context and in tags. The benign side is lib/screen-benign.test.mjs.
const TAG_FREE = [...FREE, 'tag'];
addCase('refuse: moved by the principle (r22)', 'refuse', FREE, 'my kid, naked truth podcast');
addCase('refuse: moved by the principle (r22)', 'refuse', FREE, 'child sex rings are real');

// accounts#0: family lead x of/to x a/an/my/'' x ages x age words x tails.
const FAMILY_LEADS_R22 = ['Daddy', 'daddy', 'Mommy', 'mommy', 'Mom', 'mom', 'Papa', 'papa', 'Dad', 'mother', 'father', 'parents', 'mama'];
const AGE_WORDS_R22 = [' year old', ' yo', 'yo', ' y/o', ' years old', ' yrs old', '-year-old'];
const SEX_TAILS_R22 = [' slut', ' girl', ' babe', ' porn star', ' and horny', ' girl nudes', ' slut 😈', ' boy, pics $20', ' girl who sells nudes'];
for (const lead of FAMILY_LEADS_R22) {
  for (const of of ['of', 'to']) {
    for (const det of ['a ', 'an ', 'my ', '']) {
      for (let n = 1; n <= 17; n++) {
        // A bare "girl" / "babe" is a sexual label only with a two-digit age
        // ("my 3 yo girl" is a parent's daughter; the principle needs a sexual
        // or commerce word for a single digit).
        const tails = n < 10 ? SEX_TAILS_R22.filter((t) => t !== ' girl' && t !== ' babe') : SEX_TAILS_R22;
        addCase('refuse: family lead + age word + sexual tail (r22)', 'refuse', TAG_FREE, `${lead} ${of} ${det}${n}${pick(AGE_WORDS_R22)}${pick(tails)}`);
      }
    }
  }
}
// ...a bare count stays a count (benign side: "parenting: mom of N").
for (const lead of ['Mom', 'mom', 'mother', 'Single mom', 'Dad']) {
  for (let n = 1; n <= 17; n++) addCase('pass: bare count (r22)', 'pass', FREE, `${lead} of ${n} and horny`);
}
// accounts#1: family subject x verb x age x separator x commerce tail.
const SUBJECTS_R22 = ['my daughter', 'My daughter', 'my son', 'our daughter', 'my niece', 'My nephew', 'my kid', 'my baby girl', 'She', 'he'];
const VERBS_R22 = [' just turned ', ' is ', ' turned ', ' is now ', ' will be '];
const SEPS_R22 = ['. ', '! ', ', ', ' - ', '; ', '\n', ' 🙈 '];
const COMMERCE_TAILS_R22 = ['nudes $20', 'pics of her for sale', 'wanna see her body?', 'dm for her pics', 'want to fuck her?', 'her pics $15',
  'content available, dm me', 'vids for sale'];
for (const subject of SUBJECTS_R22) {
  for (const verb of VERBS_R22) {
    for (const sep of SEPS_R22) {
      for (let n = 1; n <= 17; n += 4) addCase('refuse: family subject + age + commerce, any sentence (r22)', 'refuse', TAG_FREE, `${subject}${verb}${n}${sep}${pick(COMMERCE_TAILS_R22)}`);
    }
  }
}
for (const sep of SEPS_R22) {
  for (const t of [`our 13 year old daughter${sep}pics $20`, `Just turned 5${sep}pics for sale dm`, `She's 7${sep}Nudes $20`]) {
    addCase('refuse: family subject + age + commerce, any sentence (r22)', 'refuse', TAG_FREE, t);
  }
}
// accounts#2: advocacy nouns never in a label, never with a sale.
for (const lead of ['child', 'kid', 'kids', 'children', 'toddler']) {
  for (const noun of ['sex slaves', 'sex slave', 'sex workers', 'sex trade', 'sex rings']) {
    for (const tail of [' for sale', ' vids $20', ' dm me', ' for hire', ' videos']) addCase('refuse: child sex + advocacy noun + sale (r22)', 'refuse', EVERY, `${lead} ${noun}${tail}`);
    addCase('refuse: child sex + advocacy noun in a label (r22)', 'refuse', NAME_LIKE, `${lead} ${noun}`);
  }
}
// accounts#3: school x label separator x person word x sexual word.
for (const school of ['high school', 'highschool', 'middle school', 'school', 'High School', 'junior high']) {
  for (const sep of [': ', ' | ', ' • ', ' / ', ', ', ' - ', ' · ', '; ', ' — ']) {
    for (const person of ['girl', 'girls', 'boy', 'boys']) {
      for (const w of [' nudes', ' naked', ' porn', ' nudes $20']) addCase('refuse: school + separator + person + sexual (r22)', 'refuse', FREE, `${school}${sep}${person}${w}`);
    }
  }
}
// accounts#4/#5: under-ten ages with the and-tail, the age word, glued.
for (const lead of ['my niece is', 'my daughter is', 'hot', 'Hi,', 'she is', 'he is', "she's", "i'm", 'im']) {
  for (let n = 1; n <= 9; n++) {
    for (const tail of [' and horny', ' and a virgin', ', horny', ' & naked']) {
      if (tail === ', horny' && ['hot', 'Hi,'].includes(lead)) continue;
      addCase('refuse: under-ten + and-tail (r22)', 'refuse', TAG_FREE, `${lead} ${n}${tail}`);
    }
  }
}
const SPELLED_SINGLE = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'];
for (let n = 1; n <= 9; n++) {
  for (const w of ['horny', 'wet', 'sexy', 'naughty', 'naked', 'a virgin']) {
    const g = w.replace(/ /g, '');
    for (const age of [String(n), SPELLED_SINGLE[n - 1]]) {
      addCase('refuse: under-ten + and-tail (r22)', 'refuse', EVERY, `${age} and ${w}`);
      for (const ageWord of [' yo', ' years old', ' year old']) addCase('refuse: under-ten + age word + and-tail (r22)', 'refuse', EVERY, `${age}${ageWord} and ${w}`);
      for (const join of ['and', 'n', '_and_']) {
        addCase('refuse: under-ten glued and-form, labels (r22)', 'refuse', NAME_LIKE, `${age}${join}${g}`);
        // A letter-prefixed SPELLED age is the end of an ordinary name
        // (Simone, Janine, Antoine), so the name prefix is digits-only.
        if (/^[0-9]/.test(age)) addCase('refuse: under-ten glued and-form, labels (r22)', 'refuse', ['handle', 'username'], `jess${age}${join}${g}`);
      }
    }
  }
}
for (const t of ["i'm 9 and wet", 'im 8 and sexy', "I'm 8 and naughty", "she's 8 and wants cock", "i'm 9 and love cock"]) addCase('refuse: under-ten + and-tail (r22)', 'refuse', EVERY, t);
// accounts#6: reverse child pairs, "with", the acronyms, the younger schools.
for (const w of ['nude', 'naked', 'nudes', 'porn', 'topless']) {
  for (const child of ['kids', 'kid', 'children', 'child', 'toddlers', 'minors', 'infants', 'kiddos']) {
    for (const tail of ['', ' pics', ' for sale', ' 💦', ' dm me']) addCase('refuse: reverse child pair (r22)', 'refuse', EVERY, `${w} ${child}${tail}`);
    addCase('refuse: reverse child pair (r22)', 'refuse', NAME_LIKE, `${w}${child}`);
  }
}
for (const w of ['sex', 'fuck', 'porn', 'nudes']) {
  for (const art of ['', 'a ', 'the ']) {
    for (const child of ['kid', 'kids', 'child', 'children', 'minors', 'toddler']) addCase('refuse: <sexual> with a child (r22)', 'refuse', EVERY, `${w} with ${art}${child}`);
  }
}
for (const t of ['fuck a kid', 'fucking a child', 'fucked the kid', 'fuck a minor']) addCase('refuse: <sexual> with a child (r22)', 'refuse', EVERY, t);
for (const t of ['csam', 'csam for sale', 'selling csam', 'csam links', 'CSAM 4 sale', 'child sexual abuse material for sale', 'prepubescent',
  'prepubescent porn', 'pre-pubescent nudes', 'pre pubescent girl', 'kiddo porn', 'kiddos nudes', 'lil girl nudes', 'lil boy porn', 'lil girl',
  'kindergarten porn', 'elementary school porn', 'elementary school girl', 'grade schooler nudes', 'primary school girl nudes', 'cp for sale',
  'selling cp', 'cp links', 'buy cp', 'pre-teen nudes', 'preteen porn']) addCase('refuse: CSAM terms (r22)', 'refuse', EVERY, t);
for (const t of ['nakedkids', 'lilgirl', 'csam4sale', 'nudekids', 'kiddoporn']) addCase('refuse: CSAM terms (r22)', 'refuse', NAME_LIKE, t);
for (const t of ['We report CSAM to NCMEC', 'Zero tolerance for CSAM', 'No sex with minors ever', 'No sex with minors, obviously',
  'CP 12 is the checkpoint', 'Fighting child sex slavery', 'Stop child sex tourism', 'No nude kids ever, all models 18+']) addCase('pass: disclaimers (r22)', 'pass', FREE, t);
// accounts#7: the little one -- refused in a label or with any sexual word.
for (const one of ['little girl', 'little boy', 'lil girl', 'lil boy']) {
  addCase('refuse: little one, label (r22)', 'refuse', NAME_LIKE, one);
  for (const t of [`daddy's ${one}`, `your ${one}`, `my ${one} is so horny`, `my ${one}, pics $20`, `${one} nudes`]) addCase('refuse: little one + sexual (r22)', 'refuse', FREE, t);
}
// Round 22 fix-up: a child subject with an age is an age whatever noun
// follows -- a verb form, a bracketed age and a bare age after a family noun.
{
  const SUBJ = ['my daughter', 'daughter', 'my son', 'son', 'my niece', 'niece', 'my nephew', 'girl', 'boy', 'sis', 'my stepdaughter', 'our son'];
  const FORMS = [' is N ', ' aged N ', ' age N ', ' (N) ', ' [N] ', ', N, ', ' (age N) ', ' just turned N '];
  const TAILS = ['pics', 'vids', 'nudes', 'naked', 'dm for pics', 'for sale', '$20', 'pics $15', 'nudes for sale'];
  const BARE_OK = new Set(['my daughter', 'daughter', 'my son', 'son', 'my niece', 'niece', 'my nephew', 'my stepdaughter', 'our son']);
  for (const subj of SUBJ) {
    for (const form of FORMS) {
      if (form === ', N, ' && !BARE_OK.has(subj)) continue;
      for (let n = 1; n <= 17; n++) addCase('refuse: child subject + age + any noun (r22 fix-up)', 'refuse', TAG_FREE, `${subj}${form.replace('N', String(n))}${pick(TAILS)}`.replace(/\s+/g, ' '));
    }
    if (BARE_OK.has(subj)) for (let n = 1; n <= 17; n += 3) addCase('refuse: family noun + bare age (r22 fix-up)', 'refuse', TAG_FREE, `${subj} ${n} ${pick(TAILS)}`);
  }
}
// Round 22 fix-up: the "my little girl" exemption needs the absence of every
// adult cue, not just the family clause lists.
{
  const OWNERS = ['my', 'our', 'her', 'their', 'his', 'Mommy of a', 'mom of a'];
  const ONES = ['little girl', 'little boy', 'lil girl', 'lil boy'];
  const CUES = [', customs open', ' wants daddy', ' is waiting for you in bed', ' \u{1F346}\u{1F4A6}', ' \u{1F351}', ' tonight, tip menu open', ' spread wide',
    ' in the bath', ' is ready for you', ' wants you', ' \u{1F608}', ' in the shower', ' tip menu', ', dm me', ' wants more', ' playtime with daddy'];
  for (const owner of OWNERS) for (const one of ONES) for (const cue of CUES) addCase('refuse: little one + adult cue (r22 fix-up)', 'refuse', FREE, `${owner} ${one}${cue}`);
}
for (const t of ['selling c.p', 'c.p for sale', 'c p links', 'buy c.p', 'c_p for sale']) addCase('refuse: CSAM terms (r22 fix-up)', 'refuse', EVERY, t);
for (const school of ['school', 'high school', 'middle school', 'highschool']) {
  for (const sep of [' - ', ' | ', ': ', ' / ', ' • ']) {
    for (const sep2 of [' - ', ' | ', ': ', ' / ', ' • ', ' — ']) addCase('refuse: school + sep + person + sep + sexual (r22 fix-up)', 'refuse', FREE, `${school}${sep}girls${sep2}nude`);
  }
}
// accounts#8: a count needs no minor signal next to it.
for (let n = 10; n <= 17; n++) {
  for (const t of [`Set of ${n} year old nudes`, `Includes ${n} yo nudes`, `my daughter is ${n}. Includes ${n} nudes and 3 videos`]) addCase('refuse: count with a minor signal (r22)', 'refuse', FREE, t);
}
// accounts#9: tg with a strong routing instruction before it.
for (const cue of ['pay me on', 'pay $20 on', 'send it via', 'cheaper on my', 'payments via my', 'pay me via']) {
  for (const noun of ['babe', 'girl', 'bundle', 'content', 'gf', '']) addCase('refuse: strong instruction + tg (r22)', 'refuse', FREE, `${cue} tg ${noun}`.trim());
}
