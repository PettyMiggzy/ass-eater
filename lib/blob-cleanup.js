...
// A file that is really gone needs no retry -- but one of OUR paths keeps a
// row anyway, re-recorded as an upload ('token') with its clock restarted.
// The client upload token for that pathname may still be valid (a creator
// deleting something they uploaded minutes ago), and a token can re-create
// the file once it is gone: allowOverwrite:false only refuses an EXISTING
// blob. Dropping the row left such a re-upload in the store with nothing
// recording it (tombstoned, so never finalizable, and never swept). The sweep
// comes back after an hour, when any token has expired, and deletes whatever
// is there. Anything that is not our path is simply forgotten. Best-effort:
// a leftover row only costs the sweep one del() that answers not-found.
async function forgetDeleted(pathnames) {
  if (!pathnames.length) return;
  const ours = pathnames.filter((p) => OUR_PATH_RE.test(p));
  const others = pathnames.filter((p) => !OUR_PATH_RE.test(p));
  try {
    if (ours.length) {
      await query(
        `insert into media_uploads (pathname, reason)
           select p, 'token' from unnest($1::text[]) as p
           on conflict (pathname) do update
             set reason = 'token', created_at = now(), claimed_at = null, claim_token = null`,
        [ours],
      );
    }
    if (others.length) await query('delete from media_uploads where pathname = any($1::text[])', [others]);
  } catch (err) {
