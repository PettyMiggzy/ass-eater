...
export async function preserveMedia(items, { reportId = null, reason, preservedBy = 'admin', client = null } = {}) {
  ...
  // path as 'moved_token' for the sweep.) A 'moved_token' row is left alone:
  // it is that vacated ORIGINAL path, not this file (the file is in
  // evidence/ already), and nothing would re-record it -- deleting it on a
  // second quarantine let a replayed upload token park an untracked file at
  // the original path forever (round-13 media#0). The sweep only ever deletes
  // the original path for it, never the evidence copy.
  if (pathnames.length) {
    await runner.query(
      `delete from media_uploads where pathname = any($1::text[]) and reason is distinct from 'moved_token'`,
      [pathnames],
    );
  }
  await runner.query(
