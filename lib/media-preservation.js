 ...
 *     with a preserved or held file (listingMediaBlocked).
...
/**
 * Is any file of this listing preserved evidence or on a report hold? Checkout
 * refuses such a listing (defense in depth: preserveMedia already takes it off
 * sale, and a hold means a possible-minor report is still open). Throws on a
 * database error -- the caller must not sell on a failed check.
 */
export async function listingMediaBlocked(listing, client = null) {
  ...
  const { rows } = await runner.query(
    `select 1 from media_preservations where pathname = any($1::text[])
      union all
     select 1 from media_holds where pathname = any($1::text[])
      limit 1`,
    [pathnames],
  );
...
// and deleteUnfinalizedUpload) but NOT serving -- whether reported content also
