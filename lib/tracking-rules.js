 ...
 *     and must be 6-35 letters and digits with at least 2 digits. Round 18
 *     (money#0) lowered this from 8-35 / 6 digits: GLS prints an 8-character
 *     TrackID with 1-4 digits ("ZF8YY6HP"), Sendle references are 7-8
 *     characters with few digits. Fewer than 6 digits now only gets the
 ...
 * a short handle or a word can ride along with six digits. That is the
 * accepted trade-off -- the alternatives refused real tracking numbers --
 * backed by human review and buyer reports.
 */
...
// one in the header (6-35 letters/digits, at least 2 digits).
export const TRACKING_FORMAT_HINTS = {
  ...
  Other: 'the number your carrier gave you: 6-35 letters and digits, at least 2 of them digits',
};
...
export function trackingFieldsError({ carrier, trackingNumber } = {}) {
  ...
  if ((value.match(/[0-9]/g) || []).length < 2) {
    return { field: 'trackingNumber', message: `${hint} It needs at least 2 digits.` };
  }
