 ...
 *     more letters, the three-letter KIK only as the entire run). Inside a
...
// six letters or more, and KIK only as the ENTIRE run -- and neither short
// rule looks inside a 1Z shipper + service segment (namesContactApp).
...
function runHasShortAppName(run) {
  if (run === 'KIK') return true;
...
function namesContactApp(name, value) {
  ...
  for (const m of body.matchAll(/[A-Z]+/g)) {
    ...
    if (runHasShortAppName(run)) return true;
  }
  return false;
}
