...
function toCreatorOrder(o) {
  ...
  // so, and exactly when. The seller only needs to know that tracking can no
  // longer be changed, which trackingCorrectionsLeft === 0 already says.
  const {
    buyerId: _buyer, shippingAddress, closeReason: _reason, closedBy: _by, closeForced: _forced, trackingHistory: _history,
    addressErasedAt: _addressErased, trackingErasedAt: _trackingErased, ...rest
  } = o;
  ...
  return {
    ...
    // Erased tracking (eraseOrderShippingAddress) can no longer be corrected.
    trackingCorrectionsLeft: o.trackingErasedAt ? 0 : Math.max(0, MAX_TRACKING_CORRECTIONS - used),
  };
}
...
export async function markOrderShipped(orderId, creatorId, { carrier, trackingNumber }) {
                ...
                'shippedAt', case when data->>'status' = 'shipped' and data ? 'shippedAt' and data->>'shippedAt' is not null
                        ...
                        then jsonb_build_object(
                               ...
                               'trackingHistory', coalesce(data->'trackingHistory', '[]'::jsonb) || jsonb_build_array(jsonb_build_object(
                                 'carrier', data->'carrier', 'trackingNumber', data->'trackingNumber', 'replacedAt', $5::text)))
                        else '{}'::jsonb end
        where id = $1
          ...
          and (coalesce(data->>'status', '') = 'pending_shipment'
               or (data->>'status' = 'shipped'
                   and not (data ? 'trackingErasedAt')
                   and (data->>'carrier' is distinct from $3::text or data->>'trackingNumber' is distinct from $4::text)
  ...
  if (!rows.length) {
    ...
    if (o && o.status === 'shipped' && o.trackingErasedAt) {
      // The buyer's tracking details were erased (their account deletion or
      // an erasure request): a creator must not write a number back.
      throw Object.assign(
        // Neutral on purpose (round-16 legal-journeys#2): the seller is not
        // told that the buyer asked for erasure or deleted their account.
        new Error("Tracking can no longer be changed for this order -- contact team@onlyone1.fun if something is wrong."),
  ...
  // No notice at all for a buyer whose account was already gone.
...
export async function getOrderShipState(orderId, creatorId) {
  const { rows } = await query(
    `select data->>'status' as status, data->>'carrier' as carrier, data->>'trackingNumber' as tracking,
  ...
  return {
    ...
    correctionsLeft: r.tracking_erased ? 0 : Math.max(0, MAX_TRACKING_CORRECTIONS - (Number(r.corrections) || 0)),
  };
}
...
// went). `trackingErasedAt` is stamped, and markOrderShipped then refuses any
// further correction, so a creator cannot write a number back after the buyer
// asked for it to be gone. An order whose address was erased before this
// existed still matches (its tracking is still there), so a re-run of either
// erase clears it.
