    ...
    for (const item of items) {
      ...
      if (Number(current.priceCents) !== item.priceCents || currentKind !== item.kind || currentShipping !== itemShipping) {
        throw Object.assign(new Error(`The price of "${item.title || 'an item'}" changed. Review your cart and confirm again.`), {
          code: 'PRICE_CHANGED',
          listingId: item.listingId,
          // The locked row's CURRENT values, in the same shape the route's
          // precheck returns, so the cart can show the new price instead of
          // asking the fan to review one it never displays (round-11 money#1).
          item: {
            listingId: String(item.listingId),
            title: typeof current.title === 'string' ? current.title : (item.title || ''),
            priceCents: Number(current.priceCents),
...
const BUYER_ORDER_FIELDS = [
  ...
  'carrier', 'trackingNumber', 'shippedAt', 'trackingUpdatedAt', 'createdAt', 'closedAt', 'addressErasedAt', 'trackingErasedAt', 'tosVersion',
];
...
function toCreatorOrder(o) {
  ...
  // closedAt only.
  // trackingHistory (the carrier/number each correction replaced) is for
  // admins and support only.
  // Only the COUNT left is exposed (round-15 dashboard#1), so the dashboard
  // can stop offering "Edit tracking" once the cap is used up instead of
  // opening a form the server will refuse.
  // addressErasedAt / trackingErasedAt are not sent either (round-16
  // legal-journeys#2): they are set when the BUYER deletes their account or
  // asks for erasure, so passing them told every seller the buyer had done
  // so, and exactly when.
  // Round 17 (money#2, dashboard#0): nor may anything ELSE in the seller's
  // view change when that happens. The erased tracking number used to come
  // back null with 0 corrections left -- a state nothing but an erasure
  // produced, and immediate in the ship response for a buyer already gone.
  // The seller now sees the number THEY entered (sellerTrackingNumber, kept
  // for their view only when the buyer's copy is erased -- never shown to the
  // buyer, admins or anyone else) and the same correction count as ever; a
  // correction after an erasure updates only that seller-side copy
  // (markOrderShipped).
  const {
    buyerId: _buyer, shippingAddress, closeReason: _reason, closedBy: _by, closeForced: _forced, trackingHistory: _history,
    addressErasedAt: _addressErased, trackingErasedAt: _trackingErased, sellerTrackingNumber: _sellerTracking, ...rest
  } = o;
  const used = Array.isArray(o.trackingHistory) ? o.trackingHistory.length : 0;
  return {
    ...rest,
    trackingNumber: sellerVisibleTracking(o),
    shippingAddress: o.status === 'pending_shipment' ? safeDecrypt(shippingAddress) : null,
    trackingCorrectionsLeft: Math.max(0, MAX_TRACKING_CORRECTIONS - used),
  };
}

// The tracking number as the SELLER sees it: the order's own number, or --
// once the buyer's copy is erased -- the seller-side copy of what they entered.
function sellerVisibleTracking(o) {
  if (o.trackingErasedAt) return o.sellerTrackingNumber ?? null;
  return o.trackingNumber ?? null;
}
...
export async function markOrderShipped(orderId, creatorId, { carrier, trackingNumber }) {
  ...
  // once at checkout and never changes while the order is pending, so
  // checking it before the update is not a race.
  //
  // Only a PENDING order needs it. An order already 'shipped' is being given
  // a corrected carrier/tracking number, and its address may since have been
  // erased on purpose (eraseOrderShippingAddress, a buyer deleting their
  // account) -- refusing that correction with "contact us before shipping"
  // about an item already sent left the buyer with the wrong tracking number
  // forever (round-12 money#1). An order cannot go from shipped back to
  // pending, so a 'shipped' read here stays shipped.
  ...
  if (pre.length && pre[0].kind === 'physical' && pre[0].status === 'pending_shipment') {
  ...
  // A correction to an order already shipped keeps the ORIGINAL ship date the
  // buyer sees and stamps trackingUpdatedAt instead (round-13 dashboard#0: the
  // dashboard now has an "Edit tracking" control for shipped orders, and a
  // typo fix must not read as the parcel leaving a day later).
  //
  // Round 14 (dashboard#0, public-pages#0): the carrier/number being replaced
  // is appended to `trackingHistory` in the same UPDATE -- the order row is
  // the only receipt, and the number the buyer was first given must stay
  // recoverable by an admin -- and a shipped order takes at most
  // MAX_TRACKING_CORRECTIONS corrections, so the field is not an unlimited
  // creator-to-buyer text channel. A "correction" that changes nothing
  // matches no row here and is answered with the order as it stands.
  const now = new Date().toISOString();
  // Round 16 (legal-journeys#0) + fix-up: the ship UPDATE and, for a buyer
  // whose account is already gone, the erasure of their name/address and the
  // tracking number just written, commit TOGETHER -- as two autocommit
  // statements, a crash between them left a deleted buyer's shipped order
  // holding their address with nothing to erase it later. The buyer's users
  // row is read FOR KEY SHARE first, before the order row is touched: the
  // same users -> orders lock order deleteFanAccount takes (FOR UPDATE on the
  // user, then its own erase of shipped orders), so a deletion in flight is
  // waited for rather than deadlocked with, and its outcome is what decides.
  const { rows, buyerGone } = await withTransaction(async (client) => {
    const { rows: own0 } = await client.query(
      `select data->>'buyerId' as buyer_id from orders where id = $1 and data->>'creatorId' = $2`,
      [orderId, String(creatorId)],
    );
    const buyerId = own0[0]?.buyer_id;
    let gone = false;
    if (buyerId != null && buyerId !== '') {
      const { rowCount } = await client.query('select 1 from users where id = $1 for key share', [String(buyerId)]);
      gone = !rowCount;
    }
    // An order whose tracking was erased (the buyer deleted their account or
    // asked for erasure) still takes a correction, but ONLY into the
    // seller-side copy (round-17 money#2 / dashboard#0): the buyer's
    // trackingNumber stays null and the replaced entry keeps no number, so
    // nothing is written back that the erasure removed, while the seller's
    // view behaves exactly as for any other order.
    const { rows: shipped } = await client.query(
      `update orders
          set data = data || jsonb_build_object(
                'status', 'shipped',
                'carrier', $3::text,
                'trackingNumber', case when data ? 'trackingErasedAt' then null::text else $4::text end,
                'shippedAt', case when data->>'status' = 'shipped' and data ? 'shippedAt' and data->>'shippedAt' is not null
                                  then data->'shippedAt' else to_jsonb($5::text) end
              ) || case when data ? 'trackingErasedAt'
                        then jsonb_build_object('sellerTrackingNumber', $4::text)
                        else '{}'::jsonb end
                || case when data->>'status' = 'shipped'
                        then jsonb_build_object(
                               'trackingUpdatedAt', $5::text,
                               'trackingHistory', coalesce(data->'trackingHistory', '[]'::jsonb) || jsonb_build_array(jsonb_build_object(
                                 'carrier', data->'carrier',
                                 'trackingNumber', case when data ? 'trackingErasedAt' then 'null'::jsonb else data->'trackingNumber' end,
                                 'replacedAt', $5::text)))
                        else '{}'::jsonb end
        where id = $1
          ...
          and (coalesce(data->>'status', '') = 'pending_shipment'
               or (data->>'status' = 'shipped'
                   and (data->>'carrier' is distinct from $3::text
                        or (case when data ? 'trackingErasedAt' then data->>'sellerTrackingNumber' else data->>'trackingNumber' end)
                           is distinct from $4::text)
                   and jsonb_array_length(coalesce(data->'trackingHistory', '[]'::jsonb)) < $6::int))
        returning id, data`,
      [orderId, String(creatorId), carrier ?? null, trackingNumber ?? null, now, MAX_TRACKING_CORRECTIONS],
    );
    if (!shipped.length || !gone) return { rows: shipped, buyerGone: false };
    // The address was only kept so the item could still be sent. It has now
    // been sent, so the name/address and the tracking number just written go
    // the way every other shipped order of that buyer went. The carrier,
    // status and dates stay: the order records that it shipped.
    const { rows: erased } = await client.query(
      `update orders
          set ${ERASE_SHIPPING_SET_SQL}
        where id = $1
          and data->>'status' = 'shipped'
          and ${ERASABLE_SHIPPING_WHERE_SQL}
        returning id, data`,
      [shipped[0].id, new Date().toISOString()],
    );
    return { rows: erased.length ? erased : shipped, buyerGone: true };
  });
  if (!rows.length) {
    ...
    const { rows: own } = await query(
      `select id, data from orders where id = $1 and data->>'creatorId' = $2`,
      [orderId, String(creatorId)],
    );
    const o = own.length ? rowToRecord(own[0]) : null;
    if (o && o.status === ORDER_CLOSED_STATUS) throw orderClosedError();
    if (o && o.kind !== 'physical') throw new Error('Only physical orders can be marked shipped');
    if (o && o.status === 'shipped') {
      // Same values again (a double-click, a re-save): nothing to record.
      // Compared against what the SELLER sees, so an order whose buyer copy
      // was erased answers exactly like any other (round-17 money#2).
      if ((o.carrier ?? null) === (carrier ?? null) && sellerVisibleTracking(o) === (trackingNumber ?? null)) {
        return { ...toCreatorOrder(o), shippingAddress: undefined };
      }
      throw Object.assign(
        new Error(`Tracking can be corrected at most ${MAX_TRACKING_CORRECTIONS} times -- contact team@onlyone1.fun if it needs changing again.`),
        { code: TRACKING_EDIT_LIMIT },
      );
    }
    throw new Error('Order not found');
  }
  let record = rowToRecord(rows[0]);
  // dashboard#1: tell the buyer. The buyer id comes off the updated row
  // (toCreatorOrder strips it before anything reaches the creator). A deleted
  // buyer keeps their id on the order (it is a financial record), so it may
  // name an account that no longer exists: createNotification records nothing
  // for an account without a users row (round-15 legal-journeys#2). It may
  // also be null on an old order. A correction is the row whose
  // trackingUpdatedAt is this call's own stamp. Never the tracking number in
  // the text -- /orders shows it. createNotification never throws.
  // No notice at all for a buyer whose account was already gone, nor for
  // a correction to an order whose tracking the buyer had erased (there is
  // nothing for them to see).
  if (!buyerGone && !record.trackingErasedAt && record.buyerId != null && record.buyerId !== '') {
    const corrected = record.trackingUpdatedAt === now;
    const what = record.title ? `"${sliceText(String(record.title), 80)}"` : `#${record.id}`;
    await createNotification({
      userId: record.buyerId,
      type: corrected ? 'tracking_updated' : 'order_shipped',
      message: corrected
        ? `The tracking details for your order ${what} were updated -- see your orders page.`
        : `Your order ${what} has shipped -- tracking is on your orders page.`,
      meta: { orderId: String(record.id) },
      // Repeated corrections fold into one unread bell.
      coalesceKey: corrected ? 'orderId' : null,
    });
  }
  // Now shipped, so toCreatorOrder leaves the address (and buyer id) out.
  return { ...toCreatorOrder(record), shippingAddress: undefined };
}

export const TRACKING_EDIT_LIMIT = 'TRACKING_EDIT_LIMIT';
// Corrections allowed after an order first ships (each keeps the replaced
// values in trackingHistory).
export const MAX_TRACKING_CORRECTIONS = 3;
// The carrier / tracking-number SHAPE lives in lib/tracking-rules.js (pure,
// shared with the dashboard's ship form): a fixed carrier list and per-carrier
// tracking formats, so the fields can no longer carry a handle or a phone
// number to the buyer (round-15 money#0, public-pages#0, legal-journeys#1).
// Re-exported here for the ship route and older callers. markOrderShipped
// stores what it is given: pages/api/marketplace/orders/ship.js -- the only
// route that writes these -- validates with trackingFieldsError and stores
// normalizeTracking()'s canonical form.
export { trackingFieldsError, normalizeTracking, SHIPPING_CARRIERS } from './tracking-rules';

/**
 * What the ship route needs to know before it writes: whether this call would
 * be a CORRECTION of an already-shipped order (those are rate-limited; a first
 * shipment is not -- round-15 money#1 / dashboard#0) and how many corrections
 * remain. Scoped to the creator; null when the order is not theirs.
 * { status, carrier, trackingNumber, correctionsLeft }
 */
export async function getOrderShipState(orderId, creatorId) {
  const { rows } = await query(
    `select data->>'status' as status, data->>'carrier' as carrier,
            case when data ? 'trackingErasedAt' then data->>'sellerTrackingNumber' else data->>'trackingNumber' end as tracking,
            jsonb_array_length(case when jsonb_typeof(data->'trackingHistory') = 'array' then data->'trackingHistory' else '[]'::jsonb end) as corrections,
            data ? 'trackingErasedAt' as tracking_erased
       from orders where id = $1 and data->>'creatorId' = $2`,
    [orderId, String(creatorId)],
  );
  ...
  return {
    ...
    // The seller's view (round-17 money#2): an erased order counts the same.
    correctionsLeft: Math.max(0, MAX_TRACKING_CORRECTIONS - (Number(r.corrections) || 0)),
  };
}
...
export async function closeUnfulfilledOrder(orderId, { reason, eraseAddress = false, force = false } = {}) {
  ...
  const result = await withTransaction(async (client) => {
    const { rows: pre } = await client.query(
      `select data->>'creatorId' as creator_id, data->>'buyerId' as buyer_id, data->>'kind' as kind, data->>'status' as status
         from orders where id = $1`,
      [String(orderId)],
    );
    if (!pre.length) throw Object.assign(new Error('Order not found'), { code: ORDER_NOT_FOUND });
    const creatorId = pre[0].creator_id;
    // Round 17 (money#1, legal-journeys#0): a pending order of a buyer whose
    // account is already gone keeps their name/address only so the item can
    // still ship; closing it instead must erase them too, whatever the admin
    // ticked -- nobody is left to ask for it, and the account-deletion erase
    // already ran (it skips pending orders). The buyer's users row is read
    // FOR KEY SHARE first, before the seller's rows and the order UPDATE: the
    // users -> orders order markOrderShipped and deleteFanAccount take, so a
    // deletion in flight is waited for and its outcome decides.
    const buyerId = pre[0].buyer_id;
    let buyerGone = false;
    if (buyerId != null && buyerId !== '') {
      const { rowCount } = await client.query('select 1 from users where id = $1 for key share', [String(buyerId)]);
      buyerGone = !rowCount;
    ...
    const erased = (eraseAddress || buyerGone) ? (await eraseOrderShippingAddress(String(orderId), client)).erased : false;
    return { orderId: String(rows[0].id), buyerId: rows[0].buyer_id, buyerGone, status: ORDER_CLOSED_STATUS, closedAt, erased, forced };
  });
  // Tell the buyer, best-effort and after the commit: the close stands even
  // if the notification cannot be written. Nobody to tell when the account
  // is gone.
  if (result.buyerId && !result.buyerGone) {
    try {
      await createNotification({
  ...
  const { buyerId: _b, buyerGone: _g, ...out } = result;
  return out;
}
...
export async function getOrderSummariesForAdmin({ orderId = null, creatorId = null, buyerId = null, status = null, limit = 200 } = {}) {
  ...
  for (const row of rows) {
    ...
    out.push({
      ...
      trackingUpdatedAt: o.trackingUpdatedAt ?? null,
      // Every carrier/number a correction replaced, oldest first (dashboard#0).
      trackingHistory: Array.isArray(o.trackingHistory) ? o.trackingHistory : [],
      trackingErasedAt: o.trackingErasedAt ?? null,
...
// Round 15 (legal-journeys#0): a tracking number is as much the buyer's
// location as the address -- a carrier lookup returns the destination city/ZIP
// and delivery dates -- so erasing a shipped order's address also erases the
// tracking number and every number kept in trackingHistory (the carrier, the
// dates and the number of corrections stay: they say nothing about where it
// went). `trackingErasedAt` is stamped, and markOrderShipped then never
// writes a number back into the buyer's copy. Round 17 (money#2,
// dashboard#0): the number the seller entered is kept as
// `sellerTrackingNumber`, shown only in the seller's own view (toCreatorOrder)
// so that view does not change when the buyer erases -- a correction after an
// erasure updates only that copy. An order whose address was erased before
// this existed still matches (its tracking is still there), so a re-run of
// either erase clears it.
const ERASE_SHIPPING_SET_SQL = `data = jsonb_set(data, '{shippingAddress}', 'null'::jsonb)
                   || jsonb_build_object(
                        'addressErasedAt',
                        case when data->'shippingAddress' is not null and data->'shippingAddress' <> 'null'::jsonb
                             then to_jsonb($2::text)
                             else coalesce(data->'addressErasedAt', to_jsonb($2::text)) end,
                        'trackingNumber', null,
                        'sellerTrackingNumber',
                        case when data->>'sellerTrackingNumber' is not null then data->'sellerTrackingNumber'
                             else coalesce(data->'trackingNumber', 'null'::jsonb) end,
                        'trackingErasedAt', to_jsonb($2::text))
                   || case when jsonb_typeof(data->'trackingHistory') = 'array'
                           then jsonb_build_object('trackingHistory', (
                             select coalesce(jsonb_agg(case when jsonb_typeof(e) = 'object'
                                                            then e || jsonb_build_object('trackingNumber', null)
                                                            else e end order by ord), '[]'::jsonb)
                               from jsonb_array_elements(data->'trackingHistory') with ordinality as t(e, ord)))
                           else '{}'::jsonb end`;
const ERASABLE_SHIPPING_WHERE_SQL = `((data->'shippingAddress' is not null and data->'shippingAddress' <> 'null'::jsonb)
             or (data->'trackingNumber' is not null and data->'trackingNumber' <> 'null'::jsonb)
             or (jsonb_typeof(data->'trackingHistory') = 'array'
                 and jsonb_path_exists(data, '$.trackingHistory[*].trackingNumber ? (@ != null)')))`;
 ...
 * could only be kept with hand-written SQL against production. The tracking
 * numbers go with it (ERASE_SHIPPING_SET_SQL above).
 *
 * Sets `shippingAddress` and the tracking numbers to null and stamps
 * `addressErasedAt` / `trackingErasedAt` on ONE order,
...
/**
 * Erases the shipping address and tracking numbers of every already-SHIPPED
 * (not pending) order a buyer placed, stamping addressErasedAt and
 * trackingErasedAt, on the caller's transaction. Used by
...
/**
 * Erases the shipping address and tracking numbers of every order that is no
 * longer pending_shipment and whose buyer's account no longer exists
 * (round-17 money#1 / legal-journeys#0). Account deletion erases a buyer's
 * shipped/closed orders at deletion time and markOrderShipped /
 * closeUnfulfilledOrder erase a pending one when it ends; this catches what
 * those left behind -- an order closed before closeUnfulfilledOrder erased
 * for a deleted buyer, or any other path that ended a pending order of a gone
 * account. Run by the daily maintenance cron. Returns how many were erased.
 */
export async function eraseAddressesOfDeletedBuyers(client = null) {
  const runner = client || { query };
  const { rowCount } = await runner.query(
    `update orders o
        set ${ERASE_SHIPPING_SET_SQL}
      where coalesce(o.data->>'status', '') <> 'pending_shipment'
        and coalesce(o.data->>'buyerId', '') <> $1
        and not exists (select 1 from users u where u.id = o.data->>'buyerId')
        and ${ERASABLE_SHIPPING_WHERE_SQL}`,
    // $2 is the erase stamp ERASE_SHIPPING_SET_SQL expects.
