...
function toCreatorOrder(o) {
  ...
  const { buyerId: _buyer, shippingAddress, closeReason: _reason, closedBy: _by, closeForced: _forced, trackingHistory: _history, ...rest } = o;
...
export async function markOrderShipped(orderId, creatorId, { carrier, trackingNumber }) {
  ...
  if (!rows.length) {
    ...
    if (o && o.status === 'shipped' && o.trackingErasedAt) {
      ...
      throw Object.assign(
        new Error("This order's tracking details were erased at the buyer's request and can't be changed -- contact team@onlyone1.fun if something is wrong."),
  ...
  if (record.buyerId != null && record.buyerId !== '') {
