...
function placeOrders({ buyerId, items, idempotencyKey }) {
  return withTransaction(async (client) => {
    ...
    for (const item of items) {
      ...
      // A listing with a file quarantined as evidence, or on a possible-minor
      // report hold: never sold. preserveMedia takes such a listing off sale
      // in the quarantine's own commit; this is the backstop on the locked row
      // (a preserved file is never served, so the buyer would get nothing).
