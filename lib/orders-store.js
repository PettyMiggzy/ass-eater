...
// order, on the checkout's transaction. Returns `held`, extended.
...
function placeOrders({ buyerId, items, idempotencyKey }) {
  return withTransaction(async (client) => {
    ...
    {
      const ids = [...new Set(items.map((it) => String(it.listingId)))].filter((id) => /^[1-9]\d{0,17}$/.test(id));
      const { rows: peek } = await client.query('select data from listings where id = any($1::bigint[])', [ids]);
      const all = [];
      for (const r of peek) {
        all.push(...(Array.isArray(r.data?.media) ? r.data.media : []), ...(Array.isArray(r.data?.retainedMedia) ? r.data.retainedMedia : []));
      }
      await lockListingFiles(client, { media: all }, lockedFiles);
    }
