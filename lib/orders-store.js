...
const BUYER_ORDER_FIELDS = [
  ...
  'carrier', 'trackingNumber', 'shippedAt', 'trackingUpdatedAt', 'createdAt', 'closedAt', 'addressErasedAt', 'tosVersion',
];
...
export async function markOrderShipped(orderId, creatorId, { carrier, trackingNumber }) {
  ...
  // (toCreatorOrder strips it before anything reaches the creator); it may be
  // null when the buyer's account was deleted. A correction is the row whose
...
// A carrier name: letters, digits, spaces and a little punctuation ("UPS",
// "Royal Mail", "DHL eCommerce", "J&T Express", "Canada Post / Postes
// Canada"), up to 40. Not a fixed list -- small and international carriers
// are legitimate -- but no "$", "@", ":" or URL shapes.
const CARRIER_RE = /^[A-Za-z0-9][A-Za-z0-9 .&'()\/-]{0,39}$/;
// A tracking number: letters, digits, spaces and "-", 4 to 40, with at least
// one digit -- every real tracking number has digits; "jessxo" is a handle.
const TRACKING_NUMBER_RE = /^(?=[^0-9]*[0-9])[A-Za-z0-9 -]{4,40}$/;
// A carrier that is really a contact or payment app, with a handle or number
// in the tracking field ("Snap" + "jessxo", "WhatsApp" + "44 7700 900123")
// is a handover the text screen cannot see in two short fields (round-14
// fix-up). Compared on the letters only, so "Tele gram" and "Cash-App" count.
// The long names are refused anywhere in the carrier, the short ones only as
// the whole carrier name ("XPO", "DHL", "Line Haul Express" are real).
...
function carrierNamesContactApp(carrier) {
...
/**
 * null when `{ carrier, trackingNumber }` are acceptable (already trimmed),
 * otherwise `{ field: 'carrier' | 'trackingNumber', message }`. Checked by
 * pages/api/marketplace/orders/ship.js -- the only route that writes these --
 * together with the payment-circumvention / prohibited-terms screen (round-14
 * public-pages#0: they are shown on the buyer's /orders page).
 */
export function trackingFieldsError({ carrier, trackingNumber } = {}) {
  if (typeof carrier !== 'string' || !CARRIER_RE.test(carrier)) {
    return { field: 'carrier', message: 'Carrier: letters, numbers, spaces and . & \' ( ) / - only, up to 40 characters.' };
  }
  if (carrierNamesContactApp(carrier)) {
    return { field: 'carrier', message: 'Carrier: enter the shipping company (e.g. USPS, UPS, FedEx, DHL), not an app or contact.' };
  }
  if (typeof trackingNumber !== 'string' || !TRACKING_NUMBER_RE.test(trackingNumber)) {
    return { field: 'trackingNumber', message: 'Tracking number: letters, numbers, spaces and "-" only, 4 to 40 characters, including at least one number.' };
  }
  return null;
}
