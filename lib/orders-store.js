...
function toCreatorOrder(o) {
  ...
  // so, and exactly when. (trackingErasedAt only survives on orders erased
  // before round 18; nothing sets it now.)
  // Round 18 (money#1/#2, public-pages#0, legal-journeys#0): the carrier and
  // tracking number are the SELLER's own carrier reference, not the buyer's
  // personal data -- an erasure removes only the buyer's shipping name and
  // address -- so buyer and seller see the SAME carrier and number. The
  // round-17 seller-side copy (sellerTrackingNumber) is gone; lib/db.js
  // folds any stored copy back into trackingNumber.
  const {
...
export async function markOrderShipped(orderId, creatorId, { carrier, trackingNumber }) {
  ...
  // whose account is already gone, the erasure of their name/address, commit
  // TOGETHER -- as two autocommit
  ...
  const { rows, buyerGone } = await withTransaction(async (client) => {
    ...
    // The carrier and tracking number are the seller's own carrier reference
    // (round 18): one copy, the same for buyer and seller, never erased with
    // the buyer's address. A legacy trackingErasedAt marker (orders erased
    // before round 18) and any old seller-side copy are dropped when a new
    // number is written, so the buyer sees the number again.
    const { rows: shipped } = await client.query(
      `update orders
          set data = (data - 'sellerTrackingNumber' - 'trackingErasedAt') || jsonb_build_object(
                'status', 'shipped',
                'carrier', $3::text,
          ...
          and (coalesce(data->>'status', '') = 'pending_shipment'
               or (data->>'status' = 'shipped'
                   and (data->>'carrier' is distinct from $3::text
                        or data->>'trackingNumber' is distinct from $4::text)
    ...
    // been sent, so the name/address go the way every other shipped order of
    // that buyer went. The carrier, tracking number, status and dates stay:
    // the order records that it shipped, and how (round 18).
  ...
  if (!rows.length) {
    ...
    if (o && o.status === 'shipped') {
      ...
      if ((o.carrier ?? null) === (carrier ?? null) && (o.trackingNumber ?? null) === (trackingNumber ?? null)) {
  ...
  // No notice at all for a buyer whose account was already gone. A buyer who
  // asked for erasure still sees the carrier and number on /orders (round
  // 18: they are the seller's reference, not the buyer's data), so a
  // correction is announced to them like to anyone.
  if (!buyerGone && record.buyerId != null && record.buyerId !== '') {
...
export async function getOrderShipState(orderId, creatorId) {
  const { rows } = await query(
    `select data->>'status' as status, data->>'carrier' as carrier,
            data->>'trackingNumber' as tracking,
...
// Erasing a shipped order's buyer data: the shipping name and address, and
// nothing else. DECIDED in round 18 (money#1/#2, public-pages#0,
// legal-journeys#0): the carrier, the tracking number and trackingHistory are
// the carrier reference the SELLER entered, not the buyer's personal data, so
// they stay on the order and buyer and seller see the same order. (Rounds
// 15-17 erased the number too, then kept a seller-only copy that nothing ever
// cleared and that made the two views disagree; lib/db.js folds any such copy
// back into trackingNumber.) `addressErasedAt` is stamped with $2.
 ...
 * numbers stay: they are the seller's reference (ERASE_SHIPPING_SET_SQL above).
 *
...
/**
 * Erases the shipping name and address of every already-SHIPPED (not
 * pending) order a buyer placed, stamping addressErasedAt, on the caller's
 * transaction. Used by
...
/**
 * Erases the shipping name and address of every order that is no longer
 * pending_shipment and whose buyer's account no longer exists
