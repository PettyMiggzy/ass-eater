...
import { lockMediaFiles, lockFilesOfListings, listingFileItems } from './media-locks';
...
// listing's media and retainedMedia files not already in `held`, in sorted
// order, on the checkout's transaction (lib/media-locks.js). Returns `held`.
function lockListingFiles(client, listing, held) {
  return lockMediaFiles(client, listingFileItems(listing), held);
}
...
function toCreatorOrder(o) {
  const { buyerId: _buyer, shippingAddress, ...rest } = o;
 ...
 * error). Stamps `closedAt`, `closeReason` and `closedBy: 'admin'`. With
 * `eraseAddress: true` the shipping address is erased in the SAME transaction
 * (eraseOrderShippingAddress, which the new status now allows).
 *
 ...
 *
 * Throws ORDER_NOT_FOUND (no such order) or ORDER_NOT_CLOSABLE (digital, or
 * not pending shipment -- `.status` says which). Returns
 * { orderId, status, closedAt, erased }.
 */
export async function closeUnfulfilledOrder(orderId, { reason, eraseAddress = false } = {}) {
  if (!/^[1-9]\d{0,17}$/.test(String(orderId ?? ''))) {
    throw Object.assign(new Error('Order not found'), { code: ORDER_NOT_FOUND });
  }
  const why = String(reason ?? '').trim().slice(0, MAX_CLOSE_REASON);
  ...
  const result = await withTransaction(async (client) => {
    const { rows } = await client.query(
      ...
      [String(orderId), ORDER_CLOSED_STATUS, closedAt, why],
    );
    if (!rows.length) {
    ...
    return { orderId: String(rows[0].id), buyerId: rows[0].buyer_id, status: ORDER_CLOSED_STATUS, closedAt, erased };
  });
