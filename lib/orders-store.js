...
// pages/terms.js, Section 6 (Marketplace Purchases) -- the terms a buyer
// accepts per order. Bump this whenever that section's text materially
// changes, so each order records the version its buyer actually agreed to.
...
export const CURRENT_TOS_VERSION = '2026-09-24';
...
export async function createOrdersFromCredits({ buyerId, items, ageConfirmed, tosAccepted, idempotencyKey }) {
  ...
  return withTransaction(async (client) => {
    ...
    for (const item of items) {
      ...
      if (!current || current.status !== 'active' || String(current.creatorId) !== String(item.creatorId)) {
        throw Object.assign(
          new Error(
            current?.status === 'sold'
