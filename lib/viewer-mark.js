 ...
 *
 * WHAT THIS DOES NOT COVER, so no copy promises it: creator profiles are
 * public, so a signed-out visitor sees them with only a generic site mark
 * (ProtectedMedia's GUEST_MARK) that traces to nobody; and the mark is an
 * in-page overlay, not burned into the file. Any creator-facing claim must
 * say "signed-in viewers", not "every viewer".
 */
...
/**
 * A code as someone types it off a screenshot, in the canonical "XXXX-XXXX"
 * form, or null if it cannot be one: trimmed, uppercased, an optional
 * "ONLYONE" prefix dropped, and the dash accepted or missing.
 */
export function normalizeViewerMark(input) {
  if (typeof input !== 'string') return null;
  const hex = input.trim().toUpperCase().replace(/^ONLYONE\s*/, '').replace(/[\s-]/g, '');
