 ...
 *
 * WHAT IS ENFORCED, because this matters for what the recruitment page is
 * allowed to promise:
 *
 *  - The badge, the 100-slot cap and priority placement are real. They are
 *    decided here and applied by the pages that sort creators.
 *  - "0% fee for 30 days" is enforced at the money layer, not just shown:
 *    every fan-to-creator charge goes through lib/credits-store.js's
 *    transferWithFee, which asks feeWaiverActive() about the RECIPIENT and,
 *    while it is true, takes no fee at all -- no platform fee and no
 *    marketplace listing fee. A founding creator keeps 100% of what fans
 *    spend on them (marketplace sales and paid messages) for the whole
 *    window. The 2% a fan pays to BUY credits is on the fan's side of the
 *    deposit, not on anything spent on the creator, so it is not part of it.
 *  - Payouts settle in a dollar stablecoin (USDG), from earned credits.
 */
...
/**
 * How many of the 100 slots are gone. A BANNED creator does not hold one: a
 * ban clears the flag (lib/creators-store.js applyContentViolation and
 * pages/api/admin/profile.js), and this also skips any banned record that
 * still carries it, so the programme never quietly shrinks by one per ban.
 * pages/api/admin/profile.js enforces the same count atomically at grant
 * time (updateCreatorProfile's foundingSlot); this is the display side.
 */
export function countFounding(creators) {
  return (creators || []).filter((c) => isFoundingCreator(c) && c?.status !== 'banned').length;
}
 ...
 * day the platform started taking fees. Credits and marketplace checkout
 * went live with the 2026-09-24 release, which is the default below; the
 * env var exists so that date can be corrected without a code change.
 * Only server code sees the env var (it is not NEXT_PUBLIC_), so a client
 * bundle always reads the default -- if it is ever overridden, pages must
 * compute waiver dates in getServerSideProps rather than in the browser.
 */
export const PAYMENTS_LIVE_AT = process.env.PAYMENTS_LIVE_AT || '2026-09-24T00:00:00.000Z';
...
export function feeWaiverStartsAt(creator) {
  ...
  if (live === null || Number.isNaN(live)) return null; // misconfigured date: fail closed
 ...
 *
 * A founding creator with no `foundingSince` gets false rather than an
 * unbounded waiver: failing closed on a money question. Removing the
 * founding flag ends the waiver from that moment on.
 */
...
const PLACEHOLDER_AVATARS = ['/images/avatar-placeholder.png', '/images/mascot.png'];
...
/**
 * Would approving this creator (stored status 'pending' -> 'active') still
 * auto-grant the badge, given a finished profile and a free slot? The one
 * predicate pages/api/admin/profile.js grants on and the dashboard's share
 * kit uses to decide whether to tell a creator "finish your profile and you
 * get it" -- kept here so the promise and the grant can't drift apart.
 *
 * Only a first approval: never for a creator who already holds the badge,
 * was ever approved (approvedAt is stamped once and never cleared), was
 * ever banned (bannedAt), had the badge revoked, or has a confirmed content
 * violation. Hand grants by an admin are not limited by this.
 */
export function foundingAutoGrantEligible(creator) {
  if (!creator) return false;
  return (
    creator.status === 'pending' &&
    !isFoundingCreator(creator) &&
    !creator.foundingRevokedAt &&
    !creator.approvedAt &&
    !creator.bannedAt &&
    !(Number(creator.contentViolationCount) > 0)
  );
}
