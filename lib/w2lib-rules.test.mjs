...
section('Admin manual ban: every unsold listing taken down as moderation');
{
  ...
  // A real sale leaves a paid order behind; files are kept for its buyer.
  await query(`insert into orders (data) values ($1::jsonb)`,
    [JSON.stringify({ listingId: String(sold.id), creatorId: String(c.id), buyerId: 'buyer-1', kind: 'digital', status: 'fulfilled' })]);
