...
async function deleteOrphanedUsers(client) {
  ...
  // As in deleteFanAccount: nothing can be left in a deleted login's bell
  // (createNotification waits on the users row lock and then inserts nothing).
  if (ids.length) await client.query('delete from notifications where user_id = any($1::text[])', [ids]);
  return ids;
}
 ...
 */
// Locks the login (users) rows of these creators FOR UPDATE, in id order.
async function lockCreatorLogins(client, creatorIds) {
  if (!creatorIds.length) return;
  await client.query(
    `select id from users where data->>'creatorId' = any($1::text[]) order by id for update`,
    [creatorIds.map(String)],
  );
}
...
export async function deleteCreator(creatorId, { force = false } = {}) {
  ...
  // Retried once on a deadlock, like account deletion (round 18).
  const { deleted, obligations, files, removedUserIds } = await withTransactionRetryOnDeadlock(async (client) => {
    // The creator's LOGIN rows are locked first -- users before creators,
    // the order transferWithFee takes them in. A creator account can deposit
    // credits, and a deposit locks only its users row (lib/deposit.js
    // recordDepositCredit, FOR SHARE); with just the creator row locked here,
    // a deposit could commit after the obligations read below and before
    // deleteOrphanedUsers removed the login, leaving the credit on a deleted
    // account with nothing reporting it as stranded. Now the deposit either
    // commits first (and is counted below) or waits and finds no account.
    await lockCreatorLogins(client, [id]);
...
export async function deleteAllCreators(includeSeed, { force = false } = {}) {
  await ensureSeeded();
  const { deleted, skipped, stranded, files, removedUserIds } = await withTransactionRetryOnDeadlock(async (client) => {
    // Login rows first, then the creator rows (see deleteCreator): the
    // candidate set is read unlocked, their logins locked, then exactly those
    // creator rows locked.
    const { rows: pick } = await client.query(
      includeSeed
        ? 'select id from creators'
        : `select id from creators where coalesce((data->>'seed')::boolean, false) = false`,
    );
    const pickedIds = pick.map((r) => String(r.id));
    await lockCreatorLogins(client, pickedIds);
    const { rows } = await client.query(
      'select id, data from creators where id::text = any($1::text[]) order by id for update',
      [pickedIds],
    );
