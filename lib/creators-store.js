...
export async function setCreatorAvatar(creatorId, url) {
  ...
  const { creator, previous } = await withTransaction(async (client) => {
    ...
    const { rows: updated } = await client.query(
      `update creators set data = data || jsonb_build_object('img', $2::text), updated_at = now()
        where id = $1 returning id, data`,
      [String(creatorId), url],
    );
    return { creator: rowToRecord(updated[0]), previous: rows[0].img };
  });
  if (previous && previous !== url) await deleteMediaQuietly(previous);
  return creator;
}
 ...
 * the change commits.
 */
export async function removeGalleryItem(creatorId, { src, index } = {}) {
  await ensureSeeded();
  ...
  const { creator, removed } = await withTransaction(async (client) => {
    ...
    return { creator: rowToRecord(updated[0]), removed: gone };
  });
  // Only delete the file if no other gallery entry still points at it.
  if (!(creator.gallery || []).some((item) => item && item.src === removed.src)) {
    await deleteMediaQuietly(removed);
  }
  return creator;
}
 ...
 * every media file they uploaded.
 *
...
export async function deleteCreator(creatorId, { force = false } = {}) {
  ...
  const { deleted, obligations } = await withTransaction(async (client) => {
    ...
    await deleteOrphanedUsers(client);
    return { deleted: rowToRecord(rows[0]), obligations };
  });
  if (deleted) {
    await removeListingsForCreator(id);
    await deleteMediaQuietly(creatorMedia(deleted));
  }
  return { creators: await getCreators(), stranded: deleted && hasObligations(obligations) ? [obligations] : [] };
}
 ...
 * listed in `stranded`. Returns { creators, skipped, stranded }.
 */
export async function deleteAllCreators(includeSeed, { force = false } = {}) {
  await ensureSeeded();
  const { deleted, skipped, stranded } = await withTransaction(async (client) => {
    ...
    await deleteOrphanedUsers(client);
    return { deleted: toDelete, skipped, stranded };
  });
  for (const c of deleted) {
    await removeListingsForCreator(String(c.id));
    await deleteMediaQuietly(creatorMedia(c));
  }
  return { creators: await getCreators(), skipped, stranded };
}
...
export async function applyContentViolation(creatorId, client = null, { reportId = null } = {}) {
  ...
  const { rows } = await run(
    `update creators
      ...
      returning id, data`,
    [String(creatorId), new Date(Date.now() + SUSPENSION_MS).toISOString(), report],
  );
  if (!rows.length) {
