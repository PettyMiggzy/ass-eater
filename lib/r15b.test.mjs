...
section('money#0 / public-pages#0 / legal-journeys#1: tracking fields are structured, not free text');
{
  ...
  for (const [c, t] of [
    ...
    ['Other', 'SNAP12345678'], ['Other', 'jessxo 12345678'],
    ...
    ['UPS', '1ZKIK00012345678901'], ['Other', 'JESS61755512340'], ['Other', 'ONLY12345678FANS'], ['Other', 'PAYP123456789'],
  ]) {
    const e = E(c, t);
  ...
  for (const [c, t] of [['Other', 'SNAP12345678'], ['UPS', '1ZWHATSAPP12345678'], ['UPS', '1ZSNAPJESSXO123456'],
  ...
  for (const [c, t] of [['Other', 'JESS61755512340'], ['Other', 'PAYP123456789'], ['Other', 'ONLY12345678FANS'], ['USPS', 'jessxo2']]) {
...
section('legal-journeys#2: no notification is recorded for a deleted buyer');
{
  ...
  // buyer's address and tracking number are erased, so there is nothing left
  // to correct (the correction is refused with a neutral message).
  check('an order whose buyer was deleted still ships', r1.statusCode === 200, JSON.stringify(r1.body));
  check('...its tracking is erased at once, so a correction is refused', r2.statusCode === 409
