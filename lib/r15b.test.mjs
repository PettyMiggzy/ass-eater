...
section('money#0 / public-pages#0 / legal-journeys#1: tracking fields are structured, not free text');
{
  ...
  for (const [c, t] of [
    ...
    // Round 18 (money#0, decided): the floor is 6-35 characters with at least
    // 2 digits (GLS TrackIDs), so 'tg jess 99' is now accepted with a
    // non-blocking note -- the stated residual (a short handle riding with a
    // couple of digits). With one digit it is still refused.
    ['USPS', 'jessxo2'], ['USPS', 'ig jessxo 1'], ['USPS', 'tg jess 9'],
    ['Other', 'SNAP12345678'],
