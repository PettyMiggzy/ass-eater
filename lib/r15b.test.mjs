...
section('money#0 / public-pages#0 / legal-journeys#1: tracking fields are structured, not free text');
{
  ...
  for (const [c, t] of [
    ...
    ['UPS', 'discord jess 1'], ['UPS', 'wickr jess 1'],
    // Round 19 (money#0, DECIDED): no digit floor, so 'jessxo2', 'ig jessxo
    // 1', 'tg jess 9' and 'jessxo 2' moved to the accepted list below -- the
    // stated residual (a bare word/handle naming no app).
    ['Other', 'SNAP12345678'],
  ...
  for (const [c, t] of [['USPS', 'jessxo2'], ['USPS', 'ig jessxo 1'], ['USPS', 'tg jess 9'], ['USPS', 'jessxo 2']]) {
    check(`now accepted (round 19, no digit floor): ${c} | ${t}`, !E(c, t), JSON.stringify(E(c, t)));
  }
  ...
  // (Round 19: 'jessxo2' is accepted now -- no digit floor; 'jess.xo2' is
  // still a malformed value.)
  for (const [c, t] of [['USPS', 'jess.xo2'], ['Other', 'jess@x.co 12345678'], ['UPS', '1Z999AA1012345678']]) {
  ...
  const word = await ship('USPS', 'jess.xo2');
  check('the route refuses a malformed handle under USPS (400, trackingNumber)', word.statusCode === 400 && word.body.field === 'trackingNumber', JSON.stringify(word.body));
...
section('legal-journeys#2: no notification is recorded for a deleted buyer');
{
  ...
  // buyer's address is erased. Round 18 (decided): the tracking number is the
  // seller's reference and stays; a correction is stored like any other.
  check('an order whose buyer was deleted still ships', r1.statusCode === 200, JSON.stringify(r1.body));
  const goneRow = (await query('select data from orders where id = $1', [gone])).rows[0].data;
  check('...its address is erased at once; a correction is stored like any other', r2.statusCode === 200
    && goneRow.shippingAddress === null && goneRow.trackingNumber === '1Z999AA10123456793', JSON.stringify(goneRow));
