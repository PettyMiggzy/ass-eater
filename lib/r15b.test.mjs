...
section('money#0 / public-pages#0 / legal-journeys#1: tracking fields are structured, not free text');
{
  ...
  for (const [c, t] of [
    ...
    ['UPS', 'discord jess 1'], ['UPS', 'wickr jess 1'], ['USPS', '617 555 1234'], ['FedEx', '6175551234'], ['USPS', 'jessxo 2'],
    ['USPS', 'jessxo2'], ['USPS', 'ig jessxo 1'], ['USPS', 'tg jess 99'], ['Other', '617 555 1234'], ['Other', '1 617 555 1234'],
    ['Other', 'IG6175551234'], ['Other', 'SNAP12345678'], ['Other', '07700900123'], ['Other', 'jessxo 12345678'],
    ['UPS', '617-555-1234'], ['Other', 'WHATSAPP447700900123'], ['UPS', '1Z999AA1012345678'], ['FedEx', '1234567890123'],
    // Round-15 fix-up: app names inside a UPS number, phone numbers padded
    // into the digit-only formats, and free letters under Other.
    ['UPS', '1Z WHATSAPP 12345678'], ['UPS', '1ZINSTAJESS1234567'], ['UPS', '1ZSNAPJESSXO123456'], ['UPS', '1ZCASHAPPJESS12345'],
    ['UPS', '1ZWA6175551234 0000'], ['UPS', '1ZKIK00012345678901'], ['USPS', '61755512340000000000'], ['USPS', '0000617555123400000'],
    ['FedEx', '000000617555123400'], ['Other', 'JESS61755512340'], ['Other', 'ONLY12345678FANS'], ['Other', 'PAYP123456789'],
    ['Other', '617555123412'], ['Other', 'AB6175551234'],
    // ...and wrong check digits (a typo, or a stuffed value) under the formats that carry one.
  ...
  for (const [c, t] of [['Other', '617 555 1234'], ['Other', 'IG6175551234'], ['Other', 'SNAP12345678'], ['UPS', '1ZWHATSAPP12345678'],
    ['UPS', '1ZSNAPJESSXO123456'], ['UPS', '1ZWA61755512340000'], ['USPS', '61755512340000000000'], ['Other', 'JESS61755512340'],
    ['Other', 'PAYP123456789'], ['Other', 'ONLY12345678FANS']]) {
  ...
  const phone = await ship('USPS', '617 555 1234');
  check('the route refuses a phone number under USPS (400, trackingNumber)', phone.statusCode === 400 && phone.body.field === 'trackingNumber', JSON.stringify(phone.body));
  ...
  const handle = await ship('Other', 'IG6175551234');
  check('...and an app prefix + phone under Other', handle.statusCode === 400 && handle.body.field === 'trackingNumber');
  const viol = (await query('select data from violations order by id')).rows.map((r) => r.data.context);
  check('the suspected handover is logged as order_tracking_shape', viol.includes('order_tracking_shape'), JSON.stringify(viol));
...
section('legal-journeys#2: no notification is recorded for a deleted buyer');
{
  ...
  check('an order whose buyer was deleted still ships and can be corrected', r1.statusCode === 200 && r2.statusCode === 200);
