 ...
 * the report and its stored copies. A purged reporter's id is never returned
 * either, and a copy's participant list reads null for them.
 */
function stripReporterTrail(r) {
  const out = { ...r };
  const strip = (copy) => {
...
export async function attachReportTargets(reports) {
  ...
  return reports.map(stripReporterTrail).map((r) => {
