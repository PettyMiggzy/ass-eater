...
const cut = (v) => (typeof v === 'string' ? v.slice(0, SNAPSHOT_TEXT_MAX) : null);
...
function normalizeNote(note) {
  ...
  return text ? text.slice(0, REPORT_NOTE_MAX) : null;
}
 ...
 * listing report's files are put back on hold. Throws REPORT_NOT_FOUND /
...
export async function reopenReport(id, { reason, by = 'admin' } = {}) {
  ...
  return withTransaction(async (client) => {
    ...
    if (report.category === 'minor' && report.targetType === 'listing') {
