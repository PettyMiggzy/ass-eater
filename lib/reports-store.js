...
import { query, rowToRecord, rowsToRecords, withTransaction, REPORT_PRIORITY_SQL } from './db';
...
export function validateReportInput({ reason, category } = {}) {
  ...
  // A NUL or half an emoji cannot be stored as jsonb: refused with a 400
  // here, not a 500 from the insert -- a possible-minor report with such text
  // was never filed at all (round-11 social#1).
  if (!isWellFormedText(text)) {
    return { error: 'The reason contains an invalid character (a control character or half of an emoji). Remove it and try again.', field: 'reason' };
  }
...
/**
 * One page of the admin REPORTS queue (round-12 social#0, the same fix as
 * getNciiReportsPage): filtered by status in SQL and bounded, instead of the
 * whole table loaded, filtered in JS and returned (with every reported
 * conversation's row attached) in one response that could outgrow the
 * platform's response limit. Order matches reportPriority: possible minor,
 * then non-consensual, then everything else; newest first within each.
 * Keyset-paged by `cursor` (the previous page's `nextCursor`).
 * Returns { reports, hasMore, nextCursor }.
 */
export const REPORTS_PAGE_SIZE = 50;
export const REPORTS_PAGE_MAX = 100;
// Shared with the queue index in lib/db.js, which must match it exactly.

export async function getReportsPage({ status = 'open', limit = REPORTS_PAGE_SIZE, cursor = null } = {}) {
  const size = Math.min(Math.max(Number.parseInt(limit, 10) || REPORTS_PAGE_SIZE, 1), REPORTS_PAGE_MAX);
  const st = typeof status === 'string' && status ? status.slice(0, 40) : 'open';
  const m = typeof cursor === 'string' ? cursor.match(/^([012]):([1-9]\d{0,17})$/) : null;
  const { rows } = await query(
    `select id, data, ${REPORT_PRIORITY_SQL} as prio from reports
      where ($1::text = 'all' or data->>'status' = $1::text)
        and ($2::int is null or ${REPORT_PRIORITY_SQL} > $2::int
             or (${REPORT_PRIORITY_SQL} = $2::int and id < $3::bigint))
      order by ${REPORT_PRIORITY_SQL}, id desc
      limit $4`,
 ...
 */
export async function addReport(report, { holdMedia = null, requireReporter = false } = {}) {
  ...
  return withTransaction(async (client) => {
    // `requireReporter` (every filing route, round-20 social#1): the
    // reporter's users row FOR KEY SHARE first (lib/author-lock.js), so a
    // report filed while its reporter's account is being deleted either lands
    // before the purge anonymises their reports, or is refused with
    // AUTHOR_ACCOUNT_GONE -- never kept with a reporterId the purge already
    // passed.
    if (requireReporter) await lockAuthorRow(client, entry.reporterId);
 ...
 *
 * A participant whose account no longer exists is copied as null (round-20
 * media#0 / social#0) -- the same rule as attachReportTargets and the
 * deletion-time copy above: a conversation outlives a deleted participant and
 * its participant list still names them, and a copy taken later (an admin
 * takedown of a message in that thread) wrote the purged id back into a
 * stored record the admin panel then showed.
 */
export async function snapshotMessage(message, { conversationId, participantIds } = {}, client = null) {
  ...
  if (ids.length) {
    const { rows } = await (client || { query }).query('select id from users where id = any($1::text[])', [ids]);
  ...
  return {
    ...
    participantIds: ids.map((p) => (live.has(p) ? p : null)),
    capturedAt: new Date().toISOString(),
  };
}
    ...
    await client.query(
      `update reports r
          set data = r.data || jsonb_build_object('reportedContent', jsonb_build_object(
                ...
                'createdAt', m->'createdAt', 'conversationId', c.id,
                -- A participant whose account is already gone (a reporter
                -- purged earlier, round-17 legal-journeys#1) is copied as
                -- null, never put back by this later copy (round-18 social#1).
                'participantIds', (select coalesce(jsonb_agg(case when exists (select 1 from users u where u.id = p #>> '{}')
                                                                  then p else 'null'::jsonb end order by ord), '[]'::jsonb)
                                     from jsonb_array_elements(case when jsonb_typeof(c.data->'participantIds') = 'array'
                                                                    then c.data->'participantIds' else '[]'::jsonb end)
...
/**
 * What of a report's reporter trail may reach the admin panel (round-18
 * social#1). A DM conversation id is the two participant ids joined
 * ("<a>__<b>"), so it names the reporter; the panel never needs it (the
 * resolve route reads the stored report server-side), so it is dropped from
 * the report -- top level included -- and from its stored copies. A purged
 * reporter's id is never returned either, and a copy's participant list reads
 * null for them.
 *
 * EVERY admin response that carries a report goes through this: GET
 * /api/admin/reports (attachReportTargets) and all three report bodies of
 * /api/admin/reports-resolve (success, the 409 already_resolved, reopen) --
 * round-19 media#0 / admin-ui#0 / legal-journeys#0: round 18 stripped only
 * the first. The STORED row is never changed (moderation needs the id).
 * `keepConversationId` is for attachReportTargets alone, which reads the
 * top-level id to find the message and then drops it itself.
 */
export function stripReporterTrail(r, { keepConversationId = false } = {}) {
  if (!r || typeof r !== 'object') return r;
  const out = { ...r };
  if (!keepConversationId) delete out.conversationId;
  const strip = (copy) => {
    if (!copy || typeof copy !== 'object') return copy;
    const { conversationId: _c, ...rest } = copy;
    return rest;
  };
  if (out.reportedContent) out.reportedContent = strip(out.reportedContent);
  if (out.removedContent) out.removedContent = strip(out.removedContent);
  if (out.reporterDeletedAt) delete out.reporterId;
  return out;
}
...
export async function attachReportTargets(reports) {
  ...
  if (messageReports.length) {
    ...
    // Which participants still have an account (round-18 social#1): a
    // conversation survives its reporter's deletion (it keeps the other
    // person's messages, and its participantIds), so the live row still
    // names the purged reporter. A gone account is returned as null.
    const participantIds = new Set();
    for (const r of messageReports) {
      const convo = byId.get(r.conversationId);
      if (Array.isArray(convo?.participantIds)) for (const p of convo.participantIds) participantIds.add(String(p));
    }
    const { rows: livePeople } = participantIds.size
      ? await query('select id from users where id = any($1::text[])', [[...participantIds]])
      : { rows: [] };
    const liveIds = new Set(livePeople.map((u) => String(u.id)));
    for (const r of messageReports) {
      const convo = byId.get(r.conversationId);
      const m = (Array.isArray(convo?.messages) ? convo.messages : []).find((x) => x && x.id === r.targetId);
      if (!m) continue;
      ...
      messages.set(`${r.conversationId}|${r.targetId}`, {
        ...
        participantIds: Array.isArray(convo?.participantIds)
          ? convo.participantIds.map(String).map((p) => (liveIds.has(p) ? p : null))
          : [],
  ...
  return reports.map((r) => stripReporterTrail(r, { keepConversationId: true })).map((r) => {
    ...
    if (r.targetType === 'message') {
      ...
      const { conversationId: _convo, ...rest } = r;
      const { conversationId: _targetConvo, ...shownTarget } = target;
      return { ...rest, targetId: String(r.targetId ?? ''), target: shownTarget };
    }
    const tid = normalizeTargetId(r.targetId);
