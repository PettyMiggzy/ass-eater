 ...
 */
export function mintBridgeToken(user, creatorStatus = null) {
  const payload = {
    typ: 'bridge',
    uid: String(user.id),
    ...
    creatorStatus: user.role === 'creator' ? (CREATOR_STATUSES.has(creatorStatus) ? creatorStatus : 'banned') : null,
 ...
 * "the creator whose site user id is `uid` now stands at `creatorStatus`".
 ...
 * mintBridgeToken.
 */
export function mintBridgeStatusToken(uid, creatorStatus) {
  const payload = {
    typ: 'bridge_status',
    uid: String(uid),
    jti: crypto.randomUUID(),
    creatorStatus: CREATOR_STATUSES.has(creatorStatus) ? creatorStatus : 'banned',
    exp: Date.now() + MAX_AGE_MS,
  };
  const payloadB64 = Buffer.from(JSON.stringify(payload)).toString('base64url');
  return `${payloadB64}.${sign(payloadB64)}`;
