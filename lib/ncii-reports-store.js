...
/*
 * THE ADMIN VIEW OF A DM TAKEDOWN (round-20 media#0 / social#0). A DM's
 * conversation id is its two participants' ids joined ("<a>__<b>",
 * lib/messages-store.js pairId), so a takedown entry that carries it names
 * both people -- including one whose account has since been deleted, which is
 * exactly the re-identification the round-19 content-lookup fix set out to
 * stop. The panel needs only the message id and the copy, so every admin
 * response drops `conversationId` from a takedown's target and copy (in
 * `takedowns` and `history`, and in the audit rows of
 * /api/admin/content-takedown). The STORED records keep it (moderation and the
 * audit trail need it); a deleted participant is written as null in the copy
 * (lib/content-takedown.js) and nulled on deletion (purgeUserContent).
 */
export function adminTakedownEntryView(entry) {
  if (!entry || typeof entry !== 'object') return entry;
  const out = { ...entry };
  if (out.target && typeof out.target === 'object') {
    const { conversationId: _c, ...target } = out.target;
    out.target = target;
  }
  if (out.snapshot && typeof out.snapshot === 'object') {
    const { conversationId: _s, ...snapshot } = out.snapshot;
    out.snapshot = snapshot;
  }
  return out;
}

/** A takedown request as every admin response returns it (see adminTakedownEntryView). */
export function adminNciiReportView(report) {
  if (!report || typeof report !== 'object') return report;
  const out = { ...report };
  if (Array.isArray(out.takedowns)) out.takedowns = out.takedowns.map(adminTakedownEntryView);
  if (Array.isArray(out.history)) out.history = out.history.map((h) => (h && h.target ? adminTakedownEntryView(h) : h));
  return out;
}

/*
 * The same view for a PAGE of requests, with every copied participant whose
 * account no longer exists read as null (round-21 media#0 / social#1). The
 * purge on account deletion nulls the deleted id in stored takedown copies,
 * but that UPDATE only reaches rows that already name the account when it
 * starts: a DM takedown committed while the deletion is in flight (its copy
 * read the account as live) keeps the id for good. So, like
 * listModerationActions (lib/content-takedown.js), liveness is decided when
 * the admin reads: one users query for the whole page. The purge stays as
 * belt and braces. (Locking users inside takeDownContent would invert the
 * deletion's users -> ncii_reports lock order.)
 */
function snapshotParticipantIds(entry) {
  const ids = entry?.snapshot?.participantIds;
  return Array.isArray(ids) ? ids : [];
}
function nullGoneParticipants(entry, live) {
  if (!entry || typeof entry !== 'object' || !Array.isArray(entry.snapshot?.participantIds)) return entry;
  return {
    ...entry,
    snapshot: { ...entry.snapshot, participantIds: entry.snapshot.participantIds.map((p) => (p != null && live.has(String(p)) ? String(p) : null)) },
  };
}
export async function adminNciiReportsView(reports) {
  const list = Array.isArray(reports) ? reports : [];
  const ids = new Set();
  for (const r of list) {
    for (const t of Array.isArray(r?.takedowns) ? r.takedowns : []) for (const p of snapshotParticipantIds(t)) if (p != null) ids.add(String(p));
    for (const h of Array.isArray(r?.history) ? r.history : []) for (const p of snapshotParticipantIds(h)) if (p != null) ids.add(String(p));
  }
  let live = new Set();
  if (ids.size) {
    // Fails CLOSED: if the liveness read fails, every copied participant reads
    // as null rather than printing an id that may belong to a purged account
    // (and a resolve that has already committed still answers 200).
    try {
      const { rows } = await query('select id from users where id = any($1::text[])', [[...ids]]);
      live = new Set(rows.map((u) => String(u.id)));
    } catch (err) {
      console.error('[ncii-reports] participant liveness read failed; hiding participant ids:', err);
    }
  }
  return list.map((report) => {
    const out = adminNciiReportView(report);
    if (!out || typeof out !== 'object') return out;
    if (Array.isArray(out.takedowns)) out.takedowns = out.takedowns.map((t) => nullGoneParticipants(t, live));
    if (Array.isArray(out.history)) out.history = out.history.map((h) => nullGoneParticipants(h, live));
    return out;
  });
}
/** One request through adminNciiReportsView. */
export async function adminNciiReportViewLive(report) {
  if (!report || typeof report !== 'object') return report;
  const [out] = await adminNciiReportsView([report]);
  return out;
}
