 ...
 * (see preserveCreatorMedia below); the report records their pathnames in
...
export async function resolveNciiReport(id, action, { creatorId = null, resolvedBy = 'admin', reason = null, requireTakedown = false, contentGone = false } = {}) {
  ...
  const result = await withTransaction(async (client) => {
    ...
    // takedown below still takes everything off sale and hides it; it just
    // no longer destroys what has to be kept. Other NCII categories keep the
    // plain delete: there the law wants the content gone.
    //
    // It runs BEFORE the listing takedown, so this transaction takes the
    // creator's file locks before any listing row lock -- the order every
    // other path uses (lib/media-locks.js). The takedown used to run first and
    // row-lock every listing, and this preservation then took the file locks:
    // the reverse of checkout and of a listing quarantine, so the two could
    // deadlock and the 48-hour resolve failed with a 500 (round-8
    // media#0/social#0).
    let preserved = [];
    ...
    if (creator?.status === 'banned') {
      ...
      // and deleteMediaQuietly both skip them anyway, but there is no reason
      // to hand them to a deleter at all.
      if (preserved.length) {
...
// Quarantines every file of a creator for the resolve (see above). Runs
// before any listing row is locked by this transaction.
async function preserveCreatorMedia(client, creatorId, reportId) {
  const items = (await lockCreatorMediaItems(client, creatorId)) || [];
