...
export function findCircumventionInTags(input) {
  ...
  // 1. Each tag on its own -- as a TAG, so the strict minor-age rule applies
  //    (round-16 accounts#0-#3: a tag is a short label, not a sentence).
  for (const text of [...strings, ...stored]) {
    const hit = hitOf(screenPublicText(text, { context: 'tag' }), text);
    if (hit) return hit;
  }

  // 2. Prohibited phrases across runs of whole single-word tags. The run is
  //    a LABEL split over chips, not a sentence, so it is judged like one tag
  //    with the strict minor-age rule and squashed whole (round-18
  //    srv-auth-core#1): ['16', 'girl'], ['16', 'hot', 'girl'], ['sixteen',
  //    'and', 'ready'] and ['fifteen', 'years', 'old'] published as adjacent
  //    chips while each one alone as a tag was refused. Plurals stay counts
  //    (['16', 'girls']) and codes stay codes (['1080p', 'girl']).
  for (const list of [strings, stored]) {
    for (const run of singleWordRuns(list)) {
      if (run.length < 2) continue;
      const text = run.join(' ');
      const found = detectProhibitedTerms(text, { strictAge: true, squashWhole: true });
