 ...
 *
 * Runs on every call (an admin re-saving a ban retries a takedown that failed
 * part-way); a listing whose files were already deleted keeps its original
 * mediaDeletedAt and is not deleted again.
 *
 * Returns { listings, kept: listingId[], cleanup }.
 */
export async function removeListingsForCreator(creatorId, { moderation = false, keepPaid = false } = {}) {
  const { rows } = await query(
    `with cur as (
       select l.id,
  ...
  const cleanup = await deleteMediaQuietly(doomed.flatMap(allFiles));
  return { listings, kept, cleanup };
}
...
export async function removeListingMediaForOwner(listingId, creatorId, src) {
  ...
  const { listing, removed, retained } = await withTransaction(async (client) => {
    ...
    return { listing: rowToRecord(updated[0]), removed: gone, retained: paid };
  });
  if (!retained) {
    const stillUsed = allFiles(listing).some((m) => m && m.src === removed.src);
    if (!stillUsed) await deleteMediaQuietly(removed);
  }
