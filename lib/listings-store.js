 ...
 * punctuation), each tag on its own and all of them JOINED -- tags render
 * publicly as adjacent #chips, so "venmo @janedoe" or a phone number split
 * across two tags must be caught the same as in a title, and so must a
 * prohibited phrase split across two tags ("barely", "legal"; "school",
 * "girl"): no single word is prohibited, the chips side by side are. The
 * joined check used to run only the payment filter, so those passed every
 * check here, on the admin go-live re-screen too. Both checks now run
...
export function findCircumventionInTags(input) {
  ...
  for (const text of [...strings, strings.join(' '), sanitizeTags(strings).join(' ')]) {
    ...
    if (hit) return { kind: hit.kind, reasons: hit.reasons, snippet: text, message: hit.message };
  }
  return null;
}
