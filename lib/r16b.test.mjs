...
//  - legal-journeys#0: a buyer whose account is gone has name/address/tracking
...
section('legal-journeys#0: every deletion path erases the buyer\'s shipping data, including at ship time');
{
  ...
  check('...and the moment it does, the deleted buyer\'s address and tracking number are erased',
    d.status === 'shipped' && d.shippingAddress === null && d.trackingNumber === null && !!d.trackingErasedAt && d.carrier === 'UPS', JSON.stringify(d));
  ...
  check('deleting a creator erases their own shipped purchases', s1.shippingAddress === null && s1.trackingNumber === null, JSON.stringify(s1));
  ...
  check('...and erases it when it ships', s2.status === 'shipped' && s2.shippingAddress === null && s2.trackingNumber === null, JSON.stringify(s2));
}
...
section('legal-journeys#2: the seller is not told the buyer asked for erasure');
{
  ...
  check('...and says only that tracking can no longer change', view.trackingCorrectionsLeft === 0);
  const buyerView = (await orders.getOrdersForBuyer(String(fan.id)))[0];
  check('the buyer still sees their own erasure', !!buyerView.trackingErasedAt);
  const err = await orders.markOrderShipped(o, creator.id, { carrier: 'UPS', trackingNumber: '1Z999AA10123456793' }).then(() => null, (e) => e);
  check('a correction is refused with a neutral message', err && !/buyer/i.test(err.message), err && err.message);
}
...
section('money#0/#1, dashboard#0/#1: real-shaped tracking numbers always pass');
{
  ...
  for (const [c, t] of [['UPS', '1ZWHATSAPP12345678'], ['UPS', '1ZSNAPJESSXO123456'], ['Other', 'VENMO12345678'], ['USPS', 'KIK123456789']]) {
    check(`an app name is refused AND flagged: ${c} | ${t}`, E(c, t)?.suspicious === true, JSON.stringify(E(c, t)));
  }
  for (const [c, t] of [['Other', 'jess@mail.com123456'], ['USPS', 'JESS61755512340'], ['UPS', '1Z12345'], ['FedEx', 'abc']]) {
    const e = E(c, t);
...
section('fix-up: the ship write and the deleted buyer\'s erasure commit together');
{
  ...
  await query(`create or replace function r16b_fail_erase() returns trigger language plpgsql as $$
    begin if new.data ? 'trackingErasedAt' and not (old.data ? 'trackingErasedAt') then raise exception 'r16b simulated failure'; end if; return new; end $$`);
  ...
  check('the retry ships and erases in one commit', again.statusCode === 200 && d2.status === 'shipped'
    && d2.shippingAddress === null && d2.trackingNumber === null, JSON.stringify(d2));
