...
section('legal-journeys#2: the seller is not told the buyer asked for erasure');
{
  ...
  check('the buyer still sees their own erasure', !!buyerView.trackingErasedAt && buyerView.trackingNumber === null);
  const fixed = await orders.markOrderShipped(o, creator.id, { carrier: 'UPS', trackingNumber: '1Z999AA10123456793' });
  check('a correction answers like any other, into the seller copy only', fixed.trackingNumber === '1Z999AA10123456793'
    && (await orderData(o)).trackingNumber === null, JSON.stringify(fixed));
}
