...
// Round 20 (legal-journeys#0): 28 C.F.R. 75.4 counts seven years from the
// record's creation or last amendment, not from the production date -- so a
// record created today (for content produced a week ago) keeps until today +
// 7 years. The old expectation ('2033-09-19', production + 7) was the bug.
check('retention is seven years from the record\'s creation (not production)', (() => {
  const floor = new Date(); floor.setUTCFullYear(floor.getUTCFullYear() + 7);
  return new Date(rec.retainUntil).getTime() >= floor.getTime() - 60_000;
})(), rec.retainUntil);
