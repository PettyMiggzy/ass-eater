...
section('dashboard#0: correcting a shipped order\'s tracking keeps its ship date');
{
  ...
  // erased the moment it ships, so the buyer here is a real (minimal) account.
  await query("insert into users (id, data) values ($1, jsonb_build_object('email', $1 || '@buyer.test', 'role', 'fan')) on conflict do nothing", ['b1']);
