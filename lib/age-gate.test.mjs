...
// Owner decision 2026-09-26: every visitor outside the US verifies.
for (const cc of ['CA', 'GB', 'FR', 'IT', 'DE', 'AU', 'BR']) {
  check(`non-US (${cc}) is gated`, rewrittenTo(await run('/home', { 'x-vercel-ip-country': cc })).endsWith('/blocked-region'));
}
check('non-US: API answers 451', (await run('/api/creators', { 'x-vercel-ip-country': 'GB' })).status === 451);
check('non-US: exempt paths still reachable', !rewrittenTo(await run('/terms', { 'x-vercel-ip-country': 'GB' })));
check('no geo headers at all (dev) is not gated', !rewrittenTo(await run('/home')));
// Round-2 gates-token#0: on Vercel a missing country means "location unknown".
process.env.VERCEL = '1';
check('on Vercel, no country header is gated', rewrittenTo(await run('/home')).endsWith('/blocked-region'));
check('on Vercel, an empty country header is gated', rewrittenTo(await run('/home', { 'x-vercel-ip-country': '' })).endsWith('/blocked-region'));
check('on Vercel, no country: API answers 451', (await run('/api/creators')).status === 451);
check('on Vercel, no country: exempt paths still reachable', !rewrittenTo(await run('/terms')));
check('on Vercel, a non-US country is gated', rewrittenTo(await run('/home', { 'x-vercel-ip-country': 'CA' })).endsWith('/blocked-region'));
...
check('a verified cookie passes the proxy for a non-US visitor', !rewrittenTo(await run('/home', { 'x-vercel-ip-country': 'GB' }, `oa_age_verified=${pTok}`)));
...
// Round-2 gates-token#2: invite cookies are revoked when the key rotates.
{
  const prev = await import('./preview-access.js');
  const tok = await prev.createPreviewToken(prev.previewSecret());
  check('an invite cookie passes while its key is current', !!(await prev.verifyPreviewToken(prev.previewSecret(), tok)));
  check('the invite cookie passes the proxy', !rewrittenTo(await run('/home', {}, `oa_preview=${tok}`)));
  const payload = JSON.parse(Buffer.from(tok.split('.')[0], 'base64url').toString());
  check('invite fingerprint is not the key', typeof payload.kf === 'string' && !payload.kf.includes('preview-key-for-test'));
  process.env.PREVIEW_ACCESS_KEY = 'preview-key-rotated';
  check('after rotation the invite cookie is refused', (await prev.verifyPreviewToken(prev.previewSecret(), tok)) === null);
  check('after rotation the proxy hides /home again', rewrittenTo(await run('/home', {}, `oa_preview=${tok}`)).endsWith('/coming-soon'));
  // A pre-change cookie with no kf is refused too.
  // (correctly signed with the preview-derived key, so only the missing kf refuses it)
  const enc = new TextEncoder();
  const root = await crypto.subtle.importKey('raw', enc.encode(prev.previewSecret()), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
  const derived = await crypto.subtle.sign('HMAC', root, enc.encode('oa:preview:v1'));
  const dk = await crypto.subtle.importKey('raw', derived, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
  const legacyPayload = Buffer.from(JSON.stringify({ typ: 'preview_access', exp: Date.now() + 60_000 })).toString('base64url');
  const legacySig = Buffer.from(await crypto.subtle.sign('HMAC', dk, enc.encode(legacyPayload))).toString('base64url');
  check('a signed cookie with no fingerprint is refused', (await prev.verifyPreviewToken(prev.previewSecret(), `${legacyPayload}.${legacySig}`)) === null);
}
delete process.env.PREVIEW_ACCESS_KEY;
