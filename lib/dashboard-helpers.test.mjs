...
test('an unchanged saved wallet is neither validated nor sent (round-19 dashboard#0)', () => {
  // A legacy stored value that fails today's rule (bad EIP-55 checksum).
  const legacy = '0xAbCdEf0123456789aBcDeF0123456789AbCdEf01';
  assert.ok(payoutWalletError(legacy), 'fixture must fail the current rule');
  const draft = { ...draftFromCreator({ name: 'Mia', walletAddress: legacy }), bio: 'new bio' };
  const built = profileFieldsFromDraft(draft, legacy);
  assert.equal(built.error, undefined);
  assert.ok(!('walletAddress' in built.fields));
  assert.equal(built.fields.bio, 'new bio');
  // Whitespace-only difference still counts as unchanged.
  assert.equal(profileFieldsFromDraft({ ...draft, walletAddress: ` ${legacy} ` }, legacy).error, undefined);
  // An edited wallet is still validated and sent.
  assert.ok(profileFieldsFromDraft({ ...draft, walletAddress: '0xnope' }, legacy).error);
  const good = '0x' + '1'.repeat(40);
  assert.equal(profileFieldsFromDraft({ ...draft, walletAddress: good }, legacy).fields.walletAddress, good);
  // Clearing it is a change and is sent as ''.
  assert.equal(profileFieldsFromDraft({ ...draft, walletAddress: '' }, legacy).fields.walletAddress, '');
});
