...
section('money#0 / dashboard#1: real carrier numbers with letter runs pass and are never logged');
{
  ...
  for (const [c, t] of [['Other', 'jess@mail.com123456'], ['UPS', '1Z12345'], ['USPS', 'jessxo2']]) {
    const e = E(c, t);
...
section('money#1 / legal-journeys#0: closing a deleted buyer\'s order erases their address');
{
  ...
  check('the sweep erases the closed and shipped orders of a gone buyer', swept === 2 && (await orderData(ghost)).shippingAddress === null
    && (await orderData(ghostShipped)).trackingNumber === null, String(swept));
...
section('money#2 / dashboard#0: the seller\'s view does not change when the buyer erases');
{
  ...
  check('the buyer\'s own copy of the number is erased', (await orderData(o)).trackingNumber === null);
  check('...but the seller still sees the number they entered', after.trackingNumber === '9405511899223197428490', JSON.stringify(after));
  ...
  check('the buyer sees no number and no seller copy', buyer.trackingNumber === null && !('sellerTrackingNumber' in buyer), JSON.stringify(buyer));
  const admin = (await orders.getOrderSummariesForAdmin({ orderId: o }))[0];
  check('the admin summary carries no seller copy either', admin && !('sellerTrackingNumber' in admin) && admin.trackingNumber === null, JSON.stringify(admin));
  ...
  check('...but writes nothing back into the buyer\'s copy', d.trackingNumber === null && d.sellerTrackingNumber === '9405511899223197428506'
    && d.trackingHistory.every((h) => h.trackingNumber === null), JSON.stringify(d));
  check('...and notifies nobody', (await count(`select count(*)::int as n from notifications where user_id = $1 and type = 'tracking_updated'`, [String(fan.id)])) === 0);
  ...
  check('...while the stored buyer copy is erased', d2.trackingNumber === null && d2.shippingAddress === null && !!d2.trackingErasedAt, JSON.stringify(d2));
