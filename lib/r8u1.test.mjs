...
async function reset() {
  await query(`truncate creators, users, listings, orders, credit_balances, credit_ledger, conversations, wall_posts,
    notifications, reports, violations, login_attempts, wall_blocks restart identity`);
...
section('dashboard#0: a block-only row is listed to the blocker, not the blocked, and not after unblock');
{
  ...
  // Round 10 (social#0): a wall block is its own record (wall_blocks), not a
  // conversation row; the inbox API appends it as an opaque row (below).
  const mine = await messages.listWallBlocksFor(c.user.id);
  check('the blocker sees the block-only row', mine.length === 1 && mine[0].blockOnly === true && mine[0].blockedByMe === true, JSON.stringify(mine));
  ...
  check('the empty row drops out of the blocker\'s inbox once unblocked', (await messages.getConversationsForUser(c.user.id)).length === 0
    && (await messages.listWallBlocksFor(c.user.id)).length === 0);
...
section('public-pages#0: store helpers');
{
  ...
  // Round 10 (social#0): wall flags come from WALL blocks only -- a DM block
  // made by user id must not flag anonymous comments.
  const dmOnly = await wall.wallBlockFlagsFor(c.user.id, [{ id: 1, authorId: fan.id }]);
  check('wallBlockFlagsFor ignores a DM block', dmOnly.get('1') === false);
  await messages.setWallBlocked(c.user.id, fan.id, true);
  await messages.setWallBlocked(other.id, c.user.id, true); // other wall-blocked the creator
