...
section('dashboard#0: a block-only row is listed to the blocker, not the blocked, and not after unblock');
{
  ...
  // The existing Inbox Unblock path: POST /api/messages/block {userId, blocked:false}.
  const unblock = await call(msgBlockRoute, { user: c.user, body: { userId: String(row.other.userId), blocked: false } });
  check('the blocker can unblock from the inbox', unblock.statusCode === 200 && unblock.body.conversation?.blockedByMe === false, JSON.stringify(unblock.body));
