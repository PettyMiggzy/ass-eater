...
//   LAUNCHPAD_V4_ADDRESS      - optional, the deployed OnlyAssLaunchpadV4 address (leave unset to
//                               disable creator-launched-token pricing for now; wire it up later
//                               via setLaunchpad(...))
...
async function main() {
  ...
  const nft = await NFT.deploy(deployer.address, platformWallet, feeBps, onlyAssToken, launchpad, contractMetadataURI);
  ...
  console.log('launchpad:', launchpad, launchpad === ethers.ZeroAddress ? '(not set -- creator-launched-token pricing disabled until setLaunchpad is called)' : '');
