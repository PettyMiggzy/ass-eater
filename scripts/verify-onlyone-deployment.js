...
// Run this the moment $ONLYONE actually deploys (Kekfun.xyz auction closing
...
//
// Required:
//   ONLYONE_TOKEN_ADDRESS   - the address to check
// Optional, expected name/symbol to assert against (skips the check if unset):
//   ONLYONE_EXPECTED_NAME
//   ONLYONE_EXPECTED_SYMBOL
// Optional, to also look up a specific Uniswap V4 pool (skips if any are unset):
//   ONLYONE_V4_POOL_MANAGER, ONLYONE_V4_CURRENCY0, ONLYONE_V4_CURRENCY1,
//   ONLYONE_V4_TICK_SPACING, ONLYONE_V4_HOOKS, ONLYONE_POOL_FEE
...
async function main() {
  const address = process.env.ONLYONE_TOKEN_ADDRESS;
  if (!address) throw new Error('Set ONLYONE_TOKEN_ADDRESS to the address you want to verify.');
  ...
  const expectedName = process.env.ONLYONE_EXPECTED_NAME;
  const expectedSymbol = process.env.ONLYONE_EXPECTED_SYMBOL;
  ...
  const { ONLYONE_V4_POOL_MANAGER, ONLYONE_V4_CURRENCY0, ONLYONE_V4_CURRENCY1, ONLYONE_V4_TICK_SPACING, ONLYONE_V4_HOOKS, ONLYONE_POOL_FEE } = process.env;
  if (ONLYONE_V4_POOL_MANAGER && ONLYONE_V4_CURRENCY0 && ONLYONE_V4_CURRENCY1) {
    console.log('\nChecking the V4 pool...');
    const fee = Number(ONLYONE_POOL_FEE ?? 3000);
    const tickSpacing = Number(ONLYONE_V4_TICK_SPACING ?? 60);
    const hooks = ONLYONE_V4_HOOKS ?? ethers.ZeroAddress;
    const poolId = computePoolId(ONLYONE_V4_CURRENCY0, ONLYONE_V4_CURRENCY1, fee, tickSpacing, hooks);
    const slot = poolStateSlot(poolId);
    const manager = new ethers.Contract(ONLYONE_V4_POOL_MANAGER, poolManagerAbi, provider);
    ...
    if (sqrtPriceX96 === 0n) {
      ...
      console.log('Pool is live. Set ONLYONE_POOL_VERSION=v4 and the matching ONLYONE_V4_* vars in server/.env to point the price oracle at it.');
    }
  } else {
    console.log('\n(Skipped V4 pool check -- set ONLYONE_V4_POOL_MANAGER/CURRENCY0/CURRENCY1 to also check a specific pool.)');
  }
}
